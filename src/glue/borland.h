...
#define BT_LAST_PREDEF                  \
    23 /* Any type index above this is  \
...
#define BTID_PSTR                   \
    0x03 /* Pascal string:          \
          ...
          */
#define BTID_SCHAR                      \
    0x04 /* 1-byte signed int range:    \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_SINT                       \
    0x05 /* 2-byte signed int range:    \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_SLONG                      \
    0x06 /* 4-byte signed int range:    \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_SQUAD                      \
    0x07 /* 8-byte signed int range:    \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_UCHAR                      \
    0x08 /* 1-byte unsigned int range:  \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_UINT                       \
    0x09 /* 2-byte unsigned int range:  \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_ULONG                      \
    0x0a /* 4-byte unsigned int range:  \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_UQUAD                      \
    0x0b /* 8-byte unsigned int range:  \
          *	- parent type index (index) \
          *	- lower bound (dword)       \
          *	- upper bound (dword)       \
          */
#define BTID_PCHAR                                \
    0x0c /* 1-byte unsigned int range (Pascal     \
          ...
          */
#define BTID_FLOAT            \
    0x0d /* IEEE 32-bit real: \
          *	- nothing         \
          */
#define BTID_TPREAL                     \
    0x0e /* Turbo Pascal real (6-byte): \
          *	- nothing                   \
          */
#define BTID_DOUBLE           \
    0x0f /* IEEE 64-bit real: \
          *	- nothing         \
          */
#define BTID_LDOUBLE          \
    0x10 /* IEEE 80-bit real: \
          *	- nothing         \
          */
#define BTID_BCD4       \
    0x11 /* 4-byte BCD: \
...
#define BTID_FAR                                     \
    0x16 /* far pointer:                             \
          ...
          */
#define BTID_SEG                               \
    0x17 /* segment pointer:                   \
          *	- index of type pointed to (index) \
          *	- extra byte                       \
          */
#define BTID_NEAR386                           \
    0x18 /* 32-bit near pointer:               \
          *	- index of type pointed to (index) \
          *	- segment base (byte):             \
          *	    0x00    unspecified            \
          *	    0x01    ES                     \
...
#define BTID_UNION                                \
    0x1f /* Union:                                \
          ...
          *	  file.                               \
          */
#define BTID_VLSTRUCT                             \
    0x20 /* Very Large Structure:                 \
...
#define BTID_ENUM                                    \
    0x22 /* Enumerated type:                         \
          ...
          *	  file.                                  \
          */
#define BTID_FUNCTION                               \
    0x23 /* Function/procedure:                     \
          *	- return type (index)                   \
...
#define BTID_SET                  \
    0x25 /* Pascal Set:           \
          ...
          */
#define BTID_TFILE            \
    0x26 /* Pascal text file: \
          *	- nothing         \
          */
#define BTID_BFILE                \
    0x27 /* Pascal binary file:   \
          *	- record type (index) \
          */
#define BTID_BOOL           \
    0x28 /* Pascal boolean: \
          ...
          */
#define BTID_PENUM                                  \
    0x29 /* Pascal enumerated type (no arithmetic): \
          *	- parent type (index)                   \
          *	- lower bound (signed word)             \
          *	- upper bound (signed word)             \
          */
#define BTID_PWORD                                  \
    0x2a /* pword (some MASM thing I've forgotten): \
          *	- nothing                               \
          */
#define BTID_TBYTE                                    \
    0x2b /* 10-byte integer, usually an encoded real: \
          *	- nothing                                 \
          */
#define BTID_SPECIALFUNC                           \
...
#define BTID_CLASS                                  \
    0x2e /* C++ class:                              \
          ...
          */
#define BTID_MEMBERPTR                               \
    0x33 /* Type pointed to by a class member ptr:   \
          ...
          */
#define BTID_NREF                               \
    0x34 /* Near reference (parameter passed by \
          * reference, I think, not value):     \
          *	- type pointed to (index)           \
          *	- extra byte (byte), always 0       \
          */
#define BTID_FREF                              \
    0x35 /* Far reference (parameter passed by \
          * reference, I think, not value):    \
          *	- type pointed to (index)          \
          *	- extra byte (byte), always 0      \
          */
...
#define BSC_REGPARAM              \
    0x0c /* Register Parameter:   \
          *  - register id (byte) \
          */

#define BSC_FUNCTION           \
    0x18 /* Static function:   \
          *  - group (index)   \
          *  - segment (index) \
          *  - offset (word)   \
          */
#define BSC_GLOBAL_FUNCTION                     \
    0x28 /* Global function:                    \
          *  - group (index)                    \
          *  - segment (index)                  \
          *  - offset (word)                    \
          ...
          * more information...                 \
          */
#define BSC_OPTIMIZED                          \
    0x08 /* Variable's been optimized:         \
...
#define BCDF_UNDERSCORES                                     \
    0x08                /* Set if underscores were prepended \
