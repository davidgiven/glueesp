 ...
 *
 ***********************************************************************/
VMHandle Obj_Open(const char* file, /* Name of file to open */
 ...
 *
 ***********************************************************************/
static int ObjIntTypeEqual(word t1, VMHandle f2, void* t2Base, word t2)
{
    ObjType* ot;
 ...
 ***********************************************************************/
int ObjTypedefEqual(
    void* base, ObjType* tdef, VMHandle f2, void* t2Base, word t2, int* result)
{
    ...
    *result = 1; /* assume things will go to plan */
    ...
    {
        if (Sym_Find(symbols, seg_Segments[i]->syms, id, &block, &off, FALSE))
        {
            ObjSym* sym;
            ...
            {
                *result = Obj_TypeEqual(
                    symbols, base, sym->u.typeDef.type, f2, t2Base, t2);
                retval = 1;
            }
            else
 ...
 *
 ***********************************************************************/
int Obj_TypeEqual(VMHandle f1, /* VM file containing any IDs for type1 */
    ...
    word t2)                   /* Type descriptor 2 */
{
    if (t1 & OTYPE_SPECIAL)
    {
        ...
        {
            return (
                Obj_TypeEqual(f1, t1Base, t1, f2, t2Base, type2->words[1]) ||
        ...
        if (((t1 & OTYPE_TYPE) == OTYPE_PTR) &&
            (OTYPE_IS_PTR(type2->words[0]) &&
    ...
    else if (t2 & OTYPE_SPECIAL)
    {
        ...
        {
            return (
                Obj_TypeEqual(f1, t1Base, type1->words[1], f2, t2Base, t2) ||
    ...
    else
    {
         ...
         * Should it?
         */
        if (OTYPE_IS_ARRAY(type1->words[0]) && !OTYPE_IS_ARRAY(type2->words[0]))
        {
        ...
        else
        {
             ...
             */
            retval =
                ((type1->words[0] == type2->words[0]) && Obj_TypeEqual(f1,
                                                             t1Base,
                                                             type1->words[1],
                                                             f2,
                                                             t2Base,
 ...
 *
 ***********************************************************************/
void Obj_PrintType(FILE* stream, VMHandle file, void* base, word type)
{
 ...
 *
 ***********************************************************************/
SegDesc* Obj_FindFrameVM(
    const char* file, /* File name (for PRIVATE segments) */
                       ...
                       * block */
{
    ...
    else
    {
        ...
        return ((SegDesc*)Seg_FindGroup(
            file, ST_Dup(fh, g->name, symbols, strings)));
    }
}
 ...
 *
 ***********************************************************************/
unsigned Obj_TypeSize(word type, /* The start of the type description */
                                  ...
                                  * the size in this case */
{
    ...
    else
    {
        ...
        {
            size = OTYPE_ARRAY_LEN(ot->words[0]) *
                   Obj_TypeSize(ot->words[1], base, mayBeUndefined);
        }
        else
        {
            ...
            for (i = 0; i < seg_NumSegs; i++)
            {
                        ...
                        FALSE))
                {
                    ...
                    sym = (ObjSym*)((genptr)VMLock(
                                        symbols, block, (MemHandle*)NULL) +
                                    off);
                    if ((sym->type == OSYM_STRUCT) ||
                        (sym->type == OSYM_RECORD) ||
                        (sym->type == OSYM_ETYPE) || (sym->type == OSYM_UNION))
                    {
                    ...
                    {
                        size = Obj_TypeSize(
                            sym->u.typeDef.type, base, mayBeUndefined);
                    }
                    VMUnlock(symbols, block);
                    break;
                }
 ...
 *
 ***********************************************************************/
static void ObjAllocSymBlock(ObjETSData* dp) /* Data describing state of
                                              * the copy */
{
        ...
        (dp->typeSize > OBJ_INIT_TYPES))
    {
        ...
        dp->nextType =
            (ObjType*)((genptr)VMLock(symbols, dp->types, &dp->tmem) +
                       dp->typeOff);
    }
 ...
 *
 ***********************************************************************/
static inline void ObjExpandBlock(ObjETSData* dp, /* State of the copy */
 ...
 *
 ***********************************************************************/
static void ObjDupType(word type, /* Type to copy */
    ...
    word* typeDest)               /* Place to store duplicate */
{
    ...
    else
    {
        ...
        if (OTYPE_IS_STRUCT(src.words[0]))
        {
             ...
             */
            ID id = ST_Dup(fh, OTYPE_STRUCT_ID(&src), symbols, strings);
 ...
 *
 ***********************************************************************/
static int ObjEnterStruct(VMHandle fh, /* Object file */
    ...
    Boolean all)                       /* TRUE to enter fields */
{
    ...
    if (!Sym_Find(symbols, dp->sd->syms, id, &otherBlock, &otherOff, FALSE))
    {
        ...
        {
            fs =
                (ObjSym*)((genptr)os + (os->u.sType.last - os->u.sType.first)) +
                2;
        }
        ...
        if (dp->nextSym->u.sType.first != 0)
        {
            ...
            dp->nextSym->u.sType.last = dp->nextSym->u.sType.first +
            ...
            while (os != fs)
            {
                 ...
                 */
                ObjDupType(dp->nextSym->u.sField.type,
                    dp,
                    fh,
                    dp->tbase,
                    &dp->nextSym->u.sField.type);
    ...
    else
    {
        ...
        {
            if (os->type == OSYM_RECORD && otherOS->type == OSYM_TYPEDEF &&
            ...
            else if ((os->type == OSYM_ENUM && otherOS->type == OSYM_METHOD) ||
                     (os->type == OSYM_METHOD && otherOS->type == OSYM_ENUM))
            {
                /*
 ...
 *
 ***********************************************************************/
static int ObjEnterTypedef(VMHandle fh, /* Object file */
    ObjSym** osPtr,                     /* I/O: current/next sym from
                                         * object file */
    word* nPtr,                         /* I/O: number of symbols left
                                         * in current block */
    ObjETSData* dp)                     /* State of the copy */
{
    ...
    if (!Sym_Find(symbols, dp->sd->syms, id, &otherBlock, &otherOff, FALSE))
    {
        ...
        ObjDupType(os->u.typeDef.type,
            dp,
            fh,
            dp->tbase,
            &dp->nextSym->u.typeDef.type);
 ...
 *
 ***********************************************************************/
static int ObjEnterEType(VMHandle fh, /* Object file */
    ObjSym** osPtr,                   /* I/O: current/next sym from
                                       * object file */
    word* nPtr,                       /* I/O: number of symbols left
                                       * in current block */
    ...
    if (!Sym_Find(symbols, dp->sd->syms, id, &otherBlock, &otherOff, FALSE))
    {
        ...
        if (dp->nextSym->u.sType.first != 0)
        {
            ...
            while (os != ms)
            {
                ...
                {
                    ObjDupType(os->u.varData.type,
                        dp,
                        fh,
                        dp->tbase,
                        &dp->nextSym->u.varData.type);
                }
    ...
    else
    {
        ...
        if (otherOS->type != os->type)
        {
            if (otherOS->type == OSYM_TYPEDEF)
            {
                 ...
                 */
                void* otherTBase =
                    VMLock(symbols, otherOSH->types, (MemHandle*)NULL);
                unsigned size;

                size = Obj_TypeSize(otherOS->u.typeDef.type, otherTBase, TRUE);
                VMUnlock(symbols, otherOSH->types);
        ...
        for (os += 1; retval && os != ms; os++)
        {
            ...
            {
                found =
                    Sym_Find(symbols, dp->sd->syms, mid, &mBlock, &mOff, FALSE);
            }
            ...
            else
            {
                ...
                if (os->type == OSYM_VARDATA)
                {
                    ...
                    {
                        Notify(NOTIFY_ERROR,
                            "%s: %i: definition of associated data differs "
 ...
 *
 ***********************************************************************/
static int ObjEnterConst(VMHandle fh, /* Object file */
    ObjSym** osPtr,                   /* I/O: current/next sym from
                                       * object file */
    word* nPtr,                       /* I/O: number of symbols left
                                       * in current block+1 */
 ...
 *
 ***********************************************************************/
static int ObjEnterExtType(VMHandle fh, /* Object file */
    ObjSym** osPtr,                     /* I/O: current/next sym from
                                         * object file */
    word* nPtr,                         /* I/O: number of symbols left
                                         * in current block+1 */
 ...
 *
 ***********************************************************************/
int Obj_EnterTypeSyms(const char* file, /* File name */
        ...
        for (os = ObjFirstEntry(osh, ObjSym); n > 0;)
        {
            ...
            switch (os->type)
            {
                ...
                default:
                {
                    ...
                    /* Undef addrsym */
                    Sym_EnterUndef(
                        symbols, sd->syms, tmp.name, &tmp, 0, fh, osh->types);
                    os++, n--;
                    break;
                }
            }
             ...
             */
            if (data.syms && ((data.symSize > OBJ_MAX_SYMS) ||
                                 (data.symOff == data.symSize)))
            {
                ObjSymHeader* tosh;

                MemInfo(mem, (genptr*)&tosh, 0);
                tosh->num =
                    (data.symSize - sizeof(ObjSymHeader)) / sizeof(ObjSym);
