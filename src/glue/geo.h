...
#define NO_LOAD_FIXED                                     \
    -3 /* lnum if library not auto-loaded but relocations \
        * are allowed */
} Library;
...
#define GH(field) \
    ((geosRelease >= 2) ? geoHeader.v2x.field : geoHeader.v1x.field)
#define GHEQ(field, val)           \
    if (geosRelease >= 2)          \
    {                              \
        geoHeader.v2x.field = val; \
    }                              \
    else                           \
    {                              \
        geoHeader.v1x.field = val; \
    }
#define GHA(field) \
    ((geosRelease >= 2) ? &(geoHeader.v2x.field) : &(geoHeader.v1x.field))
#define GH_ASSIGN(field, value)                             \
    ((geosRelease >= 2) ? ((geoHeader.v2x.field) = (value)) \
                        : ((geoHeader.v1x.field) = (value)))
...
extern void Library_ExportAs(char* name, char* alias, Boolean mustBeDefined);
extern void Library_Skip(int n);
extern void Library_SkipUntilNumber(int n);
extern void Library_SkipUntilConstant(char* name);
extern LibraryLinkValues Library_Link(char* name, /* File name */
...
extern int Library_UseEntry(
    SegDesc* sd, const ObjSym* os, int doInc, int errorIfPlatformViolated);
