...
#define PE_SHIPPED                                         \
    0x0002 /* This library is being shipped with the geode \
              at the specified protocol */

#define PE_EXEMPT                                       \
    0x0001 /* Exempt this library from any compile time \
              protocol checking */
    int major;
    int minor;
} PlatformEntry;
 ...
 *
 ***********************************************************************/
static int LibraryEnsureOpen(void)
{
    ...
    if ((ldf == NULL) && makeLDF)
    {
         ...
         */
        for (cp = GH(geodeName), i = GEODE_NAME_SIZE; *cp != ' ' && i > 0;
             i--, cp++)
        {
            if (isupper(*cp))
            {
         ...
         */
        ldf = VMOpen(
            VMO_CREATE_ONLY | FILE_DENY_RW | FILE_ACCESS_RW, 70, file, &status);
        ...
        if (ldf == NULL)
        {
             ...
             */
            Notify(NOTIFY_ERROR,
                "unable to create library definition file %s",
                file);
            return (0);
        }
        ...
        else
        {
            ...
            bcopy(OBJ_SYMTOKEN,
                gfh.core.token.chars,
                sizeof(gfh.core.token.chars));
            bcopy(
                "GLUE", gfh.core.creator.chars, sizeof(gfh.core.creator.chars));
            VMSetHeader(ldf, (char*)&gfh);
        }
         ...
         */
        map =
            VMAlloc(ldf, sizeof(ObjHeader) + sizeof(ObjSegment), OID_MAP_BLOCK);
         ...
         */
        s = (ObjSegment*)(hdr + 1);
        s->name = ST_Enter(ldf, ldfStrings, GH(geodeName), GEODE_NAME_SIZE - i);
        s->class = NullID;
        s->align = 0;
        s->type = SEG_LIBRARY;
 ...
 *
 ***********************************************************************/
static void LibraryDupType(word type, /* Description to duplicate */
    ...
    else
    {
        ...
        *dest = ldfTSize;
        ...
        if (OTYPE_IS_STRUCT(src.words[0]))
        {
             ...
             */
            ID id = ST_Dup(symbols, OTYPE_STRUCT_ID(&src), ldf, ldfStrings);
 ...
 *
 ***********************************************************************/
void Library_ExportAs(char* name, char* alias, Boolean mustBeDefined)
{
    char message[132];
    EntryPt* ep;
    SegDesc* sd;
    ...
    {
        entryPoints = (EntryPt*)realloc(
            (void*)entryPoints, (numEPs + 1) * sizeof(EntryPt));
    }
    LibraryEnsureOpen();
    ...
    if (ep->name == NullID ||
        !Sym_FindWithSegment(
            symbols, ep->name, &ep->block, &ep->offset, TRUE, &sd))
    {
        if (mustBeDefined)
        {
    ...
    else
    {
        ...
        if (makeLDF)
        {
             ...
             */
            n = osh->num -
                ((ep->offset - sizeof(ObjSymHeader)) / sizeof(ObjSym));
            ...
            switch (sym->type)
            {
                ...
                case OSYM_VARIANT_CLASS:
                {
                    ...
                    for (bsym = sym + 1, need--; need > 0; need--, bsym++)
                    {
                         ...
                         */
                        lsym->name =
                            ST_Dup(symbols, bsym->name, ldf, ldfStrings);
                        /*
                ...
                case OSYM_CHUNK:
                     ...
                     */
                    LibraryDupType(
                        sym->u.variable.type, types, &lsym->u.variable.type);
                    break;
            }
 ...
 *
 ***********************************************************************/
ID Library_TackPrependPublishedToID(
    VMHandle vmHandle, VMBlockHandle table, ID id)
{
    char name[300];
    ID returnID;
 ...
 *
 ***********************************************************************/
ID Library_ForceTackPrependPublishedToID(
    VMHandle vmHandle, VMBlockHandle table, ID id)
{
    char name[300];
    ID returnID;
 ...
 *
 ***********************************************************************/
void Library_MarkPublished(char* name)
{
    ID id;
    char message[300];
    ...
    else
    {
        ...
        seg->syms =
            VMAlloc(ldf, sizeof(ObjSymHeader) + sizeof(ObjSym), OID_SYM_BLOCK);
 ...
 *
 ***********************************************************************/
void LDFFind(ID id, VMBlockHandle* symBlock, word* symOff)
{
    ...
    for (*symBlock = ldfBlock; *symBlock != 0; *symBlock = next)
    {
         ...
         */
        for (*symOff = sizeof(ObjSymHeader); n > 0;
             *symOff += sizeof(ObjSym), n--)
        {
            sym = (ObjSym*)((genptr)osh + *symOff);
            if (sym->name == id)
            {
 ...
 *
 ***********************************************************************/
void Library_Publish(VMHandle fh, /* File handle of passed blocks */
    ...
    for (cur = relBlock; cur != 0; cur = next)
    {
        ...
        for (sourceRel = (ObjRel*)(sourceRelHead + 1); n > 0; n--, sourceRel++)
        {
                ...
                (sourceRel->offset < procOffset + bytes))
            {
                ...
                sourceFrame =
                    (ObjSegment*)((genptr)VMLock(
                                      fh, frameMap, (MemHandle*)NULL) +
                                  sourceRel->frame);

                if (sourceRel->symBlock != 0)
                {
                    targetSym = (ObjSym*)((genptr)VMLock(fh,
                                              sourceRel->symBlock,
                                              (MemHandle*)NULL) +
                                          sourceRel->symOff);
                ...
                if (sourceFrame->type == SEG_LIBRARY)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        frameSeg->size = 0;
                        frameSeg->flags = 0;
                        frameSeg->toc = frameSeg->lines = frameSeg->addrMap =
                            frameSeg->data = frameSeg->relHead = 0;
                        frameSeg->syms =
                            VMAlloc(ldf, sizeof(ObjSymHeader), OID_SYM_BLOCK);
                ...
                else
                {
                    ...
                    if (sourceRel->symBlock != 0)
                    {
                        ...
                        LDFFind(
                            targetID, &(destRel->symBlock), &(destRel->symOff));
                        /*
 ...
 *
 ***********************************************************************/
void Library_ProtoMinor(char* name)
{
    ObjSym* lsym;  /* Symbol in ldf file */
    MemHandle mem; /* General memory handle for resize and
                    * determining number of symbols left */
...
void Library_Skip(int n)
{
    ...
    {
        entryPoints = (EntryPt*)realloc(
            (void*)entryPoints, (numEPs + n) * sizeof(EntryPt));
 ...
 *
 ***********************************************************************/
void Library_SkipUntilNumber(int n)
{
    int i;
    char message[100];
    ...
    {
        entryPoints =
            (EntryPt*)realloc((void*)entryPoints, n * sizeof(EntryPt));
        for (i = numEPs; i < n; i++)
        {
 ...
 *
 ***********************************************************************/
void Library_SkipUntilConstant(char* name)
{
    char message[100];
    ID id;
    VMBlockHandle symBlock;
 ...
 *
 *********************************************************************/
static int LibraryNameStrCmp(char* name1, char* name2)
{
 ...
 *
 *********************************************************************/
static int LibraryHasBeenLinked(char* name) /* name of library to check for */
{
    int i;
 ...
 *
 ***********************************************************************/
LibraryLinkValues Library_Link(char* name, /* File name */
 ...
 *
 ***********************************************************************/
void Library_LoadPublished(void)
{
    char file[1024];       /* Path for locating a library */
 ...
 *
 ***********************************************************************/
void Library_CheckForMissingLibraries(void)
{
    char message[1000];
 ...
 *
 ***********************************************************************/
int Library_Find(ID name, word* entryNum)
{
    int i;
 ...
 *
 ***********************************************************************/
int Library_UseEntry(
    SegDesc* libSeg, const ObjSym* os, int doInc, int errorIfPlatformViolated)
{
    ObjSymHeader* osh;
    const ObjSym* os2;
    ...
    for (os2 = os - 1; os2 >= ObjFirstEntry(osh, const ObjSym); os2--)
    {
        if (os2->type == OSYM_NEWMINOR)
        {
                ...
                os2->u.newMinor.number)
            {
                 ...
                 * restrictions
                 */
                i = LibraryLookupPlatformLibrary(
                    libs[libSeg->pdata.library].entry.name);

                if (i != numPlatformLibs)
                {
                    if (!(platformLibs[i].flags & PE_EXEMPT) &&
                        (os2->u.newMinor.number > platformLibs[i].minor))
                    {
 ...
 *
 ***********************************************************************/
void Library_ReadPlatformFile(char* name) /* File name */
{
    ...
    while (fscanf(fh, "%s%d%d", libName, &majorProto, &minorProto) != EOF)
    {
        ...
        else
        {
            ...
            {
                platformLibs = (PlatformEntry*)realloc((void*)platformLibs,
                    (numPlatformLibs + 1) * sizeof(PlatformEntry));
            }
 ...
 *
 ***********************************************************************/
void Library_ReadShipFile(char* name) /* File name */
{
    char file[1024];  /* Path for locating a .plt file */
    FILE* fh;         /* File open to .plt file */
    int i;            /* Generic index */
 ...
 *
 ***********************************************************************/
void Library_ExemptLibrary(char* name) /* Library name */
{
    int i; /* Generic index */
