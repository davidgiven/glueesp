...
FileOps geoOps = {
    ...
    (SetEntryProc*)0,
    FILE_NEEDPARAM | FILE_USES_GEODE_PARAMS | FILE_AUTO_LINK_LIBS |
        FILE_PROTO_RELS,
    "geo",   /* File suffix */
...
#define TPD_SIZE                                      \
    64 /* Size of the thread-private data required at \
 ...
 *
 ***********************************************************************/
void Geo_DecodeRP(char* num, /* Number to decode */
    ...
    for (numNums = maxNums; numNums > 0; numNums--)
    {
        ...
        else
        {
            ...
            *endp = '\0';
            ...
            {
                Notify(NOTIFY_ERROR,
                    "Release/protocol constant \"%s\" not defined -- perhaps "
                    "defined with '=' instead of 'equ'?",
                    cp);
            }
            *endp = savec;
            cp = endp;
 ...
 *
 ***********************************************************************/
int GeoCompareResources(SegDesc** r1, SegDesc** r2)
{
    int f1, f2;
 ...
 *
 ***********************************************************************/
static int GeoPrepare(char* outfile, char* paramfile, char* mapfile)
{
    ...
    else
    {
        ...
        geoHeader.v1x.execHeader.geosFileHeader.core.type =
            GFT_EXECUTABLE - GFT_RELEASE_1_OFFSET;
        /*
         * Fill in creator for all geodes as 'GEOS'
         */
        ...
        geoHeader.v1x.execHeader.geosFileHeader.core.creator.manufID =
            0; /* BSW */
    ...
    {
        Notify(NOTIFY_ERROR,
            "dgroup has no segments in it -- cannot create geode");
        return (0);
    }
    ...
    for (resid = 0, i = 0; i < seg_NumSegs; i++)
    {
            ...
            (seg_Segments[i]->combine != SEG_LIBRARY))
        {
            ...
            else if (seg_Segments[i]->combine == SEG_LMEM)
            {
                ...
                {
                    seg_Segments[i]->flags = RESF_SWAPABLE | RESF_DISCARDED |
                                             RESF_LMEM | RESF_OBJECT;
                }
                else
                {
            ...
            {
                seg_Segments[i]->flags = (RESF_STANDARD & ~RESF_DISCARDABLE) |
                                         RESF_SHARED | RESF_CODE;
            }
            else
            {
                seg_Segments[i]->flags =
    ...
    if (geosRelease >= 2)
    {
        geoHeader.v2x.resCount = geoHeader.v2x.execHeader.resourceCount =
            resCount;
    }
    else
    {
        geoHeader.v1x.resCount = geoHeader.v1x.execHeader.resourceCount =
            resCount;
    }
    ...
    {
        Notify(NOTIFY_ERROR,
            "You must provide a token with the tokenChars directive in the .gp "
            "file");
        return (0);
    }
...
#define FU_APPOBJ 0x0008
        ...
        if (!noSort)
        {
            qsort(&seg_Segments[2],
                seg_NumSegs - 2,
                sizeof(seg_Segments[0]),
        ...
        for (resid = 0, i = 0; i < seg_NumSegs; i++)
        {
                ...
                (seg_Segments[i]->combine != SEG_LIBRARY))
            {
                ...
                if (seg_Segments[i]->pdata.resid != resid)
                {
                    ...
                    if (!(fixedUp & FU_LIBENT) && (GH(libEntryResource) == old))
                    {
    ...
    for (i = 1; i < seg_NumSegs; i++)
    {
            ...
            (seg_Segments[i]->combine != SEG_LIBRARY))
        {
             ...
             */
            if ((GH(execHeader.appObjResource) ==
                    seg_Segments[i]->pdata.resid) &&
    ...
    if (GH(geodeAttr) & GA_DRIVER)
    {
        ...
        if (discardableDgroup)
        {
            if (GH(driverTabResource) == 1)
            {
                Notify(NOTIFY_ERROR, "DriverTable is in dgroup\n");
                Notify(
                    NOTIFY_ERROR, "geode cannot have a discardable dgroup\n");
            }
        }
    }
    ...
    cbase = (geosRelease >= 2 ? sizeof(geoHeader.v2x) : sizeof(geoHeader.v1x)) +
            ...
            (numEPs * 2 * sizeof(word)) + (resCount * 10);
        ...
        if (localizationWanted)
        {
            if (ldfOutputDir)
            {
                ...
                localizationFileName =
                    malloc((strlen(ldfOutputDir) + 1 + strlen(LOC_PATH) + 1));
                for (cp2 = localizationFileName, cp = ldfOutputDir; *cp != 0;
                     cp++)
                {
                    *cp2++ = *cp;
                }
 ...
 *
 ***********************************************************************/
static int GeoReloc(int type, /* Relocation type (from obj file) */
                               ...
                               * PC byte-order */
{
    ...
    if (frame->combine == SEG_LIBRARY)
    {
        ...
        {
            Pass2_RelocError(
                targ, off, "library %i not linked in", frame->name);
            return (0);
        }
    ...
    switch (type)
    {
        case OREL_RESID:
        {
            ...
            if (frame->combine == SEG_LIBRARY)
            {
                ...
                {
                    Pass2_RelocError(
                        targ, off, "Cannot apply RESID to the kernel");
                    return (0);
                }
                else if (lib->lnum == NO_LOAD)
                {
        ...
        case OREL_SEGMENT:
        {
            ...
            else
            {
                ...
                if (frame->combine != SEG_LIBRARY)
                {
                     ...
                     */
                    if ((frame->combine != SEG_ABSOLUTE) &&
                        (type == OREL_SEGMENT) && (frame->pdata.resid == 1) &&
                        discardableDgroup)
                    {
                        Pass2_RelocError(targ,
                            off,
                        ...
                        (GH(geodeAttr) & GA_MULTI_LAUNCHABLE))
                    {
                        ...
                        else
                        {
                            Pass2_RelocError(targ,
                                ...
                                frame->name);
                            Pass2_RelocError(
                                targ, off, "geode cannot be multi-launchable");
#if 0
    /* Leave "multi-launchable" so all cases can be flagged */
        ...
        case OREL_CALL:
            ...
            else
            {
                 ...
                 */
                if ((frame->combine != SEG_ABSOLUTE) &&
                    (frame->pdata.resid == 1) && discardableDgroup)
                {
                    Pass2_RelocError(
                        targ, off, "call relocation to dgroup encountered");
                    Pass2_RelocError(
                        targ, off, "geode cannot have a discardable dgroup");
                }
                    ...
                    (GH(geodeAttr) & GA_MULTI_LAUNCHABLE))
                {
                    Pass2_RelocError(targ,
                        off,
                        "call relocation from shared/read-only to "
                        "unshared/writable segment %i",
                        frame->name);
                    Pass2_RelocError(
                        targ, off, "geode cannot be multi-launchable");
 ...
 ***********************************************************************/
#if defined(DOSWAP)
static inline void GeoSwapArea(word* s, /* Start of area to swap */
 ...
 *
 ***********************************************************************/
static void GeoWrite(void* base, /* Base of file buffer */
    ...
    char* outfile)               /* Name of output file */
{
    ...
    if (GH(geodeAttr) & GA_PROCESS)
    {
         ...
         */
        word sb =
            seg_Segments[1]->size + (GH(execHeader.udataSize) - stackSize);
    ...
    if (geosRelease >= 2)
    {
        ...
        GeoSwapArea(
            (word*)geoHeader.v2x.geodeName, sizeof(geoHeader.v2x.geodeName));
        ...
        GeoSwapArea(
            (word*)geoHeader.v2x.execHeader.geosFileHeader.creator.chars,
    ...
    {
        GeoSwapArea(
            (word*)geoHeader.v1x.execHeader.geosFileHeader.core.signature,
            sizeof(geoHeader.v1x.execHeader.geosFileHeader.core.signature));
        GeoSwapArea(
            (word*)geoHeader.v1x.geodeName, sizeof(geoHeader.v1x.geodeName));
        ...
        GeoSwapArea(
            (word*)geoHeader.v1x.execHeader.geosFileHeader.core.token.chars,
            sizeof(geoHeader.v1x.execHeader.geosFileHeader.core.token.chars));
        GeoSwapArea(
            (word*)geoHeader.v1x.execHeader.geosFileHeader.core.creator.chars,
            sizeof(geoHeader.v1x.execHeader.geosFileHeader.core.creator.chars));
        GeoSwapArea(
            (word*)geoHeader.v1x.execHeader.geosFileHeader.core.longName,
    ...
    if (numImport != 0)
    {
        ltable = (ImportedLibraryEntry*)malloc(
            numImport * sizeof(ImportedLibraryEntry));
        ...
        for (j = i = 0; i < numLibs; i++)
        {
            if (libs[i].lnum >= 0)
            {
                 ...
                 */
                GeoSwapArea(
                    (word*)&ltable[j].protocol, sizeof(ltable[j].protocol));
                j++;
            }
        }

        Out_Block(
            curOff, (void*)ltable, numImport * sizeof(ImportedLibraryEntry));
    ...
    if (numEPs)
    {
        ...
        for (i = 0; i < numEPs; i++)
        {
            ...
            else
            {
                ...
                {
                    Notify(NOTIFY_ERROR, "entry point #%d is in dgroup\n", i);
                    Notify(
                        NOTIFY_ERROR, "geode cannot have a discardable dgroup");
                }
 ...
 *
 ***********************************************************************/
static int GeoCheckFixed(SegDesc* targ)
{
