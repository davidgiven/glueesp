 ...
 *
 ***********************************************************************/
void Out_Init(long size) /* Size of output file */
{
 ...
 *
 ***********************************************************************/
void Out_Block(long position, void* data, int len)
{
 ...
 *
 ***********************************************************************/
int Out_Fetch(long position, void* data, int len)
{
 ...
 *
 ***********************************************************************/
void Out_Final(char* outfile) /* Name of output file */
{
    int i;
 ...
 *
 ***********************************************************************/
static int OutCmpSym(AddrSym* sym1, AddrSym* sym2)
{
    char *name1, *name2;
    int res;
 ...
 *
 ***********************************************************************/
static void OutPrintByName(FILE* mf) /* Stream open to map file */
{
    int i;
    int j;
    AddrSym* syms;
 ...
 *
 ***********************************************************************/
static void OutPrintByAddr(FILE* mf) /* Stream open to map file */
{
    int i;
 ...
 *
 ***********************************************************************/
void Out_DosMap(char* mapfile, long imgBase)
{
 ...
 *
 ***********************************************************************/
void Out_ExtraReloc(SegDesc* sd, /* Segment needing adjustment */
 ...
 *
 ***********************************************************************/
void Out_FinishLineBlock(SegDesc* sd, VMBlockHandle block)
{
    ObjLineHeader* olh;
    ObjAddrMapHeader* oamh;
    ObjAddrMapEntry* oame;
    ...
    while (n > 0)
    {
        ...
        if (allocNew)
        {
            next = VMAlloc(symbols,
                (olh->num * sizeof(ObjLine) + sizeof(ObjLineHeader)),
                OID_LINE_BLOCK);
...
void Out_PromoteGroups(void)
{
    ...
    if (seg_NumSubSegs != 0)
    {
         ...
         */
        newSegs = (SegDesc**)malloc(
            (seg_NumSegs + seg_NumGroups - seg_NumSubSegs) * sizeof(SegDesc*));
        ...
        for (i = 0; i < seg_NumGroups; i++, j++)
        {
            ...
            *sd = *gd->segs[0];
            ...
            table = (ObjHashHeader*)VMLock(symbols, sd->syms, (MemHandle*)NULL);
            ...
            for (s = 1; s < gd->numSegs; s++)
            {
                ...
                if (ns->symH)
                {
                    ...
                    {
                        osh = (ObjSymHeader*)VMLock(
                            symbols, sd->symT, (MemHandle*)NULL);
                        osh->next = ns->symH;
                 ...
                 */
                ohh =
                    (ObjHashHeader*)VMLock(symbols, ns->syms, (MemHandle*)NULL);
                ...
                for (chain = 0; chain < obj_hash_chains; chain++)
                {
                    if (ohh->chains[chain])
                    {
                        if (table->chains[chain])
                        {
                            ...
                            hb = (ObjHashBlock*)VMLock(
                                symbols, table->chains[chain], &mem);
 ...
 *
 ***********************************************************************/
int Out_FindConstSym(char* name, /* Symbol to find */
 ...
 *
 ***********************************************************************/
void Out_RegisterFinalProtoRel(Boolean isMajor, long position, SegDesc* library)
{
    OutProtoRel* rel;
    ...
    if (numProtoRels > 1)
    {
        protoRels = (OutProtoRel*)realloc(
            (malloc_t)protoRels, numProtoRels * sizeof(OutProtoRel));
    }
    else
    {
