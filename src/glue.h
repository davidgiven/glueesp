...
#define COPYRIGHT_SIZE                                                     \
    32                   /* Maximum length of a copyright notice (dictated \
...
/* segment.c */
extern SegDesc* Seg_AddSegment(
    const char* file, ID name, ID class, int type, int align, int flags);
extern GroupDesc* Seg_AddGroup(const char* file, ID name);
extern void Seg_EnterGroupMember(const char* file, GroupDesc* gd, SegDesc* sd);
...
extern void AddSrcMapEntry(ID fileName, SegDesc* sd, int start, int end);
...
#define pass2_assert(expr, sd, off)                                     \
    ...
    {                                                                   \
        Pass2_RelocError((sd),                                          \
            (off),                                                      \
            "Failed assertion " #expr " at line %d of `" __FILE__ "'.", \
            __LINE__);                                                  \
        abort();                                                        \
    }
#endif /* _GLUE_H_ */
