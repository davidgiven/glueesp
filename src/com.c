 ...
 *
 ***********************************************************************/
static int ComPrepare(char* outfile, char* paramfile, char* mapfile)
{
    int cbase;
    int i;
    SegDesc* sd;
    ...
    if (seg_NumSegs != 2)
    {
        ...
        {
            Notify(NOTIFY_ERROR,
                "cannot create .com file: all code and data must be in a "
                "single segment or group");
            return (0);
        }
    }
 ...
 *
 ***********************************************************************/
static int ComReloc(int type, /* Relocation type (from obj file) */
 ...
 *
 ***********************************************************************/
static void ComWrite(void* base, /* Base of file buffer */
    ...
    char* outfile)               /* Name of output file */
{
    ...
    {
        Notify(
            NOTIFY_ERROR, "cannot create .com file: no entry point specified");
        return;
    }
    ...
    for (lp = (long*)base, i = 256 / 4; i > 0; i--)
    {
        ...
        {
            Notify(NOTIFY_ERROR,
                "cannot create .com file: first 256 bytes contain non-zero "
                "data (offset %04x)",
 ...
 *
 ***********************************************************************/
static void ComSetEntry(SegDesc* sd, word off)
{
    if (off + sd->foff != 256)
    {
        Notify(NOTIFY_ERROR,
 ...
 *
 ***********************************************************************/
static int ComCheckFixed(SegDesc* targ)
{
