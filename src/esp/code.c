 ...
 */
#define OVERRIDE(reg) \
    (((reg) < REG_FS) ? (0x26 | ((reg) << MR_REG_SHIFT)) : (0x60 + (reg)))
...
#define ASSERT_186() \
    ASSERT(80186, 80186 - instruction not supported by current processor)
#define ASSERT_286() \
    ASSERT(80286, 80286 - instruction not supported by current processor)
#define ASSERT_386() \
    ASSERT(80386, 80386 - instruction not supported by current processor)

#define USE_386() (procType & (PROC_80386 | PROC_80486))

#define RES_IS_CONST(type) \
    (((type) == EXPR_TYPE_CONST) || ((type) == EXPR_TYPE_STRING))

#define EXPR_IS_LABEL(res)                                                    \
    (((res)->type != EXPR_TYPE_CONST) && ((res)->type != EXPR_TYPE_STRING) && \
        ((res)->type != EXPR_TYPE_FLOATSTACK) && ((res)->type != NULL) &&     \
 ...
 */
#define OPSIZE_NEEDS_PRE_OPERAND(opSize)                            \
    (((opSize) > 1) && (((opSize) == 4) ? DWORD_OPER_NEEDS_PREFIX() \
                                        : WORD_OPER_NEEDS_PREFIX()))
/*
...
#define EXPR_NEEDS_PRE_ADDRESS(resP)                      \
    (((resP)->data.ea.dword_ea) ? DWORD_EA_NEEDS_PREFIX() \
                                : WORD_EA_NEEDS_PREFIX())
/*
...
#define START_CODEGEN(pass, dot, mdretval, errretval)                          \
    ...
    if (warn_unreach && ((pass) == 1) && curSeg->u.segment.data->checkLabel && \
    ...
    curSeg->u.segment.data->blockStart = curSeg->u.segment.data->checkLabel =  \
        FALSE
 ...
 *
 ***********************************************************************/
void CreateCheckSegmentArg(Expr* expr, ExprResult* result, Arg* arg, byte stat)
{
    char* seg;
    byte ovr, modrm;
        ...
        switch (ovr)
        {
            ...
            case EXPR_DEF_OVERRIDE:
                ...
                switch (modrm & MR_RM_MASK)
                {
                    ...
                    case MR_BP: /* [bp+disp], same as MR_DIRECT */
                         ...
                         * just a dword displacement, in which case it will be
                         * ds.
                         */
                        if ((modrm & MR_DISP_MASK) == 0)
                        {
                            /*
                             * dword displacement, use ds as default
                             * seg-register
                             */
                            seg = "ds";
                        }
            ...
            default:
                ...
                Notify(NOTIFY_ERROR,
                    expr->file,
                    expr->line,
                    "Unexpected segment override value: %02xh",
                    (int)ovr);
                exit(1);
                /*NOTREACHED*/
        }
 ...
 *
 ***********************************************************************/
void CreateCheckFlagArg(ExprResult* result, Arg* arg, byte stat)
{
    byte ovr, modrm;
    char* flag;
...
static char ICM_yyinput(void)
{
    return (0); /* end of something... */
}

void InvokeCheckMacro(
    SymbolPtr macroSym, Expr* expr, ExprResult* res, byte stat)
{
    WrapProc* oldwrap;
    ID id;
    Arg* args;
...
static FixResult CodeInitial(int* addrPtr, /* Start of instruction area */
 ...
 *
 ***********************************************************************/
FixResult FixupReadWriteCheck(int* addrPtr, /* IN/OUT: Address of inst start */
    ...
    Opaque data)                            /* OpCode/other data */
{
     ...
     */
    if (Expr_Status(&res1) &
        (EXPR_STAT_CODE | EXPR_STAT_REGISTER | EXPR_STAT_CONST))
    {
 ...
 *				CodeCheckExpression
 ***********************************************************************
 * SYNOPSIS:	Verify that an expression we are reading-from/writing-to is
 *valid CALLED BY:	CodeInitial RETURN:	nothing SIDE EFFECTS: may invoke
 *_read/writecheck macro, and that macro may generate code
 *
 * STRATEGY:
 *
 * REVISION HISTORY:
...
static void CodeCheckExpression(Expr* checkExpr, /* expression to check */
    ...
    byte stat)                                   /* status of the expr */
{
     ...
     */
    if ((proc == Code_Jcc) || (proc == Code_JccWarn) ||
        (proc == Code_JccNoWarn) || (proc == Code_Jmp) || (proc == Code_Loop) ||
        (proc == Code_NoArg) || (proc == Code_Ret) ||
        (proc == Code_CallStatic) || (proc == Code_Call) ||
        (proc == Code_String) || (proc == Code_Lea) ||
    ...
    if (((checkRes->data.ea.modrm & MR_DISP_MASK) != MR_REG) &&
        (!RES_IS_CONST(checkRes->type)) && (!EXPR_IS_SEGREG(checkRes)))
    {
        ...
        if (macroSym != NULL)
        {
             ...
             * reads and writes.
             */
            int oreadCheck, owriteCheck, odo_bblock;
 ...
 *
 ***********************************************************************/
static void CodeCleanup(
    Expr* expr1, ExprResult* res1, Expr* expr2, ExprResult* res2)
{
    /*
     ...
     */
    if (expr1 && res1->type == EXPR_TYPE_STRING &&
        (res1->data.str >= expr1->elts[expr1->numElts].str ||
            res1->data.str < expr1->elts[0].str) &&
        (expr2 == NULL || res1->data.str >= expr2->elts[expr2->numElts].str ||
     ...
     */
    if (expr2 && res2->type == EXPR_TYPE_STRING &&
        (res2->data.str >= expr2->elts[expr2->numElts].str ||
            res2->data.str < expr2->elts[0].str) &&
        (expr1 == NULL || res2->data.str >= expr1->elts[expr1->numElts].str ||
                       ...
                       * should be delayed */
{
    ...
    if (expr1)
    {
        if (!Expr_Eval(expr1, res1, flags, &stat1))
        {
            ...
            {
                Notify(
                    NOTIFY_ERROR, expr2->file, expr2->line, (char*)res2->type);
            }
            return (FR_ERROR);
        }
    }
    ...
    if (pass == 1 && ((readCheck == TRUE) || (writeCheck == TRUE)))
    {
        ...
        if (readCheck == TRUE)
        {
            ...
            CodeCheckExpression(
                readArg, readRes, CHECK_VALID_READ, addrPtr, proc, stat);
        }
        ...
        if (writeCheck == TRUE)
        {
            ...
            {
                CodeCheckExpression(
                    e1, res1, CHECK_VALID_WRITE, addrPtr, proc, stat1);
            }
        }
    ...
    if (!(stat1 & EXPR_STAT_DEFINED) || !(stat2 & EXPR_STAT_DEFINED))
    {
        ...
        *addrPtr += defSize;
        ...
        CodeCleanup((stat1 & EXPR_STAT_DEFINED) ? expr1 : NULL,
            res1,
            (stat2 & EXPR_STAT_DEFINED) ? expr2 : NULL,
            res2);
        return (FR_UNDEF);
    }
 ...
 *
 ***********************************************************************/
static void CodeSimplifyType(TypePtr* typePtr, /* Ptr to type to simplify */
    ...
    int warn)                                  /* Warn if *typePtr null */
{
    ...
    if (*typePtr == NULL)
    {
        ...
        {
            Notify(
                NOTIFY_WARNING, file, line, "defaulting operand size to byte");
        }
        *typePtr = Type_Int(1);
        return;
    }
        ...
        ((*typePtr)->tn_u.tn_struct->type == SYM_STRUCT))
    {
        ...
        {
            Notify(NOTIFY_WARNING, file, line, "assuming word-sized operation");
            *typePtr = Type_Int(2);
        }
    }
}
 ...
 *
 ***********************************************************************/
static int CodeMangleString(
    Expr* expr1, Expr* expr2, ExprResult* res, TypePtr type, ID file, int line)
{
    int size;
    int len;
    char* str;
    ...
    len = strlen(str);
    freeStr =
        (str < expr1->elts[0].str || str >= expr1->elts[expr1->numElts].str) &&
        (expr2 == NULL || str < expr2->elts[0].str ||
 ...
 *
 ***********************************************************************/
static int CodeCompatible(Expr* expr1, /* First operand */
    ...
    if (RES_IS_CONST(res1->type))
    {
        if (isString)
        {
            Notify(NOTIFY_ERROR,
                file,
                line,
                "dual-operand string instructions require both operands to be "
                "in memory");
        ...
        *opSize = size = Type_Size(res1->type);
        ...
        else if (!(procType & (PROC_80386 | PROC_80486)))
        {
            ...
            Notify(NOTIFY_WARNING,
                expr1->file,
                expr1->line,
                "assuming word-sized operation (current processor can't "
                "manipulate something larger than a word)");
            *opSize = size = 2;
        }
    }
    else
 ...
 *
 ***********************************************************************/
static byte* CodeStoreEA(byte* buf, /* Place to start storing */
 ...
 *
 ***********************************************************************/
static byte* CodeNopPad(byte* ip, /* Place to store first NOP */
 ...
 *
 ***********************************************************************/
static void CodeFinal(int* addrPtr, /* Place to store instruction */
                                     ...
                                     * and fixRes are... */
{
    ...
    if (pass != 1)
    {
        ...
        if (diff < 0)
        {
            if (pass == 4)
            {
                 ...
                 */
                CodeNopPad(ibuf + length, -diff, file, line);
                length = prevLength;
            }
            else
            {
                Table_Delete(curSeg->u.segment.code, *addrPtr, -diff);
            }
        }
     ...
     */
    Table_Store(curSeg->u.segment.code, length, (void*)ibuf, *addrPtr);
 ...
 *
 ***********************************************************************/
static FixResult CodeNoConstant(Expr* expr1,
 ...
 *
 ***********************************************************************/
FixResult CodeArithCommon(int* addrPtr, /* Address for instruction.
                                         ...
                                         * registering any fixups) */
{
    ...
    if (USE_386() && notByte &&
        ((isDWord && DWORD_OPER_NEEDS_PREFIX()) ||
 ...
 *
 ***********************************************************************/
FixResult Code_Arith2(int* addrPtr, /* IN/OUT: Address of instruction start */
    ...
    Opaque data)                    /* Opcode/other data */
{
     ...
     * are finished.
     */
    return (CodeArithCommon(addrPtr,
        pass,
        prevSize,
        expr1,
        &res1,
 ...
 *
 ***********************************************************************/
FixResult Code_Arpl(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_Arpl, 2, data, &delay);
     ...
     */
    result = CodeNoConstant(
        expr1, &res1, expr2, &res2, expr1->file, expr1->line, (OpCode*)data);
    if (result != FR_DONE)
    {
        return (result);
    }
 ...
 *
 ***********************************************************************/
FixResult Code_BitNF(
    int* addrPtr, /* IN/OUT: Address of instruction start/end */
    int prevSize, /* # bytes previously allocated to inst */
    int pass,     /* Current pass */
    Expr* expr1,  /* Operand 1 */
    Expr* expr2,  /* Operand 2 */
    Opaque data)  /* Opcode/other data */
{
    ...
    switch (CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_BitNF, 4, data, &delay))
    {
        case FR_UNDEF:
            return (FR_UNDEF);
        case FR_ERROR:
 ...
 *
 ***********************************************************************/
FixResult Code_Bound(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                  /* # bytes previously allocated to inst */
    int pass,                      /* Current pass */
    Expr* expr1,                   /* Operand 1 */
    Expr* expr2,                   /* Operand 2 */
    Opaque data)                   /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_Bound, 5, data, &delay);
 ...
 *
 ***********************************************************************/
FixResult Code_Call(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Call, 3, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    result = CodeNoConstant(
        expr1, &res1, NULL, NULL, expr1->file, expr1->line, (OpCode*)data);
    if (result != FR_DONE)
    {
        return (result);
    }
        ...
        switch (size)
        {
            ...
            default:
                Notify(NOTIFY_ERROR,
                    expr1->file,
                    expr1->line,
                    "illegal size (%d) for indirect call vector",
                    size);
                CodeCleanup(expr1, &res1, NULL, NULL);
                return (FR_ERROR);
        }
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Call, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_CallStatic(
    int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize, /* # bytes previously allocated to inst */
    int pass,     /* Current pass */
    Expr* expr1,  /* Operand 1 */
    Expr* expr2,  /* Operand 2 */
 ...
 *
 ***********************************************************************/
FixResult Code_NoArgIO(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                    /* # bytes previously allocated to inst */
    int pass,                        /* Current pass */
    Expr* expr1,                     /* Operand 1 */
    Expr* expr2,                     /* Operand 2 */
 ...
 *
 ***********************************************************************/
FixResult Code_DPShift(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                    /* # bytes previously allocated to inst */
    int pass,                        /* Current pass */
    Expr* expr1,                     /* Operand 1 */
    Expr* expr2,                     /* Operand 2 */
    ...
    FixProc* proc)                   /* Calling procedure */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, proc, 2, data, &delay);
    if (result != FR_DONE)
        return (result);
     ...
     */
    if (!CodeCompatible(
            expr1, &res1, expr2, &res2, FALSE, FALSE, delay, &opSize))
    {
        return (FR_ERROR);
    }
    isDWord = (opSize == 4) ? 1 : 0;
    ...
    {
        Notify(
            NOTIFY_ERROR, expr1->file, expr1->line, "can't shift a constant");
        CodeCleanup(expr1, &res1, expr2, &res2);
        return (FR_ERROR);
    }
    ...
    *ip++ = opcode;
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, proc, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
...
{
    return Code_DPShift(
        addrPtr, prevSize, pass, expr1, expr2, data, 0xa4, Code_DPShiftLeft);
}

FixResult Code_DPShiftRight(
    int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize, /* # bytes previously allocated to inst */
    int pass,     /* Current pass */
    Expr* expr1,  /* Operand 1 */
...
{
    return Code_DPShift(
        addrPtr, prevSize, pass, expr1, expr2, data, 0xac, Code_DPShiftRight);
}
...
#define ADD_FWAIT_FOR_8087(fixAddr, ip) \
    if ((procType)&PROC_8087)           \
    {                                   \
        *(ip)++ = 0x9b;                 \
        fixAddr += 1;                   \
    }
 ...
 *
 *********************************************************************/
FixResult Code_Fbiop(int* addrPtr, /* address of instruction start  */
    ...
    Opaque data)                   /* OpCode/other data */
{
            ...
            if (res1.type != EXPR_TYPE_FLOATSTACK)
            {
                ...
                if (res1.type == 0)
                {
                    ...
                    Notify(NOTIFY_WARNING,
                        expr1->file,
                        expr1->line,
                        "%s : using default operand size, 4 bytes",
                        op->name);
                }
                else
                {
            ...
            else /* 1 stack operand */
            {
                ...
                if ((unsigned long)res1.data.number > MAX_FLOATSTACK_ELEMENT)
                {
                    Notify(NOTIFY_ERROR,
                        expr1->file,
                        expr1->line,
                        "%s : register must be from 0 to 7",
                        op->name);
                    CodeCleanup(expr1, &res1, NULL, NULL);
                    return (FR_ERROR);
                }
        ...
        else /* we have two operands */
        {
                ...
                ((unsigned long)res2.data.number != 0))
            {
                Notify(NOTIFY_ERROR,
                    expr1->file,
                    expr1->line,
                    "%s : one register must be top of stack",
                    op->name);
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Fbiop, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
    CodeCleanup(expr1, &res1, expr2, &res2);
 ...
 *
 *********************************************************************/
FixResult Code_Fcom(int* addrPtr, /* address of instruction start */
    ...
    Opaque data)                  /* OpCode/ other data */
{
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Fcom, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 *********************************************************************/
FixResult Code_Ffree(int* addrPtr, /* address of instruction start */
    int prevSize,                  /* # bytes previously allocated */
    int pass,                      /* cuurent pass */
    Expr* expr1,                   /* Operand 1 */
    Expr* expr2,                   /* Operand 2 */
    Opaque data)                   /* OpCode/ other data */
{
    ...
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Ffree, 5, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
 ...
 *
 *********************************************************************/
FixResult Code_Fgroup0(int* addrPtr, /* address of instruction start */
    int prevSize,                    /* # bytes previously allocated */
    int pass,                        /* cuurent pass */
    Expr* expr1,                     /* Operand 1 */
    Expr* expr2,                     /* Operand 2 */
    Opaque data)                     /* OpCode/ other data */
{
    ...
    {
        yyerror("%s is only available for the 80287 or later", op->name);
        return (FR_ERROR);
    }
 ...
 *
 *********************************************************************/
FixResult Code_Fgroup1(int* addrPtr, /* address of instruction start */
    int prevSize,                    /* # bytes previously allocated */
    int pass,                        /* cuurent pass */
    Expr* expr1,                     /* Operand 1 */
    Expr* expr2,                     /* Operand 2 */
    Opaque data)                     /* OpCode/ other data */
{
    ...
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Fgroup1, 5, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    result =
        CodeNoConstant(expr1, &res1, NULL, NULL, expr1->file, expr1->line, op);
    if (result != FR_DONE)
    {
        return result;
    }
 ...
 *
 *********************************************************************/
FixResult Code_Fint(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Fint, 3, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    if (res1.type == EXPR_TYPE_FLOATSTACK)
    {
        Notify(NOTIFY_ERROR,
            expr1->file,
            expr1->line,
            "%s : operand must be memory location",
            op->name);
        CodeCleanup(expr1, &res1, NULL, NULL);
        return FR_ERROR;
    }
    ...
    if (res1.type == 0)
    {
        ...
        Notify(NOTIFY_WARNING,
            expr1->file,
            expr1->line,
            "%s : using default operand size, 2 bytes",
            op->name);
    }
    else
    {
    ...
    **********************************************************/
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Fint, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 *********************************************************************/
FixResult Code_Fldst(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                  /* # bytes previously allocated to inst */
    int pass,                      /* Current pass */
    Expr* expr1,                   /* Operand 1 */
    Expr* expr2,                   /* Operand 2 */
    Opaque data)                   /* OpCode/other data */
{
    ...
    /* make sure operand is not a constant */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Fldst, 3, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Fldst, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 *********************************************************************/
FixResult Code_Fxch(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    ...
    else
    {
        ...
        if (res1.type != EXPR_TYPE_FLOATSTACK)
        {
            Notify(NOTIFY_ERROR,
                expr1->file,
                expr1->line,
                "%s can't operate on a segment register",
                op->name);
            CodeCleanup(expr1, &res1, NULL, NULL);
            return (FR_ERROR);
        }
 ...
 *
 *********************************************************************/
FixResult Code_Fzop(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
 ...
 *
 ***********************************************************************/
FixResult Code_Group1(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                   /* # bytes previously allocated to inst */
    int pass,                       /* Current pass */
    Expr* expr1,                    /* Operand 1 */
    Expr* expr2,                    /* Operand 2 */
    Opaque data)                    /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Group1, 3, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
        ...
        (op->value == 0x10f6))   /* NOT */
    {
         ...
         */
        result = CodeNoConstant(
            expr1, &res1, NULL, NULL, expr1->file, expr1->line, op);
    }
    else
    {
        /*
         ...
         */
        result = CodeNoConstant(
            NULL, NULL, expr1, &res1, expr1->file, expr1->line, op);
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Imul(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
 ...
 *
 ***********************************************************************/
FixResult Code_IO(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,               /* # bytes previously allocated to inst */
    int pass,                   /* Current pass */
    Expr* expr1,                /* Operand 1 */
    Expr* expr2,                /* Operand 2 */
    Opaque data)                /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_IO, 2, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Ins(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    Expr* expr2,                 /* Operand 2 */
    Opaque data)                 /* OpCode/other data */
{
    ...
    Notify(NOTIFY_ERROR, expr1->file, expr1->line, "INS not supported");
 ...
 *
 ***********************************************************************/
FixResult Code_Int(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    Expr* expr2,                 /* Operand 2 */
    Opaque data)                 /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Int, 2, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Jmp(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    Expr* expr2,                 /* Operand 2 */
    Opaque data)                 /* OpCode/other data */
{
        ...
        ((res1.type == Type_Near()) || (res1.type == Type_Far())))
    {
        ...
        else if (dest->segment != curSeg)
        {
            ...
            if (res1.rel.size == FIX_SIZE_BYTE)
            {
                Notify(NOTIFY_WARNING,
                    expr1->file,
                    expr1->line,
                    "can't be reached with short jump -- SHORT override "
                    "ignored");
                res1.rel.size = FIX_SIZE_WORD;
                res1.rel.type = FIX_OFFSET;
            }
        ...
        if (result == FR_OPTIM)
        {
             ...
             */
            disp =
                (dest->u.addrsym.offset + res1.data.ea.disp) - ((*addrPtr) + 2);
            ...
            if (pass == 4)
            {
                 ...
                 * instruction size.
                 */
                if (!(dest->flags & SYM_UNDEF) && (disp >= -128) &&
                    (disp <= 127))
                {
                    /*
                    ...
                    *ip++ = (byte)disp;
                    ...
                    if (prevSize != 2)
                    {
                         ...
                         */
                        ip = CodeNopPad(
                            ip, prevSize - 2, expr1->file, expr1->line);
                    }
                }
                else
                {
                    if (res1.rel.size == FIX_SIZE_BYTE)
                    {
                        ...
                        Notify(NOTIFY_WARNING,
                            expr1->file,
                            expr1->line,
                            "can't be reached with short jump -- SHORT "
                            "override ignored");
                        res1.rel.size = FIX_SIZE_WORD;
                        res1.rel.type = FIX_OFFSET;
                    }
        ...
        switch (size)
        {
            ...
            default:
                Notify(NOTIFY_ERROR,
                    expr1->file,
                    expr1->line,
                    "illegal size (%d) for indirect jump vector",
                    size);
                CodeCleanup(expr1, &res1, NULL, NULL);
                return (FR_ERROR);
        }
    ...
    if (delay && result != FR_OPTIM)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Jmp, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_JccWarn(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                    /* # bytes previously allocated to inst */
    int pass,                        /* Current pass */
    Expr* expr1,                     /* Operand 1 */
 ...
 *
 ***********************************************************************/
FixResult Code_Jcc(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    Expr* expr2,                 /* Operand 2 (NULL) */
    Opaque data)                 /* OpCode/other data */
{
    ...
    if (pass <= 2)
    {
        ...
        if (res1.rel.sym->flags & SYM_UNDEF)
        {
            if (pass == 1)
            {
                 ...
                 */
                Table_Store(curSeg->u.segment.code, 2, (void*)ibuf, *addrPtr);
                Fix_Register(FC_UNDEF,
                    warn_jmp ? Code_JccWarn : Code_JccNoWarn,
                    *addrPtr,
                    2,
    ...
    disp = ((res1.rel.sym->segment == curSeg)
                ? ((res1.rel.sym->flags & SYM_UNDEF)
                          ? -256
                          : ((res1.rel.sym->u.addrsym.offset +
                                 res1.data.ea.disp) -
                                ((*addrPtr) + 2)))
                : 65536);
...
#define JCXZ 0xe3
    ...
    else if ((pass != 4) && (disp > 0) && (prevSize > 2) &&
             ((disp - (prevSize - 2)) <= 127))
    {
        /*
        ...
        if (pass == 4)
        {
            ...
            if (prevSize < ip - ibuf)
            {
                Notify(NOTIFY_ERROR,
                    expr1->file,
                    expr1->line,
                    "can't store transformed out-of-range jump -- optimization "
                    "error");
                CodeCleanup(expr1, &res1, NULL, NULL);
                return (FR_ERROR);
            }
    ...
    if ((res1.rel.sym != NULL) && (res1.rel.sym->type == SYM_PROC) &&
 ...
 *
 ***********************************************************************/
FixResult Code_LSInfo(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                   /* # bytes previously allocated to inst */
    int pass,                       /* Current pass */
    Expr* expr1,                    /* Operand 1 */
    Expr* expr2,                    /* Operand 2 */
    Opaque data)                    /* OpCode/other data */
{
     ...
     */
    result = CodeNoConstant(
        expr1, &res1, expr2, &res2, expr1->file, expr1->line, op);
    if (result != FR_DONE)
    {
        return (result);
    }
        ...
        (Type_Size(res1.type) != 2))
    {
        Notify(NOTIFY_ERROR,
            expr1->file,
            expr1->line,
            "first operand for %s must be a word register",
            op->name);
        result = FR_ERROR;
    }
 ...
 *
 ***********************************************************************/
FixResult Code_LDPtr(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                  /* # bytes previously allocated to inst */
    int pass,                      /* Current pass */
    Expr* expr1,                   /* Operand 1 */
    Expr* expr2,                   /* Operand 2 */
    Opaque data)                   /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_LDPtr, 4, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Lea(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    Expr* expr2,                 /* Operand 2 */
    Opaque data)                 /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_Lea, 4, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Lea, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Lock(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
 ...
 *
 ***********************************************************************/
FixResult Code_Loop(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Loop, 2, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
     ...
     */
    disp =
        (res1.rel.sym->u.addrsym.offset + res1.data.ea.disp) - ((*addrPtr) + 2);
    ...
    if ((pass == 4) && ((disp < -128) || (disp > 127)))
    {
        ...
        if (prevSize < ip - ibuf)
        {
            Notify(NOTIFY_ERROR,
                expr1->file,
                expr1->line,
                "can't store transformed out-of-range loop -- optimization "
                "error");
            CodeCleanup(expr1, &res1, NULL, NULL);
            return (FR_ERROR);
        }
    ...
    else if (pass == 1)
    {
         ...
         */
        Fix_Register(
            FC_OPTIM, Code_Loop, startAddr, ip - ibuf, expr1, NULL, data);
    }
 ...
 *
 ***********************************************************************/
FixResult Code_LSDt(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_LSDt, 2, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    if ((res1.data.ea.modrm & MR_DISP_MASK) == MR_REG)
    {
        Notify(NOTIFY_ERROR,
            expr1->file,
            expr1->line,
            "register illegal as destination of %s",
            op->name);
        CodeCleanup(expr1, &res1, NULL, NULL);
        return (FR_ERROR);
    }
    ...
    *ip++ = 0x01;
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_LSDt, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Move(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
    ...
    switch (CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_Move, 4, data, &delay))
    {
        case FR_UNDEF:
            return (FR_UNDEF);
        case FR_ERROR:
    ...
    if (EXPR_IS_SEGREG(&res1))
    {
        ...
        {
            Notify(NOTIFY_ERROR, expr1->file, expr1->line, "can't move to CS");
            CodeCleanup(expr1, &res1, expr2, &res2);
            return (FR_ERROR);
        }
    ...
    else if (res2.type == EXPR_TYPE_CONST)
    {
        if ((res1.data.ea.modrm & MR_DISP_MASK) == MR_REG)
        {
            ...
            *ip++ =
                0xb0 + (notByte ? 8 : 0) + (res1.data.ea.modrm & MR_RM_MASK);
            *ip++ = (byte)res2.data.number;
            if (notByte)
            {
                *ip++ = (byte)(res2.data.number >> 8);
            ...
            *ip++ = (byte)res2.data.number;
            if (notByte)
            {
             ...
             ((!(res2.data.ea.dword_ea) && res2.data.ea.modrm == MR_DIRECT) ||
                 ((res2.data.ea.dword_ea) &&
                     res2.data.ea.modrm == MR_DIRECT32)))
    {
        /*
        ...
        *ip++ = (byte)(res2.data.ea.disp >> 8);
        if (isDWord)
        {
            ...
            *ip++ = (byte)(res2.data.number >> 24);
                 ...
                 ((res1.data.ea.dword_ea) &&
                     res1.data.ea.modrm == MR_DIRECT32)) &&
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Move, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_NoArg(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                  /* # bytes previously allocated to inst */
    int pass,                      /* Current pass */
    Expr* expr1,                   /* Operand 1 */
    Expr* expr2,                   /* Operand 2 */
 ...
 *
 ***********************************************************************/
FixResult Code_Outs(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
    ...
    Notify(NOTIFY_ERROR, expr1->file, expr1->line, "OUTS not supported");
 ...
 *
 ***********************************************************************/
FixResult Code_Override(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,
    int pass,
    Expr* expr1,
    Expr* expr2,
 ...
 *
 ***********************************************************************/
FixResult Code_Pop(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    Expr* expr2,                 /* Operand 2 */
    Opaque data)                 /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Pop, 4, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    if (EXPR_IS_SEGREG(&res1))
    {
        if (res1.data.ea.override == OVERRIDE(REG_CS))
        {
            Notify(NOTIFY_ERROR, expr1->file, expr1->line, "can't POP CS");
            CodeCleanup(expr1, &res1, NULL, NULL);
            return (FR_ERROR);
        }
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Pop, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Push(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_Push, 4, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    if (RES_IS_CONST(res1.type))
    {
        ...
        {
            (void)CodeMangleString(
                expr1, expr2, &res1, Type_Int(2), expr1->file, expr1->line);
        }
        if (res1.rel.sym)
        {
            /*
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Push, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_PWord(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                  /* # bytes previously allocated to inst */
    int pass,                      /* Current pass */
    Expr* expr1,                   /* Operand 1 */
    Expr* expr2,                   /* Operand 2 */
    Opaque data)                   /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_PWord, 2, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
    ...
    *ip++ = (byte)op->value;
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_PWord, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Rep(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    Expr* expr2,                 /* Operand 2 */
                ...
                switch (ibuf[0])
                {
                    ...
                    default:
                        Notify(NOTIFY_ERROR,
                            expr1->file,
                            expr1->line,
                            "REP may only be used with INS, MOVS, OUTS or "
                            "STOS");
                        return (FR_ERROR);
                }
        }
 ...
 *
 ***********************************************************************/
FixResult Code_Ret(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                /* # bytes previously allocated to inst */
    int pass,                    /* Current pass */
    Expr* expr1,                 /* Operand 1 */
    ...
    if (expr1)
    {
        ...
        if (res1.type != EXPR_TYPE_CONST)
        {
            Notify(NOTIFY_ERROR,
                expr1->file,
                expr1->line,
                "operand of %s must be numeric constant",
                op->name);
            return (FR_ERROR);
        }
        /*
 ...
 *
 ***********************************************************************/
FixResult Code_String(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                   /* # bytes previously allocated to inst */
    int pass,                       /* Current pass */
    Expr* expr1,                    /* Operand 1 */
    Expr* expr2,                    /* Operand 2 */
    Opaque data)                    /* OpCode/other data */
{
    ...
    if (expr1 && expr2)
    {
         ...
         */
        if (!CodeCompatible(
                expr1, &res1, expr2, &res2, FALSE, TRUE, delay, &opSize))
        {
            return (FR_ERROR);
        }
        /*
    ...
    else if (expr2)
    {
         ...
         */
        result = CodeNoConstant(
            expr1, &res2, NULL, NULL, expr2->file, expr2->line, op);
        if (result != FR_DONE)
        {
            return (result);
        }
            ...
            if ((byte)op->value == 0xd7)
            {
                ...
                if (res2.data.ea.modrm != MR_BX)
                {
                    Notify(NOTIFY_WARNING,
                        expr2->file,
                        expr2->line,
                        "%s only reads from DS:[BX] (segment override "
                        "possible)",
                        op->name);
                }
            }
         ...
         */
        CodeSimplifyType(
            &res2.type, expr2->file, expr2->line, (op->value != 0xd7));
        ...
        if (op->value == 0xd7)
        {
            if (opsize > 1)
            {
                Notify(NOTIFY_WARNING,
                    expr2->file,
                    expr2->line,
                    "XLAT operates only on a table of bytes, while the given "
                    "operand is made of %d-byte values",
                    opsize);
                opsize = 1;
            }
        }
    ...
    CodeFinal(addrPtr,
        ...
        expr1 ? expr1->line : (expr2 ? expr2->line : 0),
        delay);
    if (delay)
    {
        if (pass > 1)
 ...
 *
 ***********************************************************************/
FixResult Code_Shift(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                  /* # bytes previously allocated to inst */
    int pass,                      /* Current pass */
    Expr* expr1,                   /* Operand 1 */
    Expr* expr2,                   /* Operand 2 */
    Opaque data)                   /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_Shift, 2, data, &delay);
    ...
    *ip++ = (byte)(opcode + (Type_Size(res1.type) != 1 ? 1 : 0));
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Shift, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Test(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_Test, 4, data, &delay);
     ...
     */
    if (!CodeCompatible(
            expr1, &res1, expr2, &res2, TRUE, FALSE, delay, &opSize))
    {
        return (FR_ERROR);
    }
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Test, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
 ...
 *
 ***********************************************************************/
FixResult Code_IncDec(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                   /* # bytes previously allocated to inst */
    int pass,                       /* Current pass */
    Expr* expr1,                    /* Operand 1 */
    Expr* expr2,                    /* Operand 2 */
    Opaque data)                    /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, NULL, NULL, Code_IncDec, 3, data, &delay);
    if (result != FR_DONE)
    {
        return (result);
    }
 ...
 *
 ***********************************************************************/
FixResult Code_Xchg(int* addrPtr, /* IN/OUT: Address of instruction start */
    int prevSize,                 /* # bytes previously allocated to inst */
    int pass,                     /* Current pass */
    Expr* expr1,                  /* Operand 1 */
    Expr* expr2,                  /* Operand 2 */
    Opaque data)                  /* OpCode/other data */
{
     ...
     */
    result = CodeInitial(
        addrPtr, pass, expr1, &res1, expr2, &res2, Code_Xchg, 4, data, &delay);
    ...
    if (delay)
    {
        ...
        {
            Fix_Register(
                FC_FINAL, Code_Xchg, startAddr, ip - ibuf, expr1, expr2, data);
        }
    }
...
#define CAN_PUSHA \
    (procType & (PROC_80186 | PROC_80286 | PROC_80386 | PROC_80486))
 ...
 *
 ***********************************************************************/
void Code_Prologue(int frameNeeded, /* Non-zero if frame-pointer required*/
    ...
    int fpSaved)                    /* True if FP already saved */
{
    ...
    if (usesMask)
    {
        ...
        {
            if ((usesMask & regsave[i].mask) && !(regsave[i].mask & ignoreMask))
            {
                if (USE_386())
                {
                    int opSize = (usesMask & ((dword)(regsave[i].mask)
                                                 << REG_DWFLAGBASE))
                                     ? 4
                                     : 2;
 ...
 *
 ***********************************************************************/
void Code_Epilogue(int frameNeeded, int usesMask, int localSize)
{
 ...
 *
 ***********************************************************************/
void Code_ProfileBBlock(int* addrPtr)
{
    ExprResult res;
    static byte ibuf[8] = {0x2e, 0xc6, 0x06, 0x00, 0x00, 0xc9, 0x89, 0xc0};
    ...
    res.rel.fixed = 0;
    res.rel.sym =
        Sym_Enter(NullID, SYM_PROFILE_MARK, (*addrPtr) + 7, SYM_PROF_BBLOCK);
    ...
    Table_Store(curSeg->u.segment.code, sizeof(ibuf), (void*)ibuf, *addrPtr);
