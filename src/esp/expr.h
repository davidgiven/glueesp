...
#define MR_SEGREG_MASK                                         \
    0x18                /* Bits in override containing segment \
                         * register number */
...
#define MR_SEGREG                             \
    0x80 /* segment register; internal to Esp \
          * only */
 ...
 *	by (cptr).
 */
#define ExprStrElts(cptr) \
    (((strlen(cptr) + 1) + sizeof(ExprElt) - 1) / sizeof(ExprElt))
/*
 * ExprSkipString --
 *	Advance an ExprElt * (eptr) past the string that starts at its
 *	location.
 */
#define ExprSkipString(eptr) \
    ((ExprElt*)(eptr) + ExprStrElts(((ExprElt*)(eptr))->str))
 ...
 * a segment register.
 */
#define EXPR_IS_SEGREG(resP)                                                \
    (((resP)->data.ea.disp == 0) && ((resP)->data.ea.modrm == MR_SEGREG) && \
...
extern int Expr_Eval(Expr* expr, /* Expression to evaluate */
    ...
    byte* status);               /* Status of the evaluation.
                                  ...
                                  * more info */
#define EXPR_NOUNDEF                                \
    0x0001 /* Don't allow EXPR_IDENT terms -- if    \
            * they're still there, the symbols to   \
            * which they refer are undefined and an \
            * error should be declared */
#define EXPR_FINALIZE                                 \
    0x0002 /* Evaluation occurring during final pass, \
            * so difference of relocatable values     \
            * can be declared final */
#define EXPR_DATA_ENTER                           \
    0x0004 /* Value is being entered as data, not \
            * used as an instruction operand, so  \
            * "offset foo" should be returned as  \
            * an address, not a constant */
#define EXPR_RECURSIVE                        \
    0x0008 /* Used internally when evaluating \
            * a SYM_NUMBER */
#define EXPR_NOT_OPERAND                             \
    0x0010 /* Expression is not the operand of       \
            * an instruction, so complaints about    \
            * using an assumed segment override need \
            * not be generated */
#define EXPR_NOREF                                      \
    0x0020 /* Use of a symbol within the expression     \
...
#define EXPR_STAT_DIRECT                                                \
    0x08                        /* Uses direct addressing (CODE or DATA \
                                 * also set) */
#define EXPR_STAT_REGISTER 0x10 /* Is a register */
#define EXPR_STAT_DEFINED 0x20  /* All symbols defined */
#define EXPR_STAT_DELAY                          \
    0x40 /* Don't use value until final pass --  \
          * expression involves subtraction of   \
          * relocatable symbols that will not be \
          * resolved until then */
#define EXPR_STAT_GLOBAL                        \
    0x80 /* Symbol w.r.t. which relocation will \
