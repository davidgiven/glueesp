 ...
 *
 ***********************************************************************/
void Notify(NotifyType why, ID file, int line, char* fmt, ...)
{
    va_list args;
    ...
    *stderr = *stdout;
        ...
        if (argv[ac][0] == '-')
        {
            switch (argv[ac][1])
            {
...
#ifdef unix
                        dirs = (char**)realloc(
                            dirs, sizeof(char*) * currentMaxDirs);
#else /* this is for  lame METABLAM  */
                        dirs = (void*)realloc(
                            (malloc_t)dirs, sizeof(char*) * currentMaxDirs);
#endif
                    }
                ...
                case 'D':
                    ...
                    {
                        Usage(
                            "-D argument requires symbol name and optional "
                            "value");
                        /*NOTREACHED*/
                    }
                    doDefs = 1;
    ...
    if (libNameStr != 0)
    {
        ...
        while (1)
        {
            ...
            numLibNames += 1;
            libNames =
                (ID*)realloc((malloc_t)libNames, numLibNames * sizeof(ID));
            /*
             * Enter this name into the table and store its ID in the array
             */
            libNames[numLibNames - 1] =
                ST_Enter(output, symStrings, cp, cp2 - cp);
            /*
 ...
 *
 ***********************************************************************/
void malloc_err(int fatal, char* str, int len)
{
    write(2, str, len);
    if (fatal)
    {
