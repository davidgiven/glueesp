 ...
 */
#define EXPR_FINAL_CONVERT                            \
    0x8000 /* If passed to ExprConvertSym, indicates  \
 ...
 */
#define OVERRIDE_TO_SEGREG(reg) \
    (((reg) >= 0x64) ? ((reg)-0x60) : (((reg)&MR_SEGREG_MASK) >> MR_REG_SHIFT))
 ...
 */
#define IS_SEGREG(resP)                                       \
    (((resP)->type == NULL) && ((resP)->data.ea.disp == 0) && \
        ((resP)->data.ea.modrm == MR_SEGREG) &&               \
        ((resP)->data.ea.override != 0) && ((resP)->rel.sym == NULL))
/*
...
#define ExprSymOffset(sym)                              \
    (((sym)->type == SYM_CHUNK) ? (sym)->u.chunk.handle \
...
#define UNDEF_END_PTR \
    (&undefMsg[sizeof(undefMsg) - UNDEF_TOO_MANY_STRLEN - UNDEF_FINISH_STRLEN])
 ...
 *
 ***********************************************************************/
static void ExprUndefInit(void)
{
    undefMsg[0] = '\0';
    undefPtr = undefMsg;
 ...
 *
 ***********************************************************************/
static void ExprUndefAdd(ID id)
{
    if (!undefFull)
    {
 ...
 *
 ***********************************************************************/
static char* ExprUndefFinish(void)
{
 ...
 *
 ***********************************************************************/
Expr* Expr_Copy(Expr* expr, int usurp)
{
 ...
 *
 ***********************************************************************/
void Expr_NextPart(Expr* src, Expr* dest, int resolveInherits)
{
    ExprElt* elt;
    int n;
 ...
 *
 ***********************************************************************/
static int ExprExtractType(ExprResult* tos, /* Element to evaluate */
        ...
        case (int)EXPR_TYPE_SYM:
            switch (tos->data.sym->type)
            {
                ...
                case SYM_PROC:
                     ...
                     */
                    result =
                        Sym_IsNear(tos->data.sym) ? Type_Near() : Type_Far();
                    break;
                case SYM_CLASS:
                    /*
 ...
 *
 ***********************************************************************/
static int ExprExtractSegReg(ExprResult* tos, /* Element to evaluate */
 ...
 *
 ***********************************************************************/
byte Expr_Status(ExprResult* result)
{
    byte value;
    ...
    switch ((int)result->type)
    {
        ...
        default:
            ...
            else if (result->type == Type_Near() || result->type == Type_Far())
            {
                /*
 ...
 *
 ***********************************************************************/
static void ExprSetDispSize(
    ExprResult* result, int delay) /* Non-zero if result being delayed.
                                    ...
                                    * to make sure don't get hemmed in. */
{
             ...
             ((!(result->data.ea.dword_ea) &&
                 ...
                 (result->data.ea.dword_ea &&
                     ...
                     (((result->data.ea.modrm & MR_RM_MASK) != MR_SIB &&
                          ((result->data.ea.modrm & MR_DISP_MASK) ==
                                  MR_NULL_DISP ||
                              (result->data.ea.modrm & MR_RM_MASK) !=
                                  REG_EBP)) ||
                         /* R/M == SIB && (Mod == 0 || base != EBP) */
                         ((result->data.ea.modrm & MR_RM_MASK) == MR_SIB &&
                             ((result->data.ea.modrm & MR_DISP_MASK) ==
                                     MR_NULL_DISP ||
                                 (result->data.ea.sib & SIB_BASE_MASK) !=
                                     REG_EBP))))))
    {
        /*
    ...
    else if (result->data.ea.disp >= -128 && result->data.ea.disp <= 127)
    {
        /*
 ...
 *
 ***********************************************************************/
static int ExprConvertSym(Expr* expr, /* Expression, for figuring any
                                       * needed override. */
    ExprResult* result,
    char** msgPtr,
    int flags,
    byte* statusPtr)
{
    if (result->type == EXPR_TYPE_SYM)
    {
        ...
        {
                /*
                 * Things with addresses. The displacement is 0, since the
                 * things are relocatable (we let the linker take care of it).
                 * The relocation frame is the group in which the symbol
                 * resides, if its segment is in a group, or the symbol's
                 * segment, if not.
                 */
            case SYM_VAR:
		...
		if (sym->segment->u.segment.data->comb == SEG_ABSOLUTE) {
            ...
            case SYM_NUMBER:
                ...
                else if (!Expr_Eval(sym->u.equate.value,
                             result,
            ...
            case SYM_PROTOMINOR:
                *msgPtr =
                    "protominor symbol cannot be used in an expression as it "
                    "has no value";
                return (0);
            case SYM_METHOD:
            case SYM_VARDATA:
 ...
 *
 ***********************************************************************/
static int ExprMangleString(
    Expr* expr, ExprResult* result, char** msgPtr, int flags, byte* statusPtr)
{
    /*
    ...
    if (result->type == EXPR_TYPE_STRING)
    {
        ...
        if (result->data.str[1])
        {
            ...
            {
                result->data.number =
                    (result->data.str[0] << 8) | (result->data.str[1] & 0xff);
            }
            else
            {
                result->data.number =
                    (result->data.str[0] & 0xff) | (result->data.str[1] << 8);
            }
        }
        else
        {
...
void ExprOutputProtoMinorRelocation(Symbol* protoSym)
{
    ...
    if (protoMinorRelocationSeg == NullSymbol)
    {
        ...
        bogusId =
            ST_EnterNoLen(output, symStrings, "ProtoMinorRelocationSegment");
        protoMinorRelocationSeg =
            Sym_Enter(bogusId, SYM_SEGMENT, SEG_LIBRARY, 0, NullID);
    }
 ...
 *
 ***********************************************************************/
static int ExprConvertConstOperand(
    Expr* expr, ExprResult* result, char** msgPtr, int flags, byte* statusPtr)
{
    /*
 ...
 *
 ***********************************************************************/
static int ExprIndirect(Expr* expr,
    ExprResult* result,
    char** errMsgPtr,
    int flags,
    byte* statusPtr)
{
    if (result->type == EXPR_TYPE_CONST)
    {
        ...
        else
        {
            ...
            if (result->data.ea.disp >= -65536 && result->data.ea.disp <= 65535)
            {
                result->data.ea.modrm = MR_DIRECT;
                result->data.ea.dword_ea = 0;
            }
    ...
    Expr* expr, ExprResult* result, char** msgPtr, int flags, byte* statusPtr)
{
    ...
    else if ((result->type > EXPR_TYPE_MAX_SPECIAL) && result->rel.sym &&
 ...
 *
 ***********************************************************************/
static int ExprAddCurrentDisp(ExprResult* result)
{
    if (result->rel.sym)
    {
 ...
 *
 ***********************************************************************/
static int ExprRelOp(Expr* expr,
    ExprOp op,
    ExprResult* lhs,
    ExprResult* rhs,
    char** msgPtr,
    ...
    if ((op == EXPR_EQ) || (op == EXPR_NEQ))
    {
        ...
        switch ((long)lhs->type)
        {
            case (int)EXPR_TYPE_STRING:
                ...
                else if (!ExprMangleString(expr, lhs, msgPtr, flags, statusPtr))
                {
                    return (0);
                }
                /*FALLTHRU*/
        ...
        switch ((long)lhs->type)
        {
            ...
            case (int)EXPR_TYPE_CONST:
                ...
                if (rhs->type == EXPR_TYPE_CONST)
                {
                    ...
                    {
                        *msgPtr =
                            "can only compare external constants with EQ and "
                            "NE";
                        return (0);
                    }
                    else
            ...
            default:
                         ...
                         (rhs->type == NULL))
                {
                    ...
                    {
                        *msgPtr =
                            "addressing modes can only be compared with EQ and "
                            "NE";
                        return (0);
                    }
                    else
 ...
 *
 ***********************************************************************/
static int ExprConstBinOp(Expr* expr,
    ExprOp op,
    ExprResult* lhs,
    ExprResult* rhs,
    char** msgPtr,
 ...
 *
 ***********************************************************************/
static int ExprConstUnOp(Expr* expr,
    ExprOp op,
    ExprResult* operand,
    char** msgPtr,
    int flags,
 ...
 *
 ***********************************************************************/
static int ExprSubtract(
    Expr* expr, ExprResult* tos, char** errMsgPtr, int flags, byte* statusPtr)
{
    ...
    if (lhs->type == EXPR_TYPE_CONST)
    {
        ...
        {
            *errMsgPtr =
                "cannot subtract anything but a constant from a constant";
            return (0);
        }
        else if (!lhs->rel.sym)
        {
        ...
        else
        {
            ...
            if (rhs->rel.sym)
            {
                ...
                {
                    *errMsgPtr =
                        "cannot perform arithmetic on segments or groups";
                    return (0);
                }
    ...
    else if (lhs->data.ea.modrm == MR_DIRECT)
    {
        ...
        else if (rhs->type == EXPR_TYPE_CONST)
        {
            ...
            else if (lhs->rel.sym)
            {
                ...
                {
                    *errMsgPtr =
                        "cannot subtract operands from different segments";
                    return (0);
                }
                /*
             ...
             (rhs->data.ea.modrm == MR_DIRECT))
    {
        ...
        {
            *errMsgPtr =
                "cannot subtract a relocatable value from a non-relocatable "
                "value";
            return (0);
        }
    ...
    Expr* expr, ExprResult* tos, char** errMsgPtr, int flags, byte* statusPtr)
{
     ...
     * RHS.
     */
    if ((lhs->type == EXPR_TYPE_CONST) && (rhs->type == EXPR_TYPE_CONST))
    {
        /*
    ...
    Expr* expr, ExprResult* tos, char** errMsgPtr, int flags, byte* statusPtr)
{
            ...
            (tos->data.sym->type == SYM_INSTVAR)))
    {
         ...
         */
        while ((field->type == SYM_FIELD) || (field->type == SYM_INSTVAR))
        {
 ...
 *
 ***********************************************************************/
int Expr_Eval(Expr* expr, /* Expression to evaluate */
...
#define ASSERT_386()                                                   \
    ...
    {                                                                  \
        ExprError(                                                     \
            "80386-register/addressing form not supported by current " \
            "processor");                                              \
    }
    /*
    ...
    while (n > 0)
    {
        switch (elt->op)
        {
            ...
            case EXPR_NOT:
                 ...
                 */
                retVal =
                    ExprConstUnOp(expr, elt->op, tos, &errMsg, flags, &status);
                if (!retVal)
                {
                    goto finish;
                }
            ...
            case EXPR_GE:
                retVal = ExprRelOp(
                    expr, elt->op, tos + 1, tos, &errMsg, flags, &status);
                if (!retVal)
                {
                    goto finish;
                }
                ...
                switch (*(long*)tp)
                {
                    ...
                    case (int)EXPR_TYPE_CONST:
                        ...
                        if (tos->rel.sym)
                        {
                             ...
                             */
                            tos->rel.size = (tos->rel.sym->type == SYM_SEGMENT)
                                                ? FIX_SIZE_WORD
                                                : FIX_SIZE_BYTE;
                        }
            ...
            case EXPR_TYPEOP:
            {
                ...
                else if (masmCompatible && (tos->type != NULL) &&
                         (tos->type != EXPR_TYPE_CONST) &&
                         (tos->type != EXPR_TYPE_STRING) &&
                         (tos->type != EXPR_TYPE_TYPE) &&
                ...
                else if ((tos->type == EXPR_TYPE_CONST) && (tos->rel.sym) &&
                ...
                if (tos->type == EXPR_TYPE_SYM)
                {
                    switch (tos->data.sym->type)
                    {
                        ...
                        case SYM_RECORD:
                            ...
                            /* Code-related */
                        case SYM_PROC:
                            val = tos->data.sym->u.proc.flags << 8;
                            break;
                        case SYM_LOCALLABEL:
                        case SYM_LABEL:
                            val = tos->data.sym->u.label.near ? (1 << 8) : 0;
                            break;
                            /* Data-related */
                        case SYM_VAR:
                            val = (1 << 8);
            ...
            case EXPR_FIRST:
                ...
                {
                    tos->data.number = tos->data.sym->u.eType.firstVal;
                }
                else
                {
            ...
            case EXPR_HANDLE:
                ...
                else if (tos->type == EXPR_TYPE_SYM)
                {
                    ...
                    tos->data.number = 0;
                         ...
                         ((tos->data.number == 0) || (tos->data.number == -1)))
                {
                    /*
                ...
                else
                {
                        ...
                        tos->type != EXPR_TYPE_STRING)
                    {
                        tos[1].type = type;
                             ...
                             tos->rel.sym == NULL && tos->rel.frame != NULL)
                    {
            ...
            case EXPR_OVERRIDE:
            {
                ...
                if (tos->type == EXPR_TYPE_CONST)
                {
                    ...
                    {
                        ExprError(
                            "can't give segment override to segment symbol");
                    }
                    else
                    {
                        /*
                         ...
                         * The type we get from the frame field of the
                         * relocation, to deal with things like ds:field, where
            ...
            case EXPR_HIGHPART:
                ...
                if (tos->type != NULL && tos->type <= EXPR_TYPE_MAX_SPECIAL)
                {
                    ...
                    else if (tos->type == EXPR_TYPE_CONST && tos->rel.frame)
                    {
                        ...
                        if (elt->op == EXPR_LOWPART || elt->op == EXPR_HIGHPART)
                        {
                            if ((size != 4) && (size != 2))
                            {
                                goto lowHighSizeErr;
                    ...
                    else
                    {
                        switch (elt->op)
                        {
                            case EXPR_OFFPART:
                                ExprError(
                                    "cannot find offset/chunk portion of "
                                    "expression");
                            case EXPR_SEGPART:
                                ExprError(
                                    "cannot find segment/handle portion of "
                                    "expression");
                            case EXPR_LOWPART:
                                ExprError(
                                    "cannot find low portion of expression");
                            case EXPR_HIGHPART:
                                ExprError(
                                    "cannot find high portion of expression");
                        }
                    }
                }
                else if (elt->op == EXPR_LOWPART || elt->op == EXPR_HIGHPART)
                {
                    ...
                    lowHighSizeErr:
                        ExprError(
                            "operand of .low/.high must be 2 or 4 bytes long");
                    }
                    else
                    {
                ...
                else
                {
                    if (tos->type != NULL)
                    {
                        ...
                        segOffSizeErr:
                            ExprError(
                                "operand of .offset/.chunk/.segment/.handle "
                                "must be four bytes long");
                        }
                    }
                    /*
...
finish:
    ...
    if (retVal)
    {
        ...
        else if (ExprConvertSym(
                     expr, tos, &errMsg, flags | EXPR_FINAL_CONVERT, &status))
        {
            /*
            ...
            if (tos->type > EXPR_TYPE_MAX_SPECIAL)
            {
                    ...
                    (tos->rel.sym->type != SYM_CLASS))
                {
                    Notify(NOTIFY_WARNING,
                        expr->file,
                        expr->line,
                        "implicit override for variable %i generated by "
                        "segment assumptions",
                        tos->rel.sym->name);
                }
             ...
             */
            if ((flags & EXPR_DATA_ENTER) && (tos->type == EXPR_TYPE_CONST) &&
                (tos->rel.sym) && (tos->rel.type == FIX_OFFSET))
            {
            ...
            *result = *tos;
            ...
            else if (result->type == EXPR_TYPE_SEGSYM || IS_SEGREG(result))
            {
                ...
                result->type =
                    (result->data.ea.dword_ea) ? Type_Int(4) : Type_Int(2);
 ...
 *
 ***********************************************************************/
int Expr_InvolvesOp(Expr* expr, ExprOp op)
{
    int i;
    ExprElt* elt;
