 ...
 *
 ***********************************************************************/
static ID SymPermanentName(Symbol* sym)
{
 ...
 *
 ***********************************************************************/
static SymbolPtr SymAlloc(SymBucketPtr* bucketPtr)
{
 ...
 *
 ***********************************************************************/
ID Sym_BindMethod(SymbolPtr class, ID method, SymbolPtr proc, byte callType)
{
 ...
 *
 ***********************************************************************/
static void SymInherit(SymbolPtr inheritSym,
    ...
    int resolveInherits)
{
    ...
    for (bucket = *prev; bucket != NULL; bucket = *prev)
    {
        for (sym = bucket->syms; sym < bucket->ptr; sym++)
        {
            ...
            {
                Notify(NOTIFY_ERROR,
                    inheritSym->u.inherit.file,
                    inheritSym->u.inherit.line,
                    "cannot define additional locals when inheriting all "
                    "locals from %i",
                    sourceProc->name);
                return;
            }
     ...
     */
do_inherit:
    for (sBucket = (SymBucketPtr)sourceProc->u.proc.locals; sBucket != NULL;
         sBucket = sBucket->next)
    {
        for (sSym = sBucket->syms; sSym < sBucket->ptr; sSym++)
        {
            ...
            else if ((sSym->type == SYM_INHERIT) && !sSym->u.inherit.done)
            {
                /*
 ...
 *
 ***********************************************************************/
static void SymResolveInherit(SymbolPtr inheritSym, /* SYM_INHERIT symbol */
 ...
 *
 ***********************************************************************/
static SymbolPtr SymLookup(ID id, /* ID of symbol to find */
                                   ...
                                   * segment in bucket chain */
{
    ...
    else
    {
        ...
        if ((type == SYM_LOCALLABEL) || (type == SYM_LOCAL) ||
            (type == SYM_INHERIT))
        {
            /*
 ...
 *
 ***********************************************************************/
void Sym_SetAddress(SymbolPtr sym, int offset)
{
    ...
    if (seg->u.segment.data->last == NULL)
    {
        ...
        seg->u.segment.data->first = seg->u.segment.data->last = sym;
    }
 ...
 *
 ***********************************************************************/
LocalizeInfo* Sym_AllocLoc(SymbolPtr sym, ChunkDataType type)
{
 ...
 *
 ***********************************************************************/
SymbolPtr Sym_Enter(ID id, SymType type, ...)
{
    va_list args;
    SymbolPtr sym;
    ...
    if ((sym->flags & SYM_UNDEF) || (type == SYM_NUMBER))
    {
        ...
        switch (type)
        {
            case SYM_BITFIELD:
                ...
                {
                    yyerror(
                        "%i cannot be redefined as a record field", sym->name);
                }
                break;
            case SYM_CHUNK:
                /*
                ...
                if (old.type != SYM_LASTADDR)
                {
                    ...
                    {
                        yyerror("%i cannot be redefined as a chunk", sym->name);
                    }
            ...
            case SYM_CLASS:
                ...
                {
                    yyerror(
                        "%i cannot be redefined as an object class", sym->name);
                }
                break;
            case SYM_ENUM:
            {
                ...
                if (etype->u.eType.common.size == 1)
                {
                    ...
                    {
                        yyerror(
                            "value %d too large for byte-sized enumerated "
                            "constant %i",
                            sym->u.econst.value,
                            sym->name);
                    }
            ...
            case SYM_VARDATA:
            {
                ...
                {
                    yyerror(
                        "%i cannot be redefined as a vardata type", sym->name);
                }
                break;
            }
            case SYM_ETYPE:
            ...
            case SYM_FIELD:
            {
                ...
                {
                    sType->u.sType.last = sType->u.sType.first = sym;
                }
            ...
            case SYM_GROUP:
            {
                 ...
                 */
                sym->u.group.segs = (SymbolPtr*)malloc(
                    (sym->u.group.nSegs > 0 ? sym->u.group.nSegs : 1) *
                    sizeof(SymbolPtr));
                 ...
                 */
                numGroups += 1;
                groups = (SymbolPtr*)realloc(
                    (void*)groups, numGroups * sizeof(SymbolPtr));
            ...
            case SYM_LABEL:
                ...
                {
                    yyerror("%i cannot be redefined as a %slabel",
                        sym->name,
            ...
            case SYM_MACRO:
                ...
                {
                    yyerror("%i cannot be redefined as a macro", sym->name);
                }
                break;
            case SYM_METHOD:
            {
                ...
                if (old.type != SYM_LASTADDR && old.type != SYM_METHOD &&
                    old.type != SYM_NUMBER)
                {
                    yyerror("%i cannot be redefined as a message", sym->name);
                }
                break;
            }
            case SYM_NUMBER:
            ...
            case SYM_PROC:
                ...
                if (old.type != SYM_LASTADDR && old.type != SYM_PROC &&
                    old.type != SYM_LABEL)
                {
                    yyerror("%i cannot be redefined as a procedure", sym->name);
                }
                break;
            case SYM_PUBLIC:
                /*
            ...
            case SYM_RECORD:
                ...
                {
                    yyerror("%i cannot be redefined as a record", sym->name);
                }
                break;
            ...
            case SYM_SEGMENT:
            {
                 ...
                 */
                numSegments += 1;
                segments = (SymbolPtr*)realloc(
                    (void*)segments, numSegments * sizeof(SymbolPtr));
                segments[numSegments - 1] = sym;
                if (old.type != SYM_LASTADDR && old.type != SYM_SEGMENT)
                {
                    yyerror("%i cannot be redefined as a segment", sym->name);
                }
                /*
            ...
            case SYM_STRING:
                ...
                {
                    yyerror(
                        "%i cannot be redefined as a string equate", sym->name);
                }
                break;
            case SYM_STRUCT:
            case SYM_UNION:
            ...
            case SYM_TYPE:
                ...
                {
                    yyerror("%i cannot be redefined as a typedef", sym->name);
                }
                break;
            case SYM_VAR:
                /*
                ...
                if (old.type != SYM_LASTADDR)
                {
                    ...
                    {
                        yyerror(
                            "%i cannot be redefined as a variable", sym->name);
                    }
 ...
 *
 ***********************************************************************/
SymbolPtr Sym_Find(ID id, SymType type, int resolveInherits)
{
    SymBucketPtr bucket;
    SymbolPtr sym;
    ...
    if (curProc && (type != SYM_PROC))
    {
         ...
         */
        for (bucket = (SymBucketPtr)curProc->u.proc.locals; bucket != NULL;
             bucket = bucket->next)
        {
            for (sym = bucket->syms; sym < bucket->ptr; sym++)
            {
                ...
                else if (resolveInherits && (sym->type == SYM_INHERIT) &&
                         !sym->u.inherit.done)
                {
                    /*
 ...
 *
 ***********************************************************************/
void Sym_ForEachSegment(SymForEachProc* func, Opaque data)
{
    int i;
 ...
 *
 ***********************************************************************/
void Sym_ForEachLocal(SymbolPtr proc, SymForEachProc* func, Opaque data)
{
    SymBucketPtr bucket;
    SymbolPtr sym;
    ...
    for (bucket = (SymBucketPtr)proc->u.proc.locals; bucket != 0;
         bucket = bucket->next)
    {
        for (sym = bucket->syms; sym < bucket->ptr; sym++)
        {
 ...
 *
 ***********************************************************************/
int SAAO_Callback(SymbolPtr sym, Opaque data)
{
 ...
 *
 ***********************************************************************/
int SRAL_Callback(SymbolPtr sym, Opaque data)
{
    Sym_Reference(sym);
    return (0);
}

void Sym_ReferenceAllLocals(SymbolPtr proc)
{
 ...
 *
 ***********************************************************************/
static void SymAllocTypeBlock(SymWriteData* swd)
{
    if (swd->tmem)
    {
        ObjTypeHeader* thdr;
 ...
 *
 ***********************************************************************/
static void SymConvertType(TypePtr type, /* Type to convert */
                                          ...
                                          * description token */
{
    ...
    switch (type->tn_type)
    {
        ...
        case TYPE_SIGNED:
            *token = OTYPE_SIGNED | (type->tn_u.tn_int << 1) | OTYPE_SPECIAL;
            return;
        case TYPE_CHAR:
            *token = OTYPE_CHAR | ((type->tn_u.tn_charSize - 1) << 1) |
                     OTYPE_SPECIAL;
            return;
        case TYPE_VOID:
        ...
        case TYPE_PTR:
            ...
            {
                *token = OTYPE_PTR | (type->tn_u.tn_ptr.tn_ptrtype << 1) |
                         OTYPE_SPECIAL;
                return;
            }
            else
            {
                ot.words[0] =
                    (type->tn_u.tn_ptr.tn_ptrtype << 1) | OTYPE_SPECIAL;
                next = type->tn_u.tn_ptr.tn_base;
                break;
            }
        case TYPE_ARRAY:
            ot.words[0] =
                (0x8000 | ((type->tn_u.tn_array.tn_length << 1) & 0x7ffe) |
                    OTYPE_SPECIAL);
    ...
    else
    {
        ...
        if (swd->typeOff == swd->typeSize)
        {
            ...
            base->num =
                (swd->typeSize - sizeof(ObjTypeHeader)) / sizeof(ObjType);
        }
 ...
 *
 ***********************************************************************/
static inline void SymAllocSymBlock(SymWriteData* swd,
 ...
 *
 ***********************************************************************/
static void SymExpandBlock(SymWriteData* swd, int bytesNeeded) /* Guess what? */
{
 ...
 *
 ***********************************************************************/
static int SymCheckUnref(SymbolPtr sym, /* Symbol to check */
            ...
            sym->u.label.unreach)
        {
            ...
            if (confirmed)
            {
                ...
                Notify(NOTIFY_WARNING,
                    file,
                    line,
                    "code after %i cannot be reached",
                    sym->name);
                curProc = prevProc;
            }
        }
 ...
 *
 ***********************************************************************/
static void SymWriteNonAddress(
    ObjSegment* seg, SymWriteData* swdp, word segOff, SymbolPtr segSym)
{
    int i;
    SymbolPtr sym;
                ...
                switch (sym->type)
                {
                    ...
                    case SYM_CLASS:
                        if (sym->flags & SYM_UNDEF)
                        {
                            ...
                            else if (sym->u.class.data->flags &
                                     SYM_CLASS_MASTER)
                            {
                        ...
                        /* XXX
                    {
                        ...
                        stringEquates += 1;
                        for (mp = sym->u.string.value; mp != NULL; mp =
                    mp->next) { if (mp->dynamic) { stringBlocks += 1;
                            }
                        }
                    }
                    */
                    ...
                    case SYM_NUMBER:
                    {
                            ...
                            sym->u.equate.rdonly)
                        {
                            ...
                            if (!Expr_Eval(sym->u.equate.value,
                                    &res,
                                    ...
                                    &status))
                            {
                                Notify(NOTIFY_ERROR,
                                    ...
                                    "Cannot store value for %i in output "
                                    "table: %s",
                                    sym->name,
                                    (char*)res.type);
                                break;
                                 ...
                                 fld = fld->u.eltsym.next)
                            {
                                ...
                                if (fld->type == SYM_FIELD)
                                {
                                    swdp->nextSym->u.sField.offset =
                                        fld->u.field.offset;
                                    SymConvertType(fld->u.field.type,
                                        swdp,
                                ...
                                else
                                {
                                    ...
                                    SymConvertType(fld->u.instvar.type,
                                        swdp,
                                        &swdp->nextSym->u.sField.type);
                                }
                        ...
                        else
                        {
                                 ...
                                 fld = fld->u.eltsym.next)
                            {
                                ...
                                if (fld->u.bitField.type)
                                {
                                     ...
                                     * type description.
                                     */
                                    SymConvertType(fld->u.bitField.type,
                                        swdp,
                    ...
                    case SYM_TYPE:
                    {
                        ...
                        swdp->nextSym->flags = sym->flags;
                        SymConvertType(sym->u.typeDef.type,
                            swdp,
 ...
 *
 ***********************************************************************/
static int SymWriteSegment(SymbolPtr segSym, ObjSegment* seg)
{
    ...
    while (sym != NULL)
    {
            ...
            switch (endSym->type)
            {
                ...
                case SYM_PROC:
                {
                    ...
                    for (bucket = (SymBucketPtr)endSym->u.proc.locals;
                         ...
                         bucket = bucket->next)
                    {
                        for (local = bucket->syms; local < bucket->ptr; local++)
                        {
                            ...
                            if (local->type == SYM_LOCALLABEL)
                            {
                                         ...
                                         mustReach->u.addrsym.offset)
                                {
                                     ...
                                     * same address, we have to make sure
                                     * mustReach points to the *last* one with
                                     * that address, so we run down the
                                     * by-address list from the local label so
                                     * long as the address is still the same.
                                     * Any non-local label can go in another
                                     * block, so we avoid setting mustReach to
                                     * such a beast if it is encountered.
                                     */
                                    SymbolPtr asym;
                ...
                case SYM_CLASS:
                {
                    ...
                    {
                        swd.symSize +=
                            (bucket->ptr - bucket->syms) * sizeof(ObjSym);
                    }
                    /*
                     * Add in class symbol
                     */
        ...
        while (sym != endSym)
        {
            ...
            if (sym->type != SYM_LINE)
            {
                if ((sym->type != SYM_ONSTACK) && (sym->name != NullID))
                {
                    (void)SymCheckUnref(sym,
                        (sym->type == SYM_LOCALLABEL ? lastProcSym : NULL),
                        file,
                        line,
                        &lastRefOff);
                }
            ...
            switch (sym->type)
            {
                    ...
                    if (seg->size != 0)
                    {
                        if (lines == NULL)
                        {
                            ...
                            if (lineMap == NULL)
                            {
                                 ...
                                 */
                                seg->lines = lineMap = VMAlloc(output,
                                    sizeof(ObjAddrMapHeader),
                                    OID_ADDR_MAP);
                            }
                            /*
                             * Expand the line map by another entry and point
                             * oame at the newly created entry.
                             */
                            oamh = (ObjAddrMapHeader*)VMLock(
                                output, lineMap, &lmmem);
                            oamh->numEntries += 1;
                            MemReAlloc(lmmem,
                                sizeof(ObjAddrMapHeader) +
                                    (oamh->numEntries *
                                        sizeof(ObjAddrMapEntry)),
                                0);
                            MemInfo(lmmem, (genptr*)&oamh, (word*)NULL);
                            oame =
                                &((ObjAddrMapEntry*)(oamh +
                                                     1))[oamh->numEntries - 1];
                             ...
                             */
                            lines =
                                VMAlloc(output, OBJ_INIT_LINES, OID_LINE_BLOCK);
                            /*
                            ...
                            prevLineH = (ObjLineHeader*)VMLock(
                                output, lines, &lineHandle);
                            prevLineH->next = (VMBlockHandle)0;
                            prevLineH->num =
                                (OBJ_INIT_LINES - sizeof(ObjLineHeader)) /
                                sizeof(ObjLine);
                        ...
                        if (*curFile != sym->u.line.file)
                        {
                            ...
                            {
                                if (!MemReAlloc(lineHandle, sizeNeeded, 0))
                                {
                                    Notify(NOTIFY_ERROR,
                                        NullID,
                                        0,
                                ...
                                else
                                {
                                     ...
                                     */
                                    curLine =
                                        (ObjLine*)((char*)prevLineH + curLOff);
                                }
                                prevLineH->num +=
                                    (sizeNeeded - OBJ_INIT_LINES) /
                                    sizeof(ObjLine);
                            }
                            /*
                            ...
                            (sym->u.line.common.offset != curLine[-1].offset))
                        {
                             ...
                             * record hasn't the same offset as the previous
                             * one, so we want to store a new record.
                             */
                ...
                case SYM_VAR:
                    SymConvertType(
                        sym->u.var.type, &swd, &swd.nextSym->u.variable.type);
                    swd.nextSym->type = OSYM_VAR;
                    break;
                case SYM_CHUNK:
                    SymConvertType(
                        sym->u.chunk.type, &swd, &swd.nextSym->u.chunk.type);
                ...
                case SYM_PROC:
                    ...
                    if (sym->u.proc.locals)
                    {
                        ...
                        for (bucket = (SymBucketPtr)sym->u.proc.locals;
                             ...
                             bucket = bucket->next)
                        {
                            for (local = bucket->syms; local < bucket->ptr;
                                 local++)
                            {
                                if (local->type == SYM_LOCALLABEL)
                                {
                                ...
                                switch (local->type)
                                {
                                    case SYM_LOCAL:
                                        ...
                                        SymConvertType(local->u.localVar.type,
                                            &swd,
                ...
                case SYM_CLASS:
                {
                    ...
                    if (sym->u.class.data->bindings)
                    {
                        ...
                        for (bucket = (SymBucketPtr)sym->u.class.data->bindings;
                             ...
                             bucket = bucket->next)
                        {
                            for (binding = bucket->syms; binding < bucket->ptr;
                                 binding++)
                            {
                                swd.nextSym++;
                                swd.symOff += sizeof(ObjSym);
                ...
                case SYM_ONSTACK:
                    ...
                    OBJ_STORE_SID(
                        swd.nextSym->u.onStack.desc, sym->u.onStack.desc);
                    break;
                default:
                    Notify(NOTIFY_ERROR,
                        NullID,
...
int Sym_ProcessSegments(void)
{
    ...
    for (i = 0; i < numSegments; i++)
    {
        ...
        if ((nextSeg->type != SEG_ABSOLUTE) && (nextSeg->type != SEG_GLOBAL) &&
    ...
    if (entryPoint)
    {
        ...
        if (!Fix_OutputRel(
                (SymbolPtr)NULL, 0, &entryRes.rel, (Opaque)&hdr->entry))
        {
            Notify(NOTIFY_ERROR,
                entryPoint->file,
                entryPoint->line,
 ...
 *
 ***********************************************************************/
void Sym_Adjust(SymbolPtr seg, int start, int diff)
{
    SymbolPtr sym;
 ...
 *
 ***********************************************************************/
void Sym_AddToGroup(SymbolPtr grp, SymbolPtr seg)
{
    grp->u.group.nSegs += 1;
    grp->u.group.segs = (SymbolPtr*)realloc(
        (void*)grp->u.group.segs, grp->u.group.nSegs * sizeof(SymbolPtr));
