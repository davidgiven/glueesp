 ...
 *
 ***********************************************************************/
static void TableVerify(TablePtr tp)
{
    int i;
    TableChunk* tcp;
    int size;
 ...
 *
 ***********************************************************************/
Table Table_Init(int eltSize, /* Size of each element */
    int eltsPerChunk)         /* Elements allocated per chunk */
{
    ...
    tp->chunks = (TableChunk*)malloc(tp->numChunks * sizeof(TableChunk));
    tp->tableSize = 0;
    tp->eltSize = eltSize;
 ...
 *
 ***********************************************************************/
void Table_Delete(Table table, /* Table to alter */
    ...
    int numElts)               /* Number of elements to delete */
{
    ...
    for (i = tp->numChunks, tcp = tp->chunks; numElts > 0 && i > 0; i--, tcp++)
    {
        if (tcp->numElts <= pos)
        {
            /*
 ...
 *
 ***********************************************************************/
void Table_Insert(Table table, int pos, int numElts)
{
    TablePtr tp = (TablePtr)table;
    int i;
    TableChunk* tcp;
        ...
        else
        {
                ...
                else if (i > 1)
                {
                    ...
                    else if (tcp[1].numElts < tcp->maxElts)
                    {
                        ...
                        if (tcp[1].maxElts < tcp[1].numElts + numElts)
                        {
                            ...
                            tcp[1].elts = (genptr)malloc(SCALE(numUsed, tp));
                            bcopy(old,
                    ...
                    else
                    {
                         ...
                         * new elements.
                         */
                        tcp->elts = (genptr)realloc(
                            tcp->elts, SCALE((tcp->numElts + numElts), tp));
            ...
            else if (i > 1)
            {
                ...
                else
                {
                    ...
                    old = tcp->elts;
                    tcp->elts =
                        (genptr)malloc(SCALE((tcp->numElts + numElts), tp));
            ...
            else
            {
                ...
                tp->numChunks += 1;
                tp->chunks = (TableChunk*)realloc(
                    (void*)tp->chunks, tp->numChunks * sizeof(TableChunk));
                ...
                bcopy(
                    src, tcp[1].elts + SCALE(destOff, tp), SCALE(numCopy, tp));
                tcp[1].numElts = numCopy + destOff;
                tcp->numElts = pos + numElts - destOff;
                numElts = 0;
            }
 ...
 *
 ***********************************************************************/
int Table_Write(Table table, VMBlockHandle block)
{
    TablePtr tp = (TablePtr)table;
    int i;
    TableChunk* tcp;
 ...
 *
 ***********************************************************************/
void* Table_Store(Table table, /* Table to modify */
                                ...
                                * elements are appended to the table */
{
    ...
    if (pos == TABLE_END || pos == tp->tableSize)
    {
         ...
         */
        tp->chunks = (TableChunk*)realloc(
            (void*)tp->chunks, ((tp->numChunks + 1) * sizeof(TableChunk)));
        /*
         * Initialize same
         */
    ...
    else
    {
        ...
        if (pos < tp->tableSize)
        {
            for (i = tp->numChunks, tcp = tp->chunks; i > 0 && numElts;
                 i--, tcp++)
            {
                if (pos < tcp->numElts)
                {
        ...
        else
        {
            ...
            {
                int chunksNeeded =
                    (need + tp->eltsPerChunk - 1) / tp->eltsPerChunk;
                int first;
                int i;
                ...
                {
                    tp->chunks[i].numElts = tp->chunks[i].maxElts =
                        tp->eltsPerChunk;

                    tp->chunks[i].elts =
                        (genptr)calloc(tp->eltsPerChunk, tp->eltSize);
 ...
 *
 ***********************************************************************/
void* Table_StoreZeroes(Table table, /* Table to modify */
    int numElts,                     /* Number of elements to store */
    int pos) /* Position at which to store. If TABLE_END,
              * elements are appended to the table */
{
 ...
 *
 ***********************************************************************/
void* Table_Lookup(Table table, int pos)
{
    TablePtr tp = (TablePtr)table;
    int i;
    TableChunk* tcp;
 ...
 *
 ***********************************************************************/
void* Table_EnumFirst(Table table, Table_Enum* te)
{
    TablePtr tp = (TablePtr)table;
    TableChunk* tcp;
    int i;
 ...
 *
 ***********************************************************************/
void* Table_EnumNext(Table_Enum* te)
{
 ...
 *
 ***********************************************************************/
void Table_Fetch(Table table, /* Table from which to extract */
