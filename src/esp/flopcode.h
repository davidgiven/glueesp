...
/* Command-line: /usr/public/gperf -i14 -o -j1 -agSDptlTC -k2-5,$ -N
 * findFlopcode -H hashFlopcode flopcodes.gperf  */
    ...
    findFlopcode(register const char* str, register unsigned int len)
{
    ...
    if (len <= MAX_WORD_LENGTH && len >= MIN_WORD_LENGTH)
    {
        ...
        if (key <= MAX_HASH_VALUE && key >= MIN_HASH_VALUE)
        {
            ...
            if (len == key_len && *str == *resword->name &&
                !strcmp(str + 1, resword->name + 1))
                return resword;
        }
    }
    return 0;
