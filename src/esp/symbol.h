...
#define SYM_CLASS_CHECKING                           \
    0x00000004 /* Checking class for relativity (see \
                * CheckRelated in parse.y) */
#define SYM_CLASS_FORWARD                               \
    0x00000008   /* Class was declared forward by an    \
...
#define SYM_ANY                                            \
    SYM_LASTADDR /* Type to pass to Sym_Find to tell it to \
...
#define SYM_NAMELESS                            \
    0x0020 /* Symbol's name is fake and not for \
            * human consumption */

#define SYM_NOWRITE                           \
    0x0100 /* Symbol is not to be written to  \
            * the output file. Used for local \
            * symbols ($ and <n>$) to alert   \
            * the fixup module it must do     \
            * special things */
#define SYM_PERM                                              \
    0x0200                   /* Symbol's name is in permanent \
                              * storage already */
    ...
    union
    {
         ...
         */
        struct addrcom
        {                         /* Common info for address-bearing
                                   * symbols */
...
#define SYM_WEIRD                          \
    0x0002 /* Set if procedure contains an \
            * on_stack directive */
#define SYM_NO_JMP                                                  \
    0x0004                    /* Set if procedure may not be jumped \
                               * to */
...
#define SYM_HANDLER_MASK                                       \
    0x0070                 /* Mask of bits to indicate special \
                            * method handler */
...
#define SYM_PROF_COUNT 2
        ...
        { /* Enumerated type info */
            ...
            word incr;            /* Increment */
            byte flags;
#define SYM_ETYPE_PROTOMINOR                \
    0x01 /* Set if members of the etype are \
...
#define SYM_VAR_PUBLIC                                            \
    0x0001                   /* May be accessed by routines other \
                              * than handler for owning class     \
                              * or descendant */
#define SYM_VAR_STATE 0x0002 /* Field stored in state block */
#define SYM_VAR_NORELOC                                                  \
    0x0004                         /* Field should not be entered in the \
...
#define SYM_METH_RANGE                        \
    0x0002 /* Method is actually the start of \
            * a range */
#define SYM_METH_RANGE_LENGTH                                              \
    0xfffc                             /* Where the length of the exported \
                                        * range is stored */
#define SYM_METH_RANGE_LENGTH_OFFSET 2 /* Bit-shift to get to range length */
         ...
         */
        struct
        {                 /* Local variable (arg or variable) */
            int offset;   /* Offset from BP */
            TypePtr type; /* Type of variable */
        } localVar;
        struct
        {             /* inherit-locals-from binding */
...
#define Sym_IsNear(sym)                                                     \
    ((((sym)->type == SYM_LABEL) || ((sym)->type == SYM_LOCALLABEL))        \
            ? (sym)->u.label.near                                           \
            : (((sym)->type == SYM_PROC) ? ((sym)->u.proc.flags & SYM_NEAR) \
                                         : 0))
 ...
 */
extern SymbolPtr Sym_Find(ID id, SymType type, int resolveInherits);
 ...
 */
extern void Sym_ForEachLocal(SymbolPtr proc, SymForEachProc* func, Opaque data);
...
extern void Sym_Adjust(SymbolPtr seg, int start, int diff);
 ...
 */
extern ID Sym_BindMethod(
    SymbolPtr class, ID method, SymbolPtr proc, byte isStatic);
/*
 * Allocate and enter localization information for a chunk. The fields of
 * the thing are to be filled in later.
 */
extern LocalizeInfo* Sym_AllocLoc(SymbolPtr sym, ChunkDataType type);

extern void Sym_SetAddress(SymbolPtr sym, int offset);
