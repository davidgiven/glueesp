 ...
 ***********************************************************************/
#ifndef lint
static char* rcsid = "$Id: object.c,v 1.32 94/05/15 15:07:49 adam Exp $";
#endif lint
...
typedef struct
{
    SymbolPtr tag; /* SYM_VARDATA tag with which noreloc offset is
                    ...
                    * data, not vardata. */
#define ONR_LAST                                           \
    ((SymbolPtr)-1) /* Value for "tag" if this is the last \
...
#define ONR_ENTIRE_VARDATA                            \
    -1 /* Value in offset field for vardata if entire \
 ...
 *
 ***********************************************************************/
static TypePtr ObjFetchHandlerType(void)
{
 ...
 *
 ***********************************************************************/
static int ObjFigureMessageStart(int flags, SymbolPtr superClass)
{
    SymbolPtr cl;
    int mbase = 0;
    int masterBase;
 ...
 *
 ***********************************************************************/
static inline int ObjDataEnter(
    int* addrPtr, /* IN/OUT: address at which to store */
 ...
 *
 ***********************************************************************/
SymbolPtr Obj_DeclareClass(ID className, /* Name for new class */
    ...
    int flags)                           /* Flags describing inheritance */
{
    ...
    else
    {
        ...
        instance = Sym_Enter(ST_Enter(output,
                                 symStrings,
                                 tmpName,
        ...
        if (superClass)
        {
            ...
            base = Sym_Enter(ST_Enter(output,
                                 symStrings,
                                 tmpName,
                                 len + sizeof(OBJ_BASE_SUFFIX) - 1),
                SYM_STRUCT);
            ...
            (void)Sym_Enter(ST_Enter(output,
                                symStrings,
                                tmpName,
            ...
            if (flags & SYM_CLASS_VARIANT)
            {
                 ...
                 */
                for (cl = superClass; cl->u.class.super != NULL;
                     cl = cl->u.class.super)
                {
                    ;
                }
        ...
        else
        {
            ...
            {
                yywarning(
                    "meta-class %i defined without %s structure; using "
                    "instance structure as base",
                    className,
                    tmpName);
                /*
         ...
         */
        strcpy(tmpName + len,
            geosRelease >= 2 ? OBJ_METHODS_SUFFIX : OBJ_METHODS_SUFFIX_R1);
        methods = Sym_Enter(
            ST_Enter(output,
                symStrings,
                tmpName,
                len +
                    (geosRelease >= 2 ? sizeof(OBJ_METHODS_SUFFIX)
                                      : sizeof(OBJ_METHODS_SUFFIX_R1)) -
                    1),
            SYM_ETYPE,
            mbase,
            1,
        ...
        if (geosRelease >= 2)
        {
            ...
            vardata = Sym_Enter(ST_Enter(output,
                                    symStrings,
                                    tmpName,
 ...
 *
 ***********************************************************************/
static void ObjSetBit(SymbolPtr class, byte bit, int offset)
{
 ...
 *
 ***********************************************************************/
void Obj_EnterDefault(SymbolPtr class, /* Class to adjust */
    ...
    int callType)                      /* How handler may be called */
{
    ...
    {
        yyerror(
            "cannot define default message handler for %i as %i's class record "
            "is not defined yet",
            class->name,
            class->name);
        return;
    ...
    {
        yyerror(
            "default method already bound to %i for %i", conflict, class->name);
        return;
    }
 ...
 *
 ***********************************************************************/
void Obj_EnterReloc(SymbolPtr class, /* Class to adjust */
    SymbolPtr handler,               /* Handler procedure itself */
    Expr* expr)                      /* Expression for generating
                                      * fixup for handler pointer */
{
    ...
    {
        yyerror(
            "cannot define relocation message handler for %i as %i's class "
            "record is not defined yet",
            class->name,
            class->name);
        return;
    }
 ...
 *
 ***********************************************************************/
void Obj_EnterHandler(SymbolPtr class, /* Class for handler */
    SymbolPtr handler,                 /* Handler procedure itself */
    ...
    int callType)                      /* How handler may be called */
{
    ...
    {
        yyerror(
            "cannot define message handler for %i as %i's class record is not "
            "defined yet",
            class->name,
            class->name);
        return;
    ...
    if (conflict != NullID)
    {
        yyerror("%i already bound to %i for %i",
            method->name,
            conflict,
            class->name);
        return;
    ...
    if (callType == OBJ_DYNAMIC)
    {
        handler->u.proc.flags |=
            SYM_NO_JMP | SYM_NO_CALL | SYM_DYNAMIC | SYM_HANDLER;
    }
     ...
     */
    ObjSetBit(class,
        (1 << ((method->u.method.common.value >> 13) & 0x7)),
...
static void ObjFormRelocTable(
    SymbolPtr tag, ObjNoReloc* noreloc, SymbolPtr field, int offset);
 ...
 *
 ***********************************************************************/
static void ObjRelocType(SymbolPtr tag, /* VarData tag for which table is
    ...
    int foff)                           /* Field offset */
{
    ...
    switch (ftype->tn_type)
    {
        case TYPE_STRUCT:
            ...
            {
                ObjFormRelocTable(
                    tag, noreloc, ftype->tn_u.tn_struct->u.sType.first, foff);
            }
            break;
        case TYPE_ARRAY:
        {
            ...
            if (dot != start)
            {
                 ...
                 */
                Table_Fetch(curSeg->u.segment.code, diff, (void*)buffer, start);
                ...
                for (j = 1; j < ftype->tn_u.tn_array.tn_length; j++)
                {
                     ...
                     */
                    Table_Store(
                        curSeg->u.segment.code, diff, (void*)buffer, dot);
                    dot += diff;
                }
                free((void*)buffer);
            }
 ...
 *
 ***********************************************************************/
static void ObjFormRelocTable(SymbolPtr tag, /* VarData tag for which table
 ...
 *
 ***********************************************************************/
static void ObjFinishRelocTable(word offset, word cbase, word ptrOffset)
{
    if (dot != offset)
    {
        ExprResult res;
 ...
 *
 ***********************************************************************/
void Obj_DefineClass(SymbolPtr class, /* Class to define */
    ...
    Expr* initRoutine)                /* Initialization routine */
{
    ...
    else
    {
        ...
        if (!ObjDataEnter(&dot,
                Type_Ptr(TYPE_PTR_FAR, Obj_ClassType(NullSymbol)),
                &scExpr,
                1))
        {
            return;
    ...
    else
    {
        ...
        if (initRoutine != NULL)
        {
            (void)ObjDataEnter(&dot, ObjFetchHandlerType(), initRoutine, 1);
        }
        else
        {
            b[0] = b[1] = b[2] = b[3] = 0;
    ...
    if (flags)
    {
        ...
        if (!Expr_Eval(flags, &res, EXPR_NOUNDEF | EXPR_FINALIZE, NULL))
        {
             ...
             */
            Notify(NOTIFY_ERROR, flags->file, flags->line, (char*)res.type);
            return;
        }
    ...
    if ((cflags & (CLASSF_NEVER_SAVED | CLASSF_HAS_RELOC)) == 0)
    {
        ...
        {
            ObjFormRelocTable(
                NullSymbol, (ObjNoReloc*)class->u.class.data->noreloc, sym, 0);
        }

        ObjFinishRelocTable(offset,
            cbase,
        ...
        if (geosRelease >= 2)
        {
            ...
            ObjFinishRelocTable(
                offset, cbase, offsetof(Class, Class_vdRelocTable));
        }
    }
}
 ...
 *
 ***********************************************************************/
TypePtr Obj_ClassType(SymbolPtr sym)
{
    (void)ObjInit();
 ...
 *
 ***********************************************************************/
void Obj_NoReloc(SymbolPtr class, /* Class for which this is defined */
                                   ...
                                   */
{
        ...
        if ((res.type != EXPR_TYPE_CONST) || res.rel.sym)
        {
            if (varData == NullSymbol)
            {
                Notify(NOTIFY_ERROR,
                    expr->file,
                    expr->line,
                    "noreloc argument must be an already-defined instance "
                    "variable");
            }
            else
            {
                Notify(NOTIFY_ERROR,
                    expr->file,
                    expr->line,
                    "noreloc argument must be a field within the data bound to "
                    "%i",
                    varData->name);
            }
            return;
    ...
    else
    {
         ...
         */
        for (i = 0;
             ((ObjNoReloc*)class->u.class.data->noreloc)[i].tag != ONR_LAST;
             i++)
        {
            ;
        }
        class->u.class.data->noreloc = (Opaque)realloc(
            class->u.class.data->noreloc, (i + 2) * sizeof(ObjNoReloc));
    }
 ...
 *
 ***********************************************************************/
void Obj_ExportMessages(SymbolPtr class, /* Class from which to export it */
    ...
    Expr* length)                        /* Number of messages to export */
{
    ...
    if (!Expr_Eval(length, &res, EXPR_NOUNDEF | EXPR_FINALIZE, NULL))
    {
         ...
         * Error in evaluation -- return now
         */
        Notify(NOTIFY_ERROR, length->file, length->line, (char*)res.type);
        return;
    }
    ...
    if ((res.type != EXPR_TYPE_CONST) || res.rel.sym)
    {
        Notify(NOTIFY_ERROR,
            length->file,
            length->line,
            "the number of messages to be exported in the %i range must be a "
            "constant",
            rangeName);
        return;
    }
     ...
     */
    (void)Sym_Enter(rangeName,
        SYM_METHOD,
        class,
        SYM_METH_RANGE | (res.data.number << SYM_METH_RANGE_LENGTH_OFFSET));
 ...
 *
 ***********************************************************************/
static void ObjCheckETypeBounds(
    SymbolPtr etype, SymbolPtr class, char* typeName)
{
    int limit;
 ...
 *
 ***********************************************************************/
void Obj_CheckMessageBounds(SymbolPtr class)
{
 ...
 *
 ***********************************************************************/
void Obj_CheckVarDataBounds(SymbolPtr class)
{
