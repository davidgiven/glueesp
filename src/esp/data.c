 ...
 *
 ***********************************************************************/
static char* DataSkipInitializer(
    char** startPtr, /* Start of the field. Set to
                      ...
                      * byte */
{
    ...
    if (angles)
    {
        Notify(NOTIFY_ERROR,
            file,
            line,
            "unbalanced < initializing field %i",
            name);
        return ((char*)NULL);
    }
    else if (parens)
    ...
    else if (quote)
    {
        Notify(NOTIFY_ERROR,
            file,
            line,
            "unterminated string constant initializing %i",
            name);
        return ((char*)NULL);
    }
 ...
 *
 ***********************************************************************/
static char DataArrayString(void)
{
 ...
 *
 ***********************************************************************/
static int DataArrayStart(YYSTYPE* yylval, ...)
{
    /*
...
typedef struct
{
    ...
    int flags;
#define DRS_DEF_VAR                            \
    1 /* Non-zero if defining a variable (i.e. \
       * parse caused by Data_Enter, not       \
       * Data_EncodeRecord) */
#define DRS_WARNED                                              \
    2                /* Non-zero if not defining a variable,    \
 ...
 *
 ***********************************************************************/
static int DataRecordNotEOF(void)
{
    yywrap = yystdwrap;
 ...
 *
 ***********************************************************************/
static char DataRecordValue(void)
{
...
static int DataRecordContinue(YYSTYPE* yylval, ...)
{
    ...
    switch (drs->state)
    {
        ...
        case DRS_DONE:
            ...
            {
                yywarning(
                    "extra fields in initializer for %i", drs->field->name);
            }
 ...
 *
 ***********************************************************************/
static int DataRecordStart(YYSTYPE* yylval, ...)
{
    DataRecordState* drs = (DataRecordState*)top;
    SymbolPtr sym;
 ...
 *
 ***********************************************************************/
static char DataFieldInput(void)
{
 ...
 *
 ***********************************************************************/
static int DataFieldStart(YYSTYPE* yylval, ...)
{
    yylex = yystdlex;
 ...
 *
 ***********************************************************************/
static int DataEnterStruct(TypePtr type, Expr* expr, int maxElts)
{
    /*
        ...
        else if (type->tn_u.tn_struct->u.sType.first)
        {
                 ...
                 field = field->u.eltsym.next)
            {
                ...
                if (field->name != NullID)
                {
                     ...
                     */
                    cp = DataSkipInitializer(
                        &init, field->name, cur.file, cur.line, &blank, &save);
                    if (cp == (char*)NULL)
                    {
                        /*
                ...
                if (blank)
                {
                    ...
                    if (type->tn_type == TYPE_ARRAY)
                    {
                        ...
                        retval = !(
                            !Data_Enter(
                                &dot, type->tn_u.tn_array.tn_base, value, 0) ||
                            !retval);
                    }
                    else
                    {
                        retval =
                            !(!Data_Enter(&dot, type, value, 0) || !retval);
                    }
                }
                else
                {
                    ...
                    if (DataParse((DataState*)&dss, DataFieldStart, yyinput))
                    {
                        ...
                        if (undefined > 0)
                        {
                             ...
                             */
                            Table_StoreZeroes(
                                curSeg->u.segment.code, undefined, dot);
                        }
                        else if (undefined < 0)
                        {
                            /*
    ...
    {
        Notify(
            NOTIFY_ERROR, expr->file, expr->line, "too many elements in array");
        retval = FALSE;
    }
    return (retval);
}
 ...
 *
 ***********************************************************************/
static int DataEnterUnion(TypePtr type, Expr* expr, int maxElts)
{
    SymbolPtr field;
    Expr cur; /* Current structure */
    DataStructState dss;
            ...
            if (cp != fname)
            {
                ...
                else
                {
                    ...
                    if (DataParse((DataState*)&dss, DataFieldStart, yyinput))
                    {
                         ...
                         */
                        undefined =
                            Type_Size(field->u.field.type) - (dot - startDot);
 ...
 *
 ***********************************************************************/
static int DataParse(DataState* state, LexProc* newyylex, InputProc* newyyinput)
{
...
#define DESD_CHUNK_ONLY_MASK               \
    0x00000008 /* Only a chunk symbol or 0 \
                * will do */
#define DESD_CHUNK_ONLY_SHIFT 3
#define DESD_FIX_TYPE_MASK                  \
    0x000000f0 /* Type of external fixup to \
                * register */
 ...
 *
 ***********************************************************************/
static FixResult DataEnterSingle(
    ...
    Opaque data)  /* Special stuff */
{
    ...
    if (!Expr_Eval(expr1,
            ...
            ((pass > 1) ? EXPR_NOUNDEF : 0) |
                ...
                (DESD_FIX_TYPE(data) ? EXPR_DATA_ENTER : 0) | EXPR_NOT_OPERAND,
            &status))
    {
        /*
         * Error in piece -- print error message.
         */
        Notify(NOTIFY_ERROR, expr1->file, expr1->line, (char*)result.type);
        retcode = FR_ERROR;
    }
    else if (((status & EXPR_STAT_DEFINED) == 0) || (status & EXPR_STAT_DELAY))
    {
        /*
         ...
         */
        Table_Store(curSeg->u.segment.code, size, val, *addrPtr);
        *addrPtr += size;
    }
    else if (result.type == EXPR_TYPE_STRING)
    {
        ...
        else if (size == 1 || (size == 2 && dbcsRelease))
        {
            ...
            else
            {
                ...
                {
                    Table_Store(
                        curSeg->u.segment.code, len, result.data.str, *addrPtr);
                    *addrPtr += len;
                }
                else
                {
                    ...
                    for (i = 0; i < len; i++)
                    {
                        ...
                        dbcs[1] = '\0';
                        Table_Store(curSeg->u.segment.code, 2, &dbcs, *addrPtr);
                        *addrPtr += 2;
                    }
                }
            }
        ...
        else
        {
            ...
            if (len > size)
            {
                Notify(NOTIFY_WARNING,
                    expr1->file,
                    expr1->line,
                    "string constant '%s' longer than data element (%d bytes) "
                    "-- truncating",
                    result.data.str,
                    size);
                len = size;
    ...
    else if (result.type == EXPR_TYPE_CONST)
    {
         ...
         * 1.
         */
        if ((size != 4) && (result.data.number & ~mask) &&
         ...
         */
        for (i = size, v = result.data.number, bp = val; i > 0;
             v >>= 8, i--, bp++)
        {
            *bp = (byte)v;
        }
        ...
        if (result.rel.sym)
        {
            ...
            if ((size == 4) && ((DESD_FIX_TYPE(data) == FIX_HANDLE) ||
    ...
    else
    {
        switch (size)
        {
            case 1:
            {
                ...
                Table_Store(curSeg->u.segment.code, 1, &b, *addrPtr);
                Fix_Enter(&result, *addrPtr, *addrPtr);
                break;
            }
            case 2:
            {
                ...
                result.rel.size = FIX_SIZE_WORD;
                result.rel.type =
                    DESD_FIX_TYPE(data) ? DESD_FIX_TYPE(data) : FIX_OFFSET;
                ...
                Table_Store(curSeg->u.segment.code, 2, val, *addrPtr);
            ...
            case 4:
            {
                ...
                Table_Store(curSeg->u.segment.code, 4, val, *addrPtr);

                /*
                 * First the segment relocation
                 */
                fix.size = FIX_SIZE_WORD;
                fix.type = (DESD_FIX_TYPE(data)
                                ? DESD_FIX_TYPE(data)
                                : (DESD_CHUNK_ONLY(data) ? FIX_HANDLE
                                                         : FIX_SEGMENT));
                fix.pcrel = 0;
                fix.fixed = 0;
            ...
            default:
                Notify(NOTIFY_ERROR,
                    expr1->file,
                    expr1->line,
                    "Unhandled size (%d) in DataEnterSingle",
                    size);
                retcode = FR_ERROR;
                break;
        }
 ...
 *
 ***********************************************************************/
static int DataEnterInt(int* addrPtr, /* IN/OUT: address at which to store */
                                       ...
                                       * (if not FIX_TYPE_OFFSET) */
{
    ...
    if (cur.numElts != 0)
    {
        Notify(NOTIFY_ERROR,
            cur.file,
            cur.line,
            "too many elements in array (%d max)",
            initMax);
        return (FALSE);
    }
    return (retval);
 ...
 *
 ***********************************************************************/
int Data_Enter(int* addrPtr, /* IN/OUT: address at which to store */
                              ...
                              * be in expr */
{
        ...
        curSeg->u.segment.data->comb == SEG_LIBRARY)
    {
        Notify(NOTIFY_ERROR,
            expr->file,
            expr->line,
            "Data in global/library segment %i",
            curSeg->name);
        return (FALSE);
    }
    else if (warn_inline_data && (!curSeg->u.segment.data->checkLabel ||
...
again: /* Return point for handling SYM_TYPE */
    ...
    switch (type->tn_type)
    {
        ...
        case TYPE_CHAR:
            return DataEnterInt(
                addrPtr, Type_Int(Type_Size(type)), expr, maxElts, FALSE, 0);
        case TYPE_PTR:
            switch (type->tn_u.tn_ptr.tn_ptrtype)
            {
                ...
                case TYPE_PTR_FAR: /* FIX_SEGMENT for high word if non-zero */
                    return DataEnterInt(
                        addrPtr, type, expr, maxElts, FALSE, FIX_SEGMENT);
                ...
                case TYPE_PTR_HANDLE: /* FIX_HANDLE if non-zero */
                    return DataEnterInt(
                        addrPtr, type, expr, maxElts, FALSE, FIX_HANDLE);
                case TYPE_PTR_OBJ: /* FIX_HANDLE if non-zero + offset=chunk */
                    return DataEnterInt(
                        addrPtr, type, expr, maxElts, TRUE, FIX_HANDLE);
                case TYPE_PTR_VM: /* High word should be 0 */
                    return DataEnterInt(
                        addrPtr, type, expr, maxElts, FALSE, FIX_BOGUS);
            }
        ...
        case TYPE_ARRAY:
        {
             ...
             * be done with simple expressions, as above. If so, we recurse,
             * then pad any extra space with 0's. Otherwise, we lose.
             *
            ...
            if (expr->elts[0].op != EXPR_INIT)
            {
                switch (type->tn_u.tn_array.tn_base->tn_type)
                {
                    ...
                    case TYPE_CHAR:
                    case TYPE_PTR:
                        return Data_Enter(addrPtr,
                            type->tn_u.tn_array.tn_base,
                            expr,
                            type->tn_u.tn_array.tn_length);
                }
            }
        ...
        case TYPE_STRUCT:
            switch (type->tn_u.tn_struct->type)
            {
                ...
                case SYM_RECORD:
                {
                    ...
                    do
                    {
                        ...
                        if (state.common.cur.elts[0].op != EXPR_INIT)
                        {
                             ...
                             * a string or with a real expression. In the case
                             * of a real expression, use DataEnterInt, which
                             * will make sure the expression isn't too big.
                             */
                            DataEnterInt(
                                addrPtr, type, &state.common.cur, 1, FALSE, 0);
                        }
                ...
                case SYM_TYPE:
                    /*
                     * Switch to the type recorded in the typedef and try
                     * again... Why recurse when you don't have to?
                     */
 ...
 *
 ***********************************************************************/
void Data_EncodeRecord(SymbolPtr record, /* Record involved */
