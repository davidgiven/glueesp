...
#define PARAM_SIZE                                     \
    32 /* Number of entries for macro dummy parameters \
    ...
    switch (c)
    {
        case '%':
        {
            ...
            if (NextID(&pb))
            {
                ...
                sym = Sym_Find(
                    ST_EnterNoLen(output, symStrings, yytext), SYM_ANY, FALSE);
                if (sym && sym->type == SYM_STRING)
                {
                    ...
                    DBPRINTF((stderr,
                        "returning macarg(%s) from equate\n",
                        yylval->string));
        ...
        case '\\':
            ...
            {
                /*
                 * Anything else and we return the backslash unharmed, placing
                 * it at the start of the macro argument.
                 */
                unput(c);
                c = '\\';
            }
...
static int ScanStruct(YYSTYPE* yylval) /* Place to store result */
{
    ...
    while (1)
    {
        ...
        switch (c)
        {
            case 0:
                yyerror(
                    "end-of-file in initializer (began on line %d)\n", start);
                return (0);
            case '<':
                /*
            ...
            case '\n':
                ...
                {
                    yywarning(
                        "%c-terminated string constant terminated by newline",
                        close);
                    c = close;
                    close = 0;
                    if (!inmacro)
            ...
            case '"':
                 ...
                 * If we're looking for a close quote and this is it, just
                 * reset "close" to 0. If it's a duplicate that's to be one,
                 * we'll just re-open the case when the duplicate comes in next,
            ...
            case '\\':
                ...
                *cp++ = c;
                ...
                if (!close && !level && c == '>')
                {
                     ...
                     * encountered an escaped close angle -- swallow the
                     * backslash but include the close-angle in the string.
                     */
                    cp--;
                }
                else if (c == '\n')
                {
                    /*
                     * Swallow both the \ and the newline when newline is
                     * escaped like this.
                     */
                    cp--;
                    c = input();
                ...
                else
                {
                     ...
                     */
                    yywarning(
                        "not molesting \\%c escape inside structure "
                        "initializer; is this what you expect?",
                        c);
                }
                break;
            case ';':
    ...
    switch (c)
    {
        ...
        skip_comment:
            ...
            else
            {
                ...
                {
                    /*
                     * Handle continuation at end of comment by going back to
                     * get another token w/o returning the newline.
                     */
                    if (c == '\\')
                    {
        ...
        newline_thing:
            ...
            return (c == '|' ? '\n' : c);
        case '<':
            return defStruct ? ScanStruct(yylval)
                             : yyreadstring('<', '>', 0, yylval);
        case '\'':
            ...
            if (!baseSet)
            {
                switch (c)
                {
                    ...
                    default:
                         ...
                         * look for binary or decimal radix char as trailer.
                         * They will be in the digits read as they are valid hex
                         * characters.
                         */
                        unput(c);
                        if (cp != yytext)
            ...
            *cp = '\0';
            ...
            if ((opp != (const OpCode*)NULL) && !noSymTrans)
            {
                ...
                switch (opp->token)
                {
                    ...
                    case EXITM:
                        /*
                         * Exit the current macro call by popping the macro
                         * state (if any) and going back for another token.
                         */
                        if (!ignore)
                        {
                            ...
                            {
                                /*
                                 * Discard any pushback from after the EXITM
                                 * before popping the macro state. NOTE: set
                                 * iflevel only here, not in PopMacro, to deal
                                 * with conditionals inside macro arguments,
                                 * where the conditional is seen, iflevel is
                                 * set, then the end of the argument is reached
                                 * and iflevel gets reset by PopMacro, as was
                                 * happening before.
                                 * -- ardeb 3/18/92
                                 */
                                yysptr = yysbot;
                    ...
                    case INSTR:
                         ...
                         * place of a string. In fact, they need to else the
                         * user will get confused, since the equate doesn't have
                         * the bounding <>'s and won't be returned as a STRING
                         * otherwise...
                         */
                        noEquate = TRUE;
                        break;
                    ...
                    case ERRNB:
                         ...
                         * a string equate to take the place of a <>-enclosed
                         * string as one of its arguments (it's just good
                         * sense...). To accomplish this, we set noEquate to
                         * TRUE so the handling of SYM_STRING below doesn't
                    ...
                    case ERRDEF:
                         ...
                         * translated since we don't want what it means but
                         * rather the identifier itself. IRP and IRPC
                         * essentially want just an identifier. Rather than
                         * write a rule to handle the use of any type of symbol,
                         * as we'd have to do if we left symbol translation on,
                         * we just turn it off for them and return the
                    ...
                    case MACRO:
                        /*
                         * Read to the end of the definition -- yyreadmacro
                         * deals with "ignore" state.
                         */
                        yyreadmacro(yylval);
                        break;
                    case EXITF:
                        /*
                         * Exit current input file -- get out of the current
                         * macro.
                         */
                        while (curBlock != NULL)
                        {
            ...
            else if (!noSymTrans)
            {
                ...
                if (sym != (Symbol*)NULL)
                {
                    ...
                    switch (sym->type)
                    {
                        ...
                        {
                            /*
                             * To handle the redefinition of string constants,
                             * we need to look ahead in the stream to see if the
                             * next token might be EQU. If it is, we push the
                             * token back and just return a SYM token. Else we
                             * push to the constant's value and start over.
                             */
                            if (noEquate)
                            {
                                yylval->string = ScanEquateToString(sym);
                                DBPRINTF((stderr,
                                    "Returning string from %i <%s>\n",
                                    sym->name,
                                    yylval->string));
                                return (STRING);
                            }
                            ...
                            {
                                /*
                                 * Followed by EQU, CATSTR or SUBSTR -- return
                                 * SYM for redefinition
                                 */
                                noEquate = TRUE;
                                return SYM;
                            }
                            else
                            {
                                ...
                                {
                                    /*
                                     * If ID name ended in &, throw away the &.
                                     * Again, this is to deal with the MASM
                        ...
                        case SYM_MACRO:
                            ...
                            {
                                /*
                                 * Invoking a macro -- switch into special
                                 * macro- processing mode and tell the parser
                                 * what it's got.
                                 */
                        ...
                        case SYM_STRUCT:
                             ...
                             * structured-type symbols to make life easier and
                             * cleaner.
                             */
                            DBPRINTF((stderr,
                                "Returning STRUCT_SYM(%i)\n",
                                sym->name));
                            defStruct = TRUE;
                            return STRUCT_SYM;
                        case SYM_SEGMENT:
                        case SYM_GROUP:
                            DBPRINTF((stderr,
                                "Returning MODULE_SYM(%i)\n",
                                sym->name));
                            return MODULE_SYM;
                        case SYM_TYPE:
                            DBPRINTF((
                                stderr, "Returning TYPE_SYM(%i)\n", sym->name));
                            defStruct = TRUE;
                            return TYPE_SYM;
                        case SYM_METHOD:
                            DBPRINTF((stderr,
                                "Returning METHOD_SYM(%i)\n",
                                sym->name));
                            return METHOD_SYM;
                        case SYM_INSTVAR:
                            DBPRINTF((stderr,
                                "Returning INSTVAR_SYM(%i)\n",
                                sym->name));
                            return INSTVAR_SYM;
                        case SYM_PROTOMINOR:
                            DBPRINTF((stderr,
                                "Returning PROTOMINOR_SYM(%i)\n",
                                sym->name));
                            return PROTOMINOR_SYM;
                        case SYM_CLASS:
                            DBPRINTF((stderr,
                                "Returning CLASS_SYM(%i)\n",
                                sym->name));
                            return CLASS_SYM;
                        case SYM_RECORD:
                            DBPRINTF((stderr,
                                "Returning RECORD_SYM(%i)\n",
                                sym->name));
                            defStruct = TRUE;
                            return RECORD_SYM;
                        case SYM_NUMBER:
                            DBPRINTF((
                                stderr, "Returning EXPR_SYM(%i)\n", sym->name));
                            return (EXPR_SYM);
                        default:
                            /*
                             * Not an opcode, but it's defined -- tell the
                             * parser it's got a defined symbol on its hands.
                             */
                            DBPRINTF(
                                (stderr, "Returning SYM(%i)\n", sym->name));
                            return SYM;
                    }
                }
                else
            ...
            {
                yylval->ident =
                    ST_Enter(output, symStrings, yytext, cp - yytext);
            }
    ...
    YYSTYPE* yylval)               /* Place to store result */
{
    ...
    DBPRINTF(
        (stderr, "reading %c%c string literal...", open ? open : close, close));
    cp = base;
    while (1)
    {
        c = input();
        if (c == 0)
        {
            ...
            {
                yyerror(
                    "end-of-file in string constant (began on line %d)", start);
                return (0);
            }
            else
            {
        ...
        else if (c == '\\')
        {
            ...
            switch (c = input())
            {
                ...
                case '9':
                {
                    ...
                    for (val = c - '0'; isdigit(c = input()) && c < '8';
                         val += c - '0')
                    {
                        val <<= 3;
                    }
...
int yystdwrap(void)
{
    ...
    if (iflevel != curFile->iflevel)
    {
        ...
        {
            Notify(
                NOTIFY_ERROR, curFile->name, -1, "%d too many endifs", -diff);
        }
        else
        {
            Notify(NOTIFY_ERROR,
    ...
    Arg* args)                   /* Arguments to pass */
{
    ...
    if (numMaps != 0)
    {
         ...
         */
        for (i = 0; i < which->u.macro.numArgs && ap != (Arg*)NULL;
             ap = ap->next, i++)
        {
            /*
     ...
     */
    while ((curBlock != NULL) && (curMacCount <= 0) &&
...
static MBlk* yyreadmacrobody(
    char*** paramsPtr, /* Array of words to be mapped when macro
    ...
    while (1)
    {
        ...
        scan_id:
            ...
            *cp2 = '\0'; /* null terminate */
            ...
            {
                if (!instring && ((ustrcmp(word, "macro") == 0) ||
                                     (ustrcmp(word, "rept") == 0) ||
                                     (ustrcmp(word, "irp") == 0) ||
                                     (ustrcmp(word, "irpc") == 0)))
                {
    ...
    fetch_param:
        ...
        if (isfirstid(c))
        {
            ...
            *cp++ = '\0';
            ...
            {
                params = (char**)realloc(
                    (void*)params, (maxParams + PARAM_SIZE) * sizeof(char*));
                maxParams += PARAM_SIZE;
            }
            /*
             * Copy the identifier into more permanent storage on
...
readezle:
    ...
    yylval->macro.text =
        yyreadmacrobody(&params, maxParams, &numLocals, startLine);
    ...
    while (1)
    {
        ...
        while (!isfirstid(c = input()) && (c != 0))
        {
            if (c == ';')
            {
                ...
                {
                    yyerror(
                        "end-of-file in false conditional starting on line %d",
                        startLine);
                    opp = NULL;
                    break;
                }
        ...
        if (isfirstid(c))
        {
            ...
            *cp2 = '\0'; /* null terminate */
			...
			yynewline();
                ...
                else if (!nesting && (opp->token == ELSE || opp->value == 1) &&
                         orElse)
                {
                    /*
