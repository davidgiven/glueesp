 ...
 *
 ***********************************************************************/
void Fix_Register(FixClass class, /* Class of fixup */
    FixProc* func,                /* Function to be called */
...
static int FixDoFixups(SymbolPtr sym, /* Segment to fix up */
    ...
    int pass)                         /* Which pass we're on */
{
    ...
    for (fix = head->next; fix != head; fix = next)
    {
        ...
        if (fix->class == fclass)
        {
            ...
            res = (*fix->func)(
                &addr, fix->size, pass, fix->expr1, fix->expr2, fix->data);
            ...
            if (res != FR_ERROR && (addr - fix->addr) != fix->size)
            {
                ...
                for (tefix = efix; tefix != ehead; tefix = tefixNext)
                {
                     ...
                     */
                    if ((diff < 0) && ((tefix->addr >= fixNewEnd) &&
 ...
 *
 ***********************************************************************/
static int FixPass2(SymbolPtr sym, /* Segment to fix up */
 ...
 *
 ***********************************************************************/
static int FixPass3(SymbolPtr sym, /* Segment to fix up */
    Opaque data)                   /* Place to store result */
{
 ...
 *
 ***********************************************************************/
static int FixPass4(SymbolPtr sym, /* Segment to fix up */
    Opaque data)                   /* Place to store error */
{
    Fixup* fix;    /* Fixup being performed */
    Fixup* next;   /* Next fixup to process */
    ...
    for (fix = head->next; fix != head; fix = next)
    {
        ...
        res = (*fix->func)(
            &addr, fix->size, 4, fix->expr1, fix->expr2, fix->data);
 ...
 *
 ***********************************************************************/
void Fix_Adjust(SymbolPtr sym, /* Segment needing adjustment */
    ...
    int diff)                  /* Amount by which to adjust the fixups */
{
     ...
     */
    for (efix = ehead->next; efix != ehead && efix->addr < addr;
         efix = efix->next)
    {
        ;
    }
 ...
 *
 ***********************************************************************/
void Fix_EnterSegment(SymbolPtr seg, FExprResult result, int addr, int ref)
{
    ExtFix* efix;
    ExtFix* enew;
    ExtFix* ehead;
    ...
    for (efix = ehead->prev; efix != ehead && efix->addr > addr;
         efix = efix->prev)
    {
        ;
    }
 ...
 *
 ***********************************************************************/
int Fix_OutputRel(SymbolPtr seg, /* Segment in which relocation is
                                  * occurring */
    ...
    switch (desc->type)
    {
        ...
        case FIX_OFFSET:
            if ((desc->sym->flags & SYM_NOWRITE) ||
                ...
                (desc->pcrel && !(desc->sym->flags & SYM_UNDEF) &&
...
VMBlockHandle Fix_Write(SymbolPtr sym) /* Segment whose fixups want writing */
{
    ...
    while (estart != ehead)
    {
         ...
         */
        for (i = 0, efix = estart; efix != ehead && i < FIX_MAX;
             efix = efix->next)
        {
         ...
         */
        block = VMAlloc(
            output, i * sizeof(ObjRel) + sizeof(ObjRelHeader), OID_REL_BLOCK);
 ...
 *
 ***********************************************************************/
int Fix_Find(FixProc* func, /* Callback of the fixup */
