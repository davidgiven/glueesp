...
extern void Obj_DefineClass(SymbolPtr class, Expr* flags, Expr* initRoutine);
extern SymbolPtr Obj_DeclareClass(
    ID className, SymbolPtr superClass, int flags);
...
extern void Obj_EnterDefault(
    SymbolPtr class, SymbolPtr handler, Expr* expr, int callType);
extern void Obj_EnterReloc(SymbolPtr class, SymbolPtr handler, Expr* expr);
extern void Obj_NoReloc(SymbolPtr class, SymbolPtr varData, Expr* expr);
extern void Obj_ExportMessages(SymbolPtr class, ID rangeName, Expr* length);
extern void Obj_CheckVarDataBounds(SymbolPtr class);
extern void Obj_CheckMessageBounds(SymbolPtr class);

#define OBJ_DYNAMIC                            \
    0 /* Method may only be called dynamically \
       * via a message */
#define OBJ_DYNAMIC_CALLABLE                                \
    1                /* Method may be called dynamically or \
                      * by a direct call or jump, but not   \
                      * staticly */
#define OBJ_STATIC 2 /* Method may always be called staticly */
#define OBJ_PRIVSTATIC                                               \
    3                     /* Method may only be called staticly from \
                           * within the defining geode */
#define OBJ_STATIC_MASK 3 /* Bits that indicate static/dynamic */

#define OBJ_EXTERN                               \
    4 /* Set if method is external, meaning the  \
...
extern void Obj_DeclareStateVar(
    ID name, SymbolPtr class, TypePtr type, Expr* value);
#endif
