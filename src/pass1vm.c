 ...
 *
 ***********************************************************************/
static VMBlockHandle Pass1VMDupTypeBlock(VMHandle fh, /* Object file */
 ...
 *
 ***********************************************************************/
static int Pass1VMRelAddrSyms(const char* file, /* File name */
 ...
 *
 ***********************************************************************/
static void Pass1VMCountRels(const char* file, /* Object file being loaded */
    VMHandle fh,                               /* Handle of same */
        ...
        for (rel = (ObjRel*)(orh + 1); n > 0; rel++, n--)
        {
            switch (rel->type)
            {
                ...
                case OREL_SEGMENT:
                {
                     ...
                     */
                    SegDesc* frame =
                        Obj_FindFrameVM((char*)file, fh, hdr, rel->frame);
 ...
 * *********************************************************************/
static int Pass1VMSearchAddrSyms(const char* file, /* File name */
    VMHandle fh,                                   /* Object file handle */
    ObjSegment* s) /* Segment descriptor in obj */
                   /* file*/
{
 ...
 *
 *********************************************************************/
static Boolean Pass1VMSearchForUndefinedSymbol(const char* file, /* File name */
    VMHandle fh,             /* Object file handle */
...
Boolean Pass1VM_FileIsNeeded(const char* file, /* file for errors */
 ...
 *
 ***********************************************************************/
void Pass1VM_Load(const char* file, /* File name (for error messages) */
    genptr handle)                  /* File handle for access */
{
     ...
     * absolute.
     */
    for (i = hdr->numSeg, s = (ObjSegment*)(hdr + 1), j = 0; i > 0;
         i--, s++, j++)
    {
        ...
        segs[j] = sd = Seg_AddSegment((char*)file,
            ...
            s->align,
            s->flags);
        if (sd && (sd->combine == SEG_LMEM) && (sd->size != 0) &&
            (s->data != 0))
        {
            Notify(NOTIFY_ERROR,
     ...
     */
    for (i = hdr->numGrp, g = (ObjGroup*)s; i > 0; i--, g = OBJ_NEXT_GROUP(g))
    {
        GroupDesc* gd =
            Seg_AddGroup((char*)file, ST_Dup(fh, g->name, symbols, strings));
    ...
    for (i = hdr->numSeg, s = (ObjSegment*)(hdr + 1); i > 0; i--, s++)
    {
         ...
         */
        if ((s->syms != 0) &&
            !Pass1VMRelAddrSyms(file, fh, s, sd, reloc, &next))
        {
            continue;
        }
         ...
         */
        if ((s->syms != 0) && (next != 0) &&
            !Obj_EnterTypeSyms(file, fh, sd, next, 0))
        {
            continue;
        }
