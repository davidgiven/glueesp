 ...
 *
 ***********************************************************************/
static int Pass2VMCheckFixed(SegDesc* sd, /* Current segment */
    ...
    if (rel->fixed)
    {
        if (!(*fileOps->checkFixed)(frame) ||
            (sym && (frame->type != S_GROUP) &&
                (frame->combine == SEG_LIBRARY) && (sym->flags & OSYM_MOVABLE)))
        {
            Pass2_RelocError(sd,
                rel->offset,
 ...
 *
 ***********************************************************************/
static int Pass2VMRelOff(SegDesc* sd, /* Segment in which relocation is
                                       * to occur */
                                       ...
                                       * in which the symbol resides */
{
    ...
    if (rel->symBlock == 0)
    {
        ...
        *relocPtr = sd->nextOff - sd->foff;
        ...
        {
            pass2_assert(
                (sd->group && sd->group->name == frame->name), sd, rel->offset);
            /*
            ...
            if (frameName != NullID)
            {
                if ((frame->type != S_GROUP) && (frame->combine == SEG_LIBRARY))
                {
                    ...
                    else
                    {
                        Pass2_RelocError(sd,
                            rel->offset,
                            "%i undefined in library %i, perhaps you forgot to "
                            "load it?",
                            id,
                            frameName);
                    }
                ...
                {
                    Pass2_RelocError(
                        sd, rel->offset, "%i undefined in %i", id, frameName);
                }
            }
            else
            {
    ...
    *symOffPtr = symOff;
    ...
    if (((rel->type == OREL_CALL) ||
        ...
        (sym->type == OSYM_PROC) && (sym->u.proc.flags & OSYM_NO_CALL))
    {
        Pass2_RelocError(
            sd, rel->offset, "procedure %i may not be called", sym->name);
        VMUnlockDirty(symbols, symBlock);
        VMUnlock(fh, rel->symBlock);
        return (0);
    }
 ...
 *
 ***********************************************************************/
static void Pass2VMHandleEntryRel(SegDesc* sd, /* Segment being operated on */
                                                ...
                                                * working */
{
    ...
    {
        Pass2_RelocError(sd, rel->offset, "%i not in %i", id, frame->name);
        return;
    }
    ...
    else
    {
        ...
        {
            Pass2_RelocError(
                sd, rel->offset, "%i not exported in geode's .gp file", id);
            entry = 0;
        }
    }
 ...
 *
 ***********************************************************************/
static int Pass2VMHandleMethCall(SegDesc* sd, /* Segment being operated on */
    void* data,                               /* Base of data block */
    const char* file,                         /* Object file name */
    VMHandle fh,                              /* VM handle open to same */
    ObjHeader* hdr,                           /* Header from same */
                                               ...
                                               * need to call maprel */
{
    ...
    {
        Pass2_RelocError(
            sd, rel->offset, "output format can't handle static method calls");
        return (0);
    }
     ...
     */
    if ((sym->type != OSYM_CLASS) && (sym->type != OSYM_MASTER_CLASS) &&
    ...
    while (1)
    {
         ...
         */
        if (!Sym_Find(
                symbols, (VMBlockHandle)0, super, &symBlock, &symOff, TRUE))
        {
            Pass2_RelocError(sd,
                rel->offset,
        ...
        if ((csym->type != OSYM_CLASS) && (csym->type != OSYM_MASTER_CLASS) &&
    ...
    if ((bsym != NULL) && (bsym->u.binding.callType >= OSYM_STATIC))
    {
         ...
         */
        if (!Sym_Find(
                symbols, (VMBlockHandle)0, proc, &psymBlock, &psymOff, FALSE))
        {
            proc = NullID;
        }
    }
    ...
    if (proc == NullID)
    {
        ...
        {
            proc = ST_LookupNoLen(
                symbols, strings, altProcs[rel->type - OREL_METHCALL].name);
            if (proc == NullID)
            {
                Pass2_RelocError(sd,
                    rel->offset,
    ...
    {
        reloc =
            psym->u.proc.address - (rel->offset + (sd->nextOff - sd->foff) + 3);
    ...
    else
    {
        *bp = psym->u.proc.address;
        ...
        if ((*fileOps->maprel)(
                OREL_CALL, frame, nextRel, sd, rel->offset, (word*)bp))
        {
            return (1);
        }
    }
 ...
 *
 ***********************************************************************/
static int Pass2VMHandleOffRel(SegDesc* sd, /* Segment being operated on */
    void* data,                             /* Base of data block */
    const char* file,                       /* Object file name */
    VMHandle fh,                            /* VM handle open to same */
    ObjHeader* hdr,                         /* Header from same */
                                             ...
                                             * need to call maprel */
{
    ...
    switch (rel->size)
    {
        case OREL_SIZE_BYTE:
            ...
            break;
        case OREL_SIZE_WORD:
            reloc += *bp | (bp[1] << 8) | (((bp[1] & 0x80) ? -1 : 0) & ~0xffff);
        ...
        case OREL_SIZE_DWORD:
            reloc += *bp | (bp[1] << 8) | (bp[2] << 16) | (bp[3] << 24);
        ...
        if (symBlock == 0)
        {
            if (fileOps->flags & FILE_PROTO_RELS)
            {
                Out_RegisterFinalProtoRel(
                    FALSE, sd->nextOff + rel->offset, frame);
            }
            else
            {
                Pass2_RelocError(sd,
                    rel->offset,
                    "protocol number relocations not supported by output "
                    "format");
            }
        }
        else
    ...
    else if (rel->type == OREL_HIGH)
    {
        if (symBlock == 0)
        {
            ...
            {
                Out_RegisterFinalProtoRel(
                    TRUE, sd->nextOff + rel->offset, frame);
            }
            else
            {
                Pass2_RelocError(sd,
    ...
    {
        if ((*fileOps->maprel)(OREL_SEGMENT,
                frame,
                nextRel,
                sd,
                rel->offset + 2,
                (word*)((genptr)data + rel->offset + 2)))
        {
            return (1);
             ...
             ((rel->type == OREL_OFFSET) && (frame->type != S_GROUP) &&
 ...
 *
 ***********************************************************************/
static int Pass2VMHandleSegRel(SegDesc* sd, /* Segment being operated on */
    void* data,                             /* Base of data block */
    const char* file,                       /* Object file name */
    VMHandle fh,                            /* VM handle open to same */
    ObjHeader* hdr,                         /* Header from same */
 ...
 *
 ***********************************************************************/
void Pass2VM_Load(const char* file, /* File name (for error messages) */
    genptr handle)                  /* File handle for access */
{
        ...
        for (block = s->syms; block != 0; block = next)
        {
            ...
            for (sym = (ObjSym*)(symHdr + 1); n > 0; sym++, n--)
            {
                if (sym->type == OSYM_PROC)
                {
                    ...
                    if (Sym_FindWithSegment(symbols,
                            symID,
                            &symBlock,
                            &symOff,
                            TRUE,
                        ...
                        if (symSym->u.proc.flags & OSYM_PROC_PUBLISHED)
                        {
                            ...
                            if (symsLeft == 0)
                            {
                                ...
                                for (nextBlock = osh->next; nextBlock != 0;
                                     nextBlock = nextNext)
                                {
                                    nextHdr = (ObjSymHeader*)VMLock(
                                        symbols, nextBlock, (MemHandle*)NULL);
            ...
            for (rel = (ObjRel*)(orh + 1); n > 0; n--, rel++)
            {
                ...
                switch (rel->type)
                {
                    case OREL_PROTOMINOR:
                        (void)Pass2VMHandleOffRel(
                            sd, data, file, fh, hdr, rel, nextRel);
                        uprel = 1;
                        break;
                    case OREL_CALL:
                    case OREL_OFFSET:
                    ...
                    case OREL_ENTRY:
                        ...
                        {
                            Pass2VMHandleEntryRel(sd, data, file, fh, hdr, rel);
                        }
                        break;
                    case OREL_SEGMENT:
                    case OREL_HANDLE:
                    case OREL_RESID:
                        uprel = Pass2VMHandleSegRel(
                            sd, data, file, fh, hdr, rel, nextRel);
                        break;
                    case OREL_METHCALL:
                    case OREL_SUPERCALL:
                        uprel = Pass2VMHandleMethCall(
                            sd, data, file, fh, hdr, rel, nextRel);
                        break;
                    default:
                        abort();
    ...
    if (hdr->entry.frame != 0)
    {
        if (entryGiven)
        {
            Notify(NOTIFY_ERROR, "%s: entry point already given", file);
        }
        else
        {
            entryGiven = TRUE;
