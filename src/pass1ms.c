...
#define MS_SYMS_NOT_ENTERED                          \
    1 /* Value stuck in osh->seg when we add symbols \
 ...
 *
 ***********************************************************************/
static void Pass1MSAddLMemSegment(SegDesc* sd)
{
 ...
 *
 ***********************************************************************/
static void Pass1MSRecordLMemHandles(SegDesc* sd,
 ...
 *
 ***********************************************************************/
static void Pass1MSCheckForObjectBlock(SegDesc* sd, /* Descriptor for header
                                                     * segment of the trio */
    byte rectype,                                   /* Type of data record */
    byte* bp,                                       /* Start of data */
    word startOff,                                  /* Offset w/in segment at
    ...
    word reclen)                                    /* Length of data record */
{
        ...
        startOff + datalen >= (offsetof(LMemBlockHeader, LMBH_lmemType) + 2))
    {
         ...
         */
        word w =
            bp[offsetof(LMemBlockHeader, LMBH_lmemType) - startOff] |
 ...
 *
 ***********************************************************************/
static void Pass1MSLayoutLMem(
    const char* file, SegDesc* sd) /* Descriptor for handles segment
                                    ...
                                    * object file */
{
     ...
     */
    heap->group->foff = lmd->handles->foff = heap->foff = heap->grpOff =
         ...
         sfp = (MSSaveFixupRec*)sfp->links.next)
    {
        ...
        {
            Notify(NOTIFY_ERROR,
                "%s: unable to prepare handle table for %i",
                file,
                lmd->handles->group->name);
            free((malloc_t)lmd);
            return;
        }
    }
 ...
 *
 ***********************************************************************/
static int Pass1MSCompareSyms(ObjSym* os1, ObjSym* os2)
{
        ...
        if (sd > MS_MIN_SEGMENT)
        {
            if (sd->lineMap)
            {
                ...
                oamh = (ObjAddrMapHeader*)VMLock(
                    symbols, sd->lineMap, (MemHandle*)NULL);
                oame =
                    ObjFirstEntry(oamh, ObjAddrMapEntry) + oamh->numEntries - 1;
            ...
            if (sd->addrT)
            {
                ...
                if (osh->seg == MS_SYMS_NOT_ENTERED)
                {
                    ...
                    qsort(base,
                        last - base,
                        sizeof(*base),
                        (CmpCallback)Pass1MSCompareSyms);
 ...
 *
 ***********************************************************************/
static void Pass1MSAddLines(SegDesc* sd, /* Segment to which to add the lines */
    ...
    word reclen)                         /* Bytes of data in the whole record */
{
    ...
    if (sd->lineT == 0)
    {
         ...
         */
        sd->lineH = sd->lineT =
            VMAlloc(symbols, OBJ_INIT_LINES, OID_LINE_BLOCK);
    ...
    while (reclen > 0)
    {
        ...
        spaceLeft =
            newSize - (olh->num * sizeof(ObjLine) + sizeof(ObjLineHeader));
        ...
        if (reclen > 0)
        {
             ...
             */
            next = olh->next = VMAlloc(symbols, OBJ_INIT_LINES, OID_LINE_BLOCK);
 ...
 *
 ***********************************************************************/
static void Pass1MSReplaceFileName(ID oldName, /* First record to add */
 ...
 *
 ***********************************************************************/
static int Pass1MSCountBlocks(word* iPtr, word offset, byte* data)
{
    word repCount;
    word blockCount;
 ...
 *
 ***********************************************************************/
unsigned Pass1MS_CountRels(const char* file, /* Object file being read */
    ...
    byte* data)                              /* Start of the enumerated data */
{
    ...
    while (bp < endRecord)
    {
        if (*bp & FLH_IS_FIXUP)
        {
            ...
            if (sd == NULL)
            {
                 ...
                 */
                Notify(NOTIFY_ERROR,
                    "%s: non-data FIXUPP record may only contain THREAD "
                    "definitions",
                    file);
                return (0);
            }
                    ...
                    if (!(fileOps->flags & FILE_NOCALL))
                    {
                        ...
                        if (!fixOffset)
                        {
                            *callArray++ = startOff - 1;
                            ...
                            {
                                sd->callArray = (word*)realloc(
                                    (malloc_t)sd->callArray,
    ...
    if (sd != NULL && data[reclen - 4] == 0x9a)
    {
        *callArray++ = startOff + reclen - 4;
        ...
        if ((size & CALL_ARRAY_MASK) == 0)
        {
            sd->callArray = (word*)realloc((malloc_t)sd->callArray,
                ((size + CALL_ARRAY_STEP) * sizeof(word)));
            callArray = sd->callArray + size;
            /*
 ...
 *
 ***********************************************************************/
static void Pass1MSProcessComdef(word reclen) /* Number of data bytes in the
                                               * whole record */
{
}
...
void Pass1MS_Load(const char* file, /* File name (for error messages) */
 ...
 *
 ***********************************************************************/
static void Pass1MSLMemSegCompareSwap(SegDesc** seg1, SegDesc** seg2)
{
    SegDesc* temp;
 ...
 *
 ***********************************************************************/
void Pass1MS_EnterExternal(ID name) /* Name of external to enter */
{
    ...
    for (i = 0; i < seg_NumSegs; i++)
    {
        ...
        if ((sd->combine == SEG_LIBRARY) &&
            (Sym_Find(symbols, sd->syms, name, &symBlock, &symOff, TRUE) ||
                ((name2 != NullID) &&
                    Sym_Find(
                        symbols, sd->syms, name2, &symBlock, &symOff, TRUE))))
        {
            ObjSym* os; /* External symbol */

            os = (ObjSym*)((genptr)VMLock(symbols, symBlock, (MemHandle*)NULL) +
                           symOff);
 ...
 *
 ***********************************************************************/
void Pass1MS_ProcessObject(const char* file, FILE* f)
{
    word reclen;
    byte rectype;
    int done = FALSE;
        ...
        switch (rectype)
        {
            ...
            case MO_COMENT:
                    ...
                    (bp[2] == '@'))
                {
                     ...
                     * GOC to tell us the actual source file name under
                     * metaware.
                     */
                    newName =
                        ST_Enter(symbols, strings, (char*)&bp[3], reclen - 3);
                    /*
                     *
                     */
                    if ((msobj_FirstFileName != NullID) &&
                        (newName != msobj_FirstFileName))
                    {
            ...
            case MO_PUBDEF:
            {
                ...
                if (*tail)
                {
                     ...
                     */
                    newSize =
                        (osh->num + i) * sizeof(ObjSym) + sizeof(ObjSymHeader);
                    ...
                    else
                    {
                        ...
                        symBlock = VMAlloc(symbols,
                            (sizeof(ObjSymHeader) + i * sizeof(ObjSym)),
                            OID_SYM_BLOCK);
                        osh->next = symBlock;
                        VMUnlockDirty(symbols, *tail);
                        *tail = symBlock;
                        osh = (ObjSymHeader*)VMLock(
                            symbols, symBlock, (MemHandle*)NULL);
                    }
                }
                else
                {
                    ...
                    *head = *tail = symBlock = VMAlloc(symbols,
                        (sizeof(ObjSymHeader) + i * sizeof(ObjSym)),
                        OID_SYM_BLOCK);
                    osh = (ObjSymHeader*)VMLock(
                        symbols, symBlock, (MemHandle*)NULL);
                     ...
                     */
                    osh->types =
                        VMAlloc(symbols, sizeof(ObjTypeHeader), OID_TYPE_BLOCK);
                    oth = (ObjTypeHeader*)VMLock(
                        symbols, osh->types, (MemHandle*)NULL);
                    oth->num = 0;
            ...
            case MO_SEGDEF:
            {
                ...
                if (sd)
                {
                    ...
                    sd->callArray =
                        (word*)malloc(CALL_ARRAY_STEP * sizeof(word));
                    memset(sd->callArray, 0, CALL_ARRAY_STEP * sizeof(word));
                    ...
                    if (sd->combine == SEG_LMEM)
                    {
                        ...
                        if (order == LMEM_HEADER)
                        {
                            ...
                            {
                                Notify(NOTIFY_ERROR,
                                    "%s: cannot have data in lmem segment %s "
                                    "in more than one object file",
                                    file,
                                    baseName);
                            }
                        }
            ...
            case MO_FIXUPP:
                ...
                (void)Pass1MS_CountRels(
                    file, rectype, (SegDesc*)NULL, 0, reclen, bp);
                break;
            case MO_LEDATA:
            case MO_LIDATA:
            {
                ...
                if (msobjBuf[reclen] == MO_FIXUPP)
                {
                     ...
                     * record. Yech.
                     */
                    sd->nrel += Pass1MS_CountRels(
                        file, rectype, sd, startOff, reclen, bp);
                }
            ...
            default:
                Notify(NOTIFY_ERROR,
                    "%s: unhandled object record type %02x",
                    file,
                    rectype);
                break;
        }
