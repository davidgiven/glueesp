 ...
 *
 ***********************************************************************/
static int ExePrepare(char* outfile, /* UNUSED */
    ...
    *bp = (cbase & 511) >> 8;
    ...
    for (i = 0; i < seg_NumGroups; i++)
    {
        ...
        for (j = 0; j < gd->numSegs; j++)
        {
            ...
            {
                Notify(NOTIFY_ERROR, "group %i spans more than 64K", gd->name);
                return (0);
            }
        }
    }
 ...
 *
 ***********************************************************************/
static int ExeReloc(int type, /* Relocation type (from obj file) */
                               ...
                               * PC byte-order */
{
    ...
    {
        Notify(NOTIFY_WARNING, "ExeReloc: Can only handle SEGMENT relocations");
        return (0);
    }
 ...
 *
 ***********************************************************************/
static void ExeSetEntry(SegDesc* sd, word off)
{
    byte* bp;
 ...
 *
 ***********************************************************************/
static void ExeWrite(void* base, /* Base of file buffer */
    ...
    char* outfile)               /* Name of output file */
{
    ...
    bcopy(
        copyright, (genptr)base + sizeof(exeHeader), (COPYRIGHT_SIZE + 1) & ~1);
     ...
     */
    for (csum = 0, i = len, wp = base; i > 1; csum += *wp++, i -= 2)
    {
        ;
    }
    if (i != 0)
 ...
 *
 ***********************************************************************/
static int ExeCheckFixed(SegDesc* targ)
{
