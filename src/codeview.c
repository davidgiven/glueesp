...
static word CVProcessTypeRecord(
    const char* file, byte** bpPtr, word len, VMBlockHandle typeBlock);
 ...
 *
 ***********************************************************************/
static Boolean CVLocatePublic(ID name, /* Name to find */
                                        ...
                                        * we found the thing. */
{
    ...
    for (srp = pubHead.next; srp != (MSSaveRec*)&pubHead; srp = srp->links.next)
    {
        bp = srp->data;
        end = bp + srp->len;
        ...
        while (bp < end)
        {
            ...
            if (((*bp == namelen) &&
                    (geosRelease >= 2
                            ...
                            : (ustrncmp((char*)bp + 1, namestr, namelen) ==
                                  0))) ||
                ((bp[1] == '_') && (*bp == namelen + 1) &&
 ...
 *
 ***********************************************************************/
static Boolean CVLocateFixup(const char* file, /* Object file being processed */
                                                ...
                                                * added to existing offset) */
{
     ...
     */
    for (sfp = (MSSaveFixupRec*)fixHead.prev; sfp != (MSSaveFixupRec*)&fixHead;
         sfp = (MSSaveFixupRec*)sfp->links.prev)
    {
        if ((sfp->startOff <= fixOff) && (sfp->endOff > fixOff))
        {
            ...
            while (bp < end)
            {
                if (!MSObj_DecodeFixup((char*)file,
                        CV_SYMS_SEGMENT,
                        &bp,
                        &fixLoc,
                        &fixData,
                ...
                if ((fixLoc & FL_OFFSET) == recOffset)
                {
                    ...
                    switch (fixData & FD_TARGET)
                    {
                        ...
                        case TFM_ABSOLUTE:
                            Notify(NOTIFY_ERROR,
                                "%s: unsupported codeview-symbol fixup target "
                                "%d",
                                file,
                                fixData & FD_TARGET);
                            return (FALSE);
 ...
 *
 ***********************************************************************/
static void CVAllocSymAndTypeBlocks(
    VMBlockHandle* symBlockPtr, VMBlockHandle* typeBlockPtr)
{
    ObjTypeHeader* oth;
    ObjSymHeader* osh;

    *typeBlockPtr = VMAlloc(
        symbols, sizeof(ObjTypeHeader) + 16 * sizeof(ObjType), OID_TYPE_BLOCK);
    oth = (ObjTypeHeader*)VMLock(symbols, *typeBlockPtr, (MemHandle*)NULL);
    oth->num = 0;
    VMUnlockDirty(symbols, *typeBlockPtr);

    *symBlockPtr = VMAlloc(
        symbols, sizeof(ObjSymHeader) + 16 * sizeof(ObjSym), OID_SYM_BLOCK);
 ...
 *
 ***********************************************************************/
static ID CVGetString(byte** bpPtr)
{
    ID retval;
 ...
 *
 ***********************************************************************/
static unsigned long CVGetInteger(byte** bpPtr)
{
    byte* bp;
 ...
 *
 ***********************************************************************/
static ObjSym* CVAllocSymLocked(
    VMBlockHandle symBlock, /* Block in which to alloc */
 ...
 *
 ***********************************************************************/
static ObjSym* CVAllocSym(VMBlockHandle symBlock, word* offsetPtr)
{
    MemHandle mem;
    ObjSymHeader* osh;
 ...
 *
 ***********************************************************************/
static byte* CVLocateType(word index, word* lenPtr)
{
    word len;
    byte* bp;
    byte* endTypes;
 ...
 *
 ***********************************************************************/
static word CVFinishStructuredType(
    const char* file,         /* Object file being read */
                               ...
                               * for this type */
{
    ...
    *dataBase++ = name & 0xff;
     ...
     */
    (void)Obj_EnterTypeSyms(
        file, symbols, globalSeg, tsymBlock, OETS_TOP_LEVEL_ONLY);
 ...
 *
 ***********************************************************************/
static void CVCreateTypedef(const char* file, /* Object file from which the
                                               * description came */
    ...
    word offset)                              /* Offset of type descriptor */
{
        ...
        ((*tagPtr)[1] != 0))
    {
        ...
        tsymBlock = VMAlloc(
            symbols, sizeof(ObjSymHeader) + sizeof(ObjSym), OID_SYM_BLOCK);
 ...
 *
 ***********************************************************************/
static Boolean CVLocateList(
    const char* file, byte** bpPtr, byte** basePtr, word* lenPtr)
{
    byte* bp = *bpPtr;
 ...
 *
 ***********************************************************************/
static word CVProcessStructure(
    const char* file, byte** bpPtr, word len, VMBlockHandle typeBlock)
{
    ...
    *bpPtr += len;
    ...
    if ((bp - dataBase < len) && (*bp == CTL_STRING) &&
        (strncmp((char*)bp + 2, "(untagged)", bp[1]) != 0) && (bp[1] != 0))
    {
        /*
    ...
    for (i = 0; i < nfields; i++)
    {
        types[i] = CVProcessTypeRecord(
            file, &tlistp, tlistp[1] | (tlistp[2] << 8), ttypeBlock);
            ...
            ((types[i] & OTYPE_BF_WIDTH) == 0))
        {
            ...
            {
                Notify(NOTIFY_ERROR,
                    "%s: invalid structure descriptor (no bitfield descriptor "
                    "before field-type list)",
                    file);
                return (OTYPE_VOID | OTYPE_SPECIAL);
            }
    ...
    for (i = 0; i < nfields; i++)
    {
        ...
        os = CVAllocSymLocked(
            tsymBlock, mem, &fsymOff, (ObjSymHeader**)&symBase);
        os->type = OSYM_FIELD;
    ...
    return CVFinishStructuredType(
        file, dataBase, len, os, tsymBlock, ttypeBlock, typeBlock);
}
    ...
    const char* file, byte** bpPtr, word len, VMBlockHandle typeBlock)
{
    ...
    if (typeBlock != 0)
    {
        ...
        {
            elType =
                CVProcessTypeRecord(file, &bp, bp[1] | (bp[2] << 8), typeBlock);
        }
        ...
        if (alen % 8)
        {
            Notify(NOTIFY_ERROR, "%s: Non-integral # bytes in array", file);
        }
        alen >>= 3;

        if (alen % elSize)
        {
            Notify(NOTIFY_ERROR, "%s: Non-integral # elements in array", file);
        }
        ...
        if ((bp - dataBase) < len)
        {
             ...
             */
            CVCreateTypedef(file, &bp, dataBase, len, typeBlock, retval);
        }
    }
    *bpPtr += len;
    return (retval);
 ...
 *
 ***********************************************************************/
static word CVProcessScalar(const char* file, /* Object file from which
                                               * the record came */
                                               ...
                                               * descriptors */
{
    ...
    {
        Notify(NOTIFY_ERROR, "%s: unknown scalar base type %02.2x", file, *bp);
    error:
        ...
        else
        {
            ...
            if (name == NullID)
            {
                ...
                if (*mlistBase == CTL_STRING)
                {
                    ...
                    if ((first != NullID) && Sym_Find(symbols,
                                                 globalSeg->syms,
                                                 first,
                                                 &tsymBlock,
                                                 &msymOff,
            ...
            while (1)
            {
                ...
                {
                    Notify(NOTIFY_ERROR,
                        "%s: invalid scalar descriptor (member name not "
                        "CTL_STRING tree)",
                        file);
                    break;
                }
                msym = CVAllocSymLocked(
                    tsymBlock, mem, &msymOff, (ObjSymHeader**)&symBase);
                msym->type = OSYM_ENUM;
             ...
             */
            retval = CVFinishStructuredType(
                file, *bpPtr, len, esym, tsymBlock, ttypeBlock, typeBlock);
        }
    }
 ...
 *
 ***********************************************************************/
static word CVFetchType(const char* file, /* Object file being read */
    ...
    if (index <= CST_LAST_PREDEF)
    {
        if (index & CST_SPECIAL)
        {
            if ((index & CST_MODE) != CSTM_DIRECT)
            {
                ...
                baseType = CVFetchType(
                    file, typeBlock, (index & ~CST_MODE) | CSTM_DIRECT);
...
#define CVSIZE_CURRENCY 3
    ...
    else
    {
        ...
        if (bp == NULL)
        {
            Notify(NOTIFY_ERROR, "%s: undefined type index 0x%x", file, index);
            return (OTYPE_VOID | OTYPE_SPECIAL);
        }
 ...
 *
 ***********************************************************************/
static word CVProcessTypeRecord(const char* file, /* Object file from which
                                                   * the record came */
                                                   ...
                                                   * descriptors */
{
    ...
    switch (typeClass)
    {
        ...
        case CTL_TYPEDEF:
        {
             ...
             */
            CVAllocSymAndTypeBlocks(&tsymBlock, &ttypeBlock);
            equivType = CVProcessTypeRecord(
                file, &bp, bp[1] | (bp[2] << 8), ttypeBlock);
            ...
            retval = CVFinishStructuredType(
                file, dataBase, len - 1, os, tsymBlock, ttypeBlock, typeBlock);
            break;
        }
        case CTL_PARAMETER:
        case CTL_CONSTANT:
            Notify(NOTIFY_ERROR,
                "%s: can't handle parameter/constant -- Microsoft didn't "
                "define them.",
                file);
            break;
        case CTL_LABEL:
        ...
        case CTL_PROCEDURE:
            ...
            else
            {
                 ...
                 */
                (void)CVProcessTypeRecord(file, &bp, bp[1] | (bp[2] << 8), 0);
                if (*bp == CTL_NEAR)
                {
                    retval = OTYPE_NEAR | OTYPE_SPECIAL;
                }
                ...
                {
                    Notify(NOTIFY_ERROR,
                        "%s: Unknown call-type (%02.2x) in PROCEDURE "
                        "definition",
                        file,
                        *bp);
                }
 ...
 *
 ***********************************************************************/
static void CVProcessUnprocessedTypeRecords(const char* file)
{
    byte* bp;
    byte* end;
    word len;
    ...
    while (bp < end)
    {
        ...
        switch (bp[3])
        {
            ...
            case CTL_SCALAR:
                bp += 3;
                (void)CVProcessTypeRecord(file, &bp, len, 0);
                break;
            default:
                bp = bp + 3 + len;
                break;
 ...
 *
 ***********************************************************************/
void CV_Init(const char* file, FILE* f)
{
 ...
 *
 ***********************************************************************/
static ObjSym* CVAllocLocalSym(VMBlockHandle symBlock,
    MemHandle mem,
    word curScope,
    word* lastLocalPtr,
    genptr* symBasePtr)
 ...
 *
 ***********************************************************************/
static Boolean CVDetermineSymbolBlock(const char* file, /* Object file being
                                                         * read */
                                                         ...
                                                         * locked symBlock */
{
    ...
    *addrPtr += (*sdPtr)->nextOff + (bp[0] | (bp[1] << 8));
    ...
    if (symBlock != 0)
    {
        VMInfo(symbols, symBlock, &curSize, (MemHandle*)NULL, (VMID*)NULL);
    ...
    if (symBlock == 0)
    {
        ...
        if ((*sdPtr)->addrT)
        {
            ...
            osh = (ObjSymHeader*)VMLock(
                symbols, (*sdPtr)->addrT, (MemHandle*)NULL);
            osh->next = symBlock;

            VMInfo(
                symbols, osh->types, &curSize, (MemHandle*)NULL, (VMID*)NULL);
    ...
    *symBasePtr =
        (genptr)(osh = (ObjSymHeader*)VMLock(symbols, symBlock, memPtr));
...
static void CVProcessSymbols(
    const char* file) /* Object file name (for errors) */
{
    ...
    while (bp < end)
    {
        ...
        switch (*bp++)
        {
            ...
            case CST_BLOCK_START:
            {
                ...
                {
                    Notify(
                        NOTIFY_ERROR, "%s: block start not in procedure", file);
                    break;
                }
                else if (scopeTop == MAX_SCOPES)
                {
                    Notify(NOTIFY_ERROR, "%s: Too many nested scopes", file);
                    break;
                }
                ...
                if (extraOffset ==
                    ((ObjSym*)(symBase + scopeStack[scopeTop - 1]))
                        ->u.proc.address)
                {
                    /*
                ...
                else
                {
                    ...
                    for (i = scopeTop - 1; i >= 0; i--)
                    {
                        if (((ObjSym*)(symBase + scopeStack[i]))->type ==
                            OSYM_PROC)
                        {
                            extraOffset -= ((ObjSym*)(symBase + scopeStack[i]))
                                               ->u.proc.address;
                            break;
                        }
                    }

                    os = CVAllocLocalSym(symBlock,
                        mem,
            ...
            case CST_PROC_START:
            {
                ...
                {
                    Notify(NOTIFY_ERROR,
                        "%s: procedure %i may not be nested inside another "
                        "scope",
                        file,
                        name);
                    break;
                ...
                if (!CVDetermineSymbolBlock(file,
                        name,
                        "procedure",
                        bp,
                        NULL,
                 ...
                 */
                if (CVLocatePublic(
                        name, (SegDesc**)NULL, (word*)NULL, &real, &alias) &&
                    real)
                {
                ...
                os->u.localVar.type = CVProcessTypeRecord(
                    file, &ptype, ptype[1] | (ptype[2] << 8), typeBlock);
            ...
            case CST_LOCAL_VAR:
            {
                ...
                os->u.localVar.type =
                    CVFetchType(file, typeBlock, MSObj_GetWordImm(bp));
                break;
            }
            case CST_VARIABLE:
            {
                ...
                os = CVAllocSymLocked(
                    tsymBlock, tmem, &symOff, (ObjSymHeader**)&tsymBase);
                os->type = OSYM_VAR;
                os->name = name;
                os->flags = 0;
                os->u.variable.address = addr;
                os->u.variable.type =
                    CVFetchType(file, ttypeBlock, bp[4] | (bp[5] << 8));
                ...
                else if (sd->name)
                {
                    ...
                    if (!(strncmp(
                            segName, "_CLASSSEG_", sizeof("_CLASSSEG_") - 1)))
                    {
                        os->flags |= OSYM_GLOBAL;
                    }
 ...
 *
 ***********************************************************************/
void CV_Finish(const char* file,
    ...
    int pass) /* Pass #: 1 if pass 1, 2 if pass 2 */
{
    switch (rectype)
    {
        ...
        case MO_SEGDEF:
        {
            ...
            if (!MSObj_DecodeSegDef((char*)file,
                    rectype,
                    bp,
                    &type,
                    &align,
            ...
            if (name == cvTypesSegment.name)
            {
                ...
                {
                    Notify(NOTIFY_ERROR,
                        "%s: type descriptor segment already defined for this "
                        "file",
                        file);
                }
                else if (size != 0)
        ...
        case MO_LEDATA:
        {
            ...
            if (sd == CV_SYMS_SEGMENT)
            {
                ...
                if (msobjBuf[reclen] == MO_FIXUPP)
                {
                     ...
                     */
                    (void)Pass1MS_CountRels(
                        (char*)file, rectype, sd, startOff, reclen, bp);
                }
            }
            else
            {
        ...
        case MO_LIDATA:
        {
            ...
            assert((sd != CV_SYMS_SEGMENT) && (sd != CV_TYPES_SEGMENT));
            return (FALSE);
        }
        case MO_CVPUB:
        case MO_PUBDEF:
