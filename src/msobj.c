    ...
    const char* file, byte rectype, word reclen, byte* data, int pass)
{
    if (rectype == MO_COMENT)
    {
        ...
        else if ((data[1] == BCC_DEPENDENCY) || (data[1] == BCC_VERSION))
        {
            /*
        ...
        else if ((pass == 1) && (data[1] == 0) && /* class 0 */
                 ...
                 (data[2] == '@'))
        {
             ...
             */
            msobj_CurFileName =
                ST_Enter(symbols, strings, (char*)&data[3], reclen - 3);
        }
    }
    /*
     * Not consumed
 ...
 *
 ***********************************************************************/
void MSObj_Init(FILE* stream) /* Stream open to object file (unused) */
{
    if (msobjBufSize == 0)
    {
        /*
        ...
        lmemSegs = Vector_Create(sizeof(MSObjLMemData*), ADJUST_ADD, 5, 5);
    }
    else
    {
 ...
 *
 ***********************************************************************/
byte MSObj_ReadRecord(FILE* stream, /* Stream from which to read */
    word* datalenPtr,
                    ...
                    * (doesn't include header or checksum) */
{
    ...
    *datalenPtr = len - 1;
     ...
     * the end.
     */
    if ((sum == 0) && (rectype == MO_LEDATA || rectype == MO_LIDATA ||
                          rectype == MO_LEDATA32 || rectype == MO_LIDATA32))
    {
 ...
 *
 ***********************************************************************/
SegDesc* MSObj_GetSegment(byte** bufPtr)
{
 ...
 *
 ***********************************************************************/
GroupDesc* MSObj_GetGroup(byte** bufPtr)
{
    word index = MSObj_GetIndex(*bufPtr);
    GroupDesc* retval;
 ...
 *
 ***********************************************************************/
VMPtr MSObj_GetExternal(byte** bufPtr)
{
    word index = MSObj_GetIndex(*bufPtr);
    VMPtr retval;
 ...
 *
 ***********************************************************************/
void MSObj_DecodeFrameOrTarget(
 ...
 *
 ***********************************************************************/
int MSObj_DecodeFixup(const char* file,
    ...
    MSFixData* framePtr)  /* Fixup frame data (Out) */
{
    ...
    *fixLocPtr = (*bp << 8) | bp[1];
    ...
    else
    {
        ...
        {
            MSObj_DecodeFrameOrTarget(
                (fixdata & FD_FRAME) >> FD_FRAME_SHIFT, &bp, framePtr);
        }
    ...
    if (fixdata & FD_TARG_IS_THREAD)
    {
        ...
        {
            fixdata =
                (fixdata & ~FD_TARGET) | (msThreads[thread].fixup & FD_TARGET);
    ...
    if ((fixdata & FD_FRAME) == (FFM_TARGET << FD_FRAME_SHIFT))
    {
        *framePtr = *targetPtr;
        fixdata =
            (fixdata & ~FD_FRAME) | ((fixdata & FD_TARGET) << FD_FRAME_SHIFT);
    }
 ...
 *
 ***********************************************************************/
int MSObj_DecodeSegDef(const char* file,
    byte rectype,
    byte* bp,
    int* typePtr,
    int* alignPtr,
 ...
 *
 ***********************************************************************/
char* MSObj_DecodeLMemName(char* segName, int* order)
{
    char* cp;
 ...
 *
 ***********************************************************************/
int MSObj_GetLMemSegOrder(SegDesc* seg)
{
    int order;
 ...
 *
 ***********************************************************************/
void MSObj_SaveRecord(byte rectype, word reclen, MSSaveRecLinks* head)
{
    MSSaveRec *srp, *prev;
 ...
 *
 ***********************************************************************/
void MSObj_SaveFixups(dword startOff, /* Starting offset of data record */
                                       ...
                                       * record */
{
    ...
    for (sfp = (MSSaveFixupRec*)head->prev; sfp != (MSSaveFixupRec*)head;
 ...
 *
 ***********************************************************************/
void MSObj_FreeSaved(MSSaveRecLinks* head)
{
 ...
 *
 ***********************************************************************/
void MSObj_FreeFixups(MSSaveRecLinks* head)
{
    MSSaveRec *srp, *nextSRP;
 ...
 *
 ***********************************************************************/
static Boolean MSCompareStructs(ObjSym* os, /* Locked structure symbol */
 ...
 *
 ***********************************************************************/
void MSObj_AddAnonStruct(ObjSym* os, /* Locked structure symbol */
                                      ...
                                      * structure */
{
    ...
    {
        for (i = Vector_Length(entries), msas = Vector_Data(entries); i > 0;
             i--, msas++)
        {
    ...
    if (new)
    {
        ...
        if (entries == NullVector)
        {
            entries = Vector_Create(
                sizeof(MSAnonStruct), ADJUST_ADD, MS_ANON_INCR, MS_ANON_INIT);
 ...
 *
 ***********************************************************************/
ObjType* MSObj_AllocType(VMBlockHandle typeBlock, word* offsetPtr)
{
    MemHandle mem;
    ObjTypeHeader* oth;
    ObjType* ot;
 ...
 *
 ***********************************************************************/
word MSObj_CreateArrayType(VMBlockHandle types, word base, int alen)
{
    int numExtra;
    word retval;
    ObjType* otp;
...
static char* SpecialStringsFromBorland[] = {
    "FIW", "FIA", "FIS", "FIE", "FID", "FJC", "FJS", "FJA", "FIC", NULL};

/* a table of values corresponding to the strings in the previous table */
static char SpecialTokensFromBorland[] = {FPED_FIWRQQ,
    FPED_FIARQQ,
    FPED_FISRQQ,
    FPED_FIERQQ,
    FPED_FIDRQQ,
...
FloatingPointExtDef MSObj_IsFloatingPointExtDef(ID name)
{
    char* str;
    ...
    if (strlen(str) == FLOATING_POINT_SYM_LENGTH && str[3] == 'R' &&
        str[4] == 'Q' && str[5] == 'Q')
    {
 ...
 *
 ***********************************************************************/
static FloatingPointExtDef MSObjMapExternal(
    const char* file,           /* Name of object file being read */
    ...
    if (targMethod == TFM_EXTERNAL)
    {
        ...
        else
        {
            ...
            if (fdataPtr->segment->combine == SEG_LIBRARY)
            {
                ...
                {
                    publishedID = Library_TackPrependPublishedToID(
                        symbols, strings, os->name);
                    if (publishedID != NullID)
                    {
                        publishedID = Library_TackPrependPublishedToID(
                            symbols, strings, publishedID);
                                ...
                                &pubSD))
                        {
                            ...
                            osh = (ObjSymHeader*)VMLock(
                                symbols, pubBlock, (MemHandle*)NULL);
                    ...
                    if (publishedID == NullID)
                    {
                         ...
                         */
                        (void)Library_UseEntry(
                            fdataPtr->segment, os, TRUE, TRUE);
                        return (FPED_FALSE);
                    }
                }
            }
            ...
            *symBlockPtr = symBlock;
            ...
            /*
             * See below for why we need to add 2 to the offset in the handle
             * table.
             */
 ...
 *
 ***********************************************************************/
static void MSObjLocateSymbol(SegDesc* sd,
    ...
    VMBlockHandle* blockPtr)
{
    ...
    if (sd->addrMap != 0)
    {
        ...
        oamh =
            (ObjAddrMapHeader*)VMLock(symbols, sd->addrMap, (MemHandle*)NULL);
         ...
         */
        for (i = oamh->numEntries, oame = (ObjAddrMapEntry*)(oamh + 1); i > 0;
             i--, oame++)
        {
            if (oame->last > offset)
            {
        ...
        while (i <= oamh->numEntries)
        {
            ...
            osh = (ObjSymHeader*)VMLock(symbols, oame->block, (MemHandle*)NULL);
 ...
 *
 *********************************************************************/
static void MSObjDoFloatingPointRelocation(byte* fixme, FloatingPointExtDef f)
{
    switch (f)
    {
        case FPED_FIWRQQ:
 ...
 *
 ***********************************************************************/
Boolean MSObj_PerformRelocations(const char* file, /* Name of object file */
    ...
    byte** nextRelPtr)
{
    byte* nextRel;
    Boolean retval = TRUE;
    Boolean isLMemHandleSegment = ((sd != NULL) && (sd->combine == SEG_LMEM) &&
    ...
    while (bp < endRecord)
    {
	    ...
	    int	    	    i;
             ...
             */
            if (!MSObj_DecodeFixup(
                    file, sd, &bp, &fixLoc, &fixdata, &target, &frame))
            {
                return (FALSE);
            }
            ...
            if ((fixdata & FD_FRAME) == (FFM_ABSOLUTE << FD_FRAME_SHIFT))
            {
                if ((fixdata & FD_TARGET) != TFM_ABSOLUTE)
                {
                    if (pass == 2)
                    {
                        Pass2_RelocError(sd,
                            fixOff,
                            "relocation target must be absolute if frame is "
                            "absolute");
                    }
                    retval = FALSE;
                    goto error;
            ...
            else if ((fixdata & FD_TARGET) == TFM_ABSOLUTE)
            {
                if (pass == 2)
                {
                    Pass2_RelocError(sd,
                        fixOff,
                        "relocation frame must be absolute if target is "
                        "absolute");
                }
                retval = FALSE;
                goto error;
            ...
            else if (target.segment->combine == SEG_LIBRARY)
            {
                ...
                fixdata =
                    (fixdata & ~FD_FRAME) | (FFM_SEGMENT << FD_FRAME_SHIFT);
            }
            ...
            switch (((fixLoc >> 8) & FLH_LOC_TYPE) >> FLH_LOC_TYPE_SHIFT)
            {
                ...
                case FLT_FAR_PTR:
                {
                    ...
                    if (sd->doObjReloc)
                    {
                        ...
                        if (target.segment->combine == SEG_LMEM)
                        {
                            ...
                            {
                                oreloc =
                                    (ORS_OWNING_GEODE << RID_SOURCE_OFFSET) +
                        ...
                        else
                        {
                            ...
                            if (os == NULL)
                            {
                                Pass2_RelocError(sd,
                                    fixOff,
                                    "cannot unrelocate far pointer to segment "
                                    "%i in segment %i, as it doesn't point to "
                                    "an exported entry point for this geode",
                                    target.segment->name,
                                    sd->name);
                                break;
                            }
                            ...
                            else
                            {
                                ...
                                if (target.segment->combine == SEG_LIBRARY)
                                {
                                    oreloc =
                                        (ORS_LIBRARY << RID_SOURCE_OFFSET) |
                                        libs[target.segment->pdata.library]
                                            .lnum;
                                    /*
                ...
                case FLT_OFFSET:
                                ...
                                (FLT_LDRRES_OFF << FLH_LOC_TYPE_SHIFT))))
                    {
                        ...
                        {
                            (void)Out_Fetch(
                                sd->nextOff + fixOff - 1, (void*)&prevByte, 1);
                        }
                        else if (fixLoc & FL_OFFSET)
                        {
                            prevByte = fixme[-1];
                        ...
                        !Obj_CheckRelated(frame.segment, sd))
                    {
                        if (pass == 2)
                        {
                            if (os != NULL)
                            {
                                Pass2_RelocError(sd,
                                    fixOff,
                                    "near call/jump to different segment "
                                    "(%i:%i)",
                                    frame.segment->name,
                                    os->name);
                            }
                ...
                case FLT_HIGH_BYTE:
                    ...
                    {
                        Pass2_RelocError(
                            sd, fixOff, "unsupported fixup type FLT_HIGH_BYTE");
                    }
                    retval = FALSE;
                    break;
            }
 ...
 *
 ***********************************************************************/
word MSObj_CalcIDataSize(byte** bufPtr)
{
    word repCount;
    word blockCount;
    word contentSize;
 ...
 *
 ***********************************************************************/
void MSObj_ExpandIData(byte** dataPtr, /* Iterated data block */
