...
typedef struct
{
    ...
    int flags;
#define FILE_NOCALL                                          \
    1                     /* CALL relocations not handled at \
                           * runtime */
...
#define FILE_USES_GEODE_PARAMS                 \
    16 /* Format uses a geode parameters file, \
        * so its libraries should be loaded    \
        * by Parse_GeodeParams before pass 1 */
#define FILE_AUTO_LINK_LIBS                    \
    32 /* If any library segment has undefined \
        * symbols, call Library_Link for it    \
        * automatically */
#define FILE_PROTO_RELS                   \
    64 /* Format supports libraries and   \
 ...
 */
extern FileOps exeOps, comOps, geoOps, vmOps, kernelOps, fontOps, rawOps;
...
extern void Out_RegisterFinalProtoRel(
    Boolean isMajor, long position, SegDesc* library);
#endif /* _OUTPUT_H_ */
