...
static word CV32ProcessTypeRecord(
    const char* file, byte** bpPtr, word len, VMBlockHandle typeBlock);
static word CV32FetchType(
    const char* file, VMBlockHandle typeBlock, unsigned index);
 ...
 *
 ***********************************************************************/
static Boolean CV32LocatePublic(ID name, /* Name to find */
    ...
    for (srp = pubHead.next; srp != (MSSaveRec*)&pubHead; srp = srp->links.next)
    {
        ...
        while (bp < end)
        {
            ...
            if (((*bp == namelen) &&
                ...
                ((aliasPtr != NULL) && (bp[1] == '_') && (*bp == namelen + 1) &&
 ...
 *
 ***********************************************************************/
static Boolean CV32LocateFixup(
    const char* file,  /* Object file being processed */
 ...
 *
 ***********************************************************************/
static void CV32AllocSymAndTypeBlocks(
 ...
 *
 ***********************************************************************/
static ID CV32GetString(byte** bpPtr)
{
    ID retval;
 ...
 *
 ***********************************************************************/
static unsigned long CV32GetInteger(byte** bpPtr)
{
    byte* bp;
 ...
 *
 ***********************************************************************/
static ObjSym* CV32AllocSymLocked(
 ...
 *
 ***********************************************************************/
static ObjSym* CV32AllocSym(VMBlockHandle symBlock, word* offsetPtr)
{
    MemHandle mem;
    ObjSymHeader* osh;
 ...
 *
 ***********************************************************************/
static byte* CV32LocateType(word index, word* lenPtr)
{
    word len;
    byte* bp;
    byte* endTypes;
 ...
 *
 ***********************************************************************/
static word CV32FinishStructuredType(
 ...
 *
 ***********************************************************************/
static void CV32CreateTypedef(const char* file, /* Object file from which the
                                                 * description came */
 ...
 *
 ***********************************************************************/
static word CV32ProcessStructure(
    const char* file, byte** bpPtr, word len, VMBlockHandle typeBlock)
{
    ...
    //    byte    	    *nlistp,	    /* Current byte in name/offset
    //    CTL_LIST
    //				     * record */
    ...
    *bpPtr += len;
    ...
    return CV32FinishStructuredType(
    ...
    const char* file, byte** bpPtr, word len, VMBlockHandle typeBlock)
{
    ...
    if (typeBlock != 0)
    {
        ...
        if ((bp - dataBase) < len)
        {
             ...
             */
            CV32CreateTypedef(file, &bp, dataBase, len, typeBlock, retval);
        }
    }
    *bpPtr += len;
    return (retval);
 ...
 *
 ***********************************************************************/
static word CV32ProcessScalar(const char* file, /* Object file from which
                                                 * the record came */
                                                 ...
                                                 * descriptors */
{
    ...
    if (fieldTypeIndex)
    {
        ...
        while (1)
        {
            ...
            {
                Notify(NOTIFY_ERROR,
                    "%s: invalid scalar descriptor (member name not CTL_STRING "
                    "tree)",
                    file);
                break;
            }
         ...
         */
        retval = CV32FinishStructuredType(
 ...
 *
 ***********************************************************************/
static word CV32FetchType(const char* file, /* Object file being read */
 ...
 *
 ***********************************************************************/
static word CV32ProcessTypeRecord(const char* file, /* Object file from which
                                                     * the record came */
    ...
    while (bp < endp)
    {
        ...
        switch (leaf)
        {
            case CTL2_PROCEDURE:
            {
                ...
                /*if (*bp++ != CTL_NIL) {
                        Notify(NOTIFY_ERROR,
                                "%s: PROCEDURE definition missing spurious NIL
                leaf", file);
                }
                else {*/
                /*
                 ...
                 *
                 * mcasadevall: THere's no length record anymore for procedures,
                 * so we calculate the length to jump based off the arg count
                 * and ignore the len variable.
                 */
 ...
 *
 ***********************************************************************/
static void CV32ProcessUnprocessedTypeRecords(const char* file)
{
    byte* bp;
    byte* end;
    word len;
    ...
    while (bp < end)
    {
        ...
        switch (leaf)
        {
            ...
            case CTL2_STRUCTURE:
                // case CTL_SCALAR:
                (void)CV32ProcessTypeRecord(file, &bp, len, 0);
                break;
            default:
                bp = bp + len;
                break;
 ...
 *
 ***********************************************************************/
void CV32_Init(const char* file, FILE* f)
{
 ...
 *
 ***********************************************************************/
static ObjSym* CV32AllocLocalSym(VMBlockHandle symBlock,
    MemHandle mem,
    word curScope,
    word* lastLocalPtr,
    genptr* symBasePtr)
 ...
 *
 ***********************************************************************/
static Boolean CV32DetermineSymbolBlock(const char* file, /* Object file being
                                                           * read */
...
static void CV32ProcessSymbols(
    const char* file) /* Object file name (for errors) */
{
    ...
    if (sig != 1)
    {
        Notify(NOTIFY_ERROR, "%s: unsupported CodeView format", file);
    }
    ...
    while (bp < end)
    {
        ...
        switch (recType)
        {
            ...
            case CST2_BLOCK16:
            {
                ...
                if (CV32LocateFixup(
                        file, bp - symSeg, &sd, &extraOffset, &name))
                {
            ...
            case CST2_BPREL16:
            {
                ...
                os->u.localVar.type =
                    CV32FetchType(file, typeBlock, MSObj_GetWordImm(bp));
                break;
            }
            case CST2_UDT:
                /*
            ...
            case CST2_GDATA16:
                 ...
                 */
                {
                    ...
                    if (!CV32DetermineSymbolBlock(file,
                            (char*)bp + 7,
                            bp[6],
                            "variable",
                            bp,
                    ...
                    os->u.variable.type =
                        CV32FetchType(file, ttypeBlock, bp[4] | (bp[5] << 8));
                        ...
                        real)
                    {
                            ...
                            (MSObj_GetLMemSegOrder(sd) == 1))
                        {
                            os->type = OSYM_VAR;
                            os->u.variable.type =
                                OTYPE_MAKE_VOID_PTR(OTYPE_PTR_NEAR);
			...
			*/
			...
			if (Sym_Find(symbols, sd->syms, name,
				...
				if ((foundOffset != tsymBlock) ||
                    ...
                    else
                    {
                         ...
                         * LOCAL_STATIC symbol inside the current scope to point
                         * to the VAR symbol we just created.
                         */
                        if (sd == procSD)
                        {
                            /*
            ...
            case CST2_GPROC16:
            {
                ...
                /*else if (*ptype++ != CTL_NIL) {
                        Notify(NOTIFY_ERROR,
                                "%s: PROCEDURE definition missing spurious NIL
                leaf", file); VMUnlock(symbols, symBlock); symBlock = 0; break;
                }*/
                 ...
                 */
                os = CV32AllocSymLocked(symBlock,
                    mem,
                    &scopeStack[scopeTop++],
                ...
                ptype -= 2;
                os->u.localVar.type =
                    CV32ProcessTypeRecord(file, &ptype, ptypeLen, typeBlock);
 ...
 *
 ***********************************************************************/
void CV32_Finish(const char* file,
 ...
 *
 ***********************************************************************/
int CV32_Check(const char* file,
    byte rectype,
    word reclen,
    byte* bp,
