 ...
 *
 ***********************************************************************/
static void Pass2MSFixupLMem(const char* file, /* File being linked */
    ...
    long size)                                 /* Size of same */
{
    ...
    if (lmem->nrel)
    {
        relocs = (GeodeRelocEntry*)malloc(lmem->nrel * sizeof(GeodeRelocEntry));
        Out_Fetch(lmem->foff + ((lmem->size + 15) & ~15),
            relocs,
    ...
    while (i > 0)
    {
        if ((*newHandle != 0) && (*newHandle != 0xffff))
        {
            ...
            finalVarType = 0;
            finalVarOff = 0; /* Be quiet, GCC (these are
                              * used only when
            ...
            size = Obj_TypeSize(finalVarType,
                VMLock(symbols, finalVarTypesBlock, (MemHandle*)NULL),
                FALSE);
            ...
            for (rel = relocs, j = lmem->nrel; j > 0; rel++, j--)
            {
                if ((swaps(rel->offset) >=
                        ((swaps(*oldHandle) - LMEM_SIZE_SIZE) +
                            heap->grpOff)) &&
                    (swaps(rel->offset) <
                        ((prevOldOff - LMEM_SIZE_SIZE) + heap->grpOff)))
                {
    ...
    if (prevOff > 2)
    {
        ...
        Notify(NOTIFY_WARNING,
            "fixing up unused space in lmem resource %i",
            lmem->name);
         ...
         */
        Out_Block(lmem->foff + offsetof(LMemBlockHeader, LMBH_totalFree),
            heapData,
            2);
    }
    /*
     ...
     * heap.
     */
    oamh = (ObjAddrMapHeader*)VMLock(symbols, lmem->addrMap, (MemHandle*)NULL);
     ...
     * chunk.
     */
    for (oldHandle = nextOldHandle - 1, newHandle = nextNewHandle - 1, i -= 1;
    ...
    while (i > 0)
    {
        ...
        { /* block look */
            ...
            if (!Obj_IsAddrSym(os))
            {
                 ...
                 * symbols, either, as they'd be an error at this point and
            ...
            { /* smybol loop */
                 ...
                 * there are no type or procedure-local symbols here...)
                 * relocate it by the amount the associated chunk shifted.
                 */
                if (Obj_IsAddrSym(os))
                {
                    if ((i == 0) ||
                                ...
                                (*nextOldHandle == NULL))))
                    {
                         ...
                         * to current chunk's relocation amount. Both *newHandle
                         * and *oldHandle have LMEM_SIZE_SIZE added into them,
                         * so we need to add it into the address explicitly.
                         */
                        newLastOffset =
                            (os->u.addrSym.address +=
                                (swaps(*newHandle) -
            ...
            /*
             * Advance to next block, dirtying this one, since we've messed with
             * it
             */
            next = osh->next;
            VMUnlockDirty(symbols, cur);
            /*
             * Update the offset for the block in the address map to be that of
             * the last symbol we relocated.
             */
            oame->last = lastOff;
            oame++, mapEntries--;
        } /* block loop */
        ...
        if (i > 0)
        {
            ...
            for (oldHandle = nextOldHandle - 1,
                ...
                i -= 1;
                 ...
                 oldHandle--, newHandle++, i--)
            {
                if ((*oldHandle != 0) && (*oldHandle != 0xffff))
                {
                    break;
                }
            }
        ...
        { /* smybol loop */
            ...
            if (os->type == OSYM_VAR)
            {
                    ...
                    strncmp(name, "__carray_", 9) == 0)
                {
                    ...
                    tbase =
                        (genptr)VMLock(symbols, osh->types, (MemHandle*)NULL);
                     ...
                     * the array.
                     */
                    bp = heapData + first[preceding].u.addrSym.address -
                         heap->grpOff;
                    *bp++ = nels;
                    *bp = nels >> 8;
                }
    ...
    if (lmem->nrel)
    {
        Out_Block(lmem->foff + ((lmem->size + 15) & ~15),
            relocs,
            lmem->nrel * sizeof(GeodeRelocEntry));
    }
...
void Pass2MS_Finish(const char* file, int happy, int pass)
{
     ...
     */
    for (backpatch = backPatches.next; backpatch != (MSSaveRec*)&backPatches;
 ...
 *
 ***********************************************************************/
static void Pass2MSProcessRels(const char* file, /* Object file being read */
                   ...
                   * record */
{
        ...
        (baseOff == 0))
    {
        ...
        if ((*fileOps->maprel)(
                OREL_HANDLE, (SegDesc*)sd->group, nextRel, sd, 0, (word*)data))
        {
    ...
    {
        (void)MSObj_PerformRelocations(
            file, data, bp, endRecord, sd, baseOff, 2, (byte**)&nextRel);
    }
 ...
 *
 ***********************************************************************/
void Pass2MS_Load(const char* file, /* File name (for error messages) */
 ...
 *
 ***********************************************************************/
void Pass2MS_ProcessObject(const char* file, FILE* f)
{
    word reclen;
    byte rectype;
    int done = FALSE;
        ...
        switch (rectype)
        {
            ...
            case MO_EXTDEF:
            {
                ...
                while (bp < endRecord)
                {
                    ...
                    else if (bp[1] == '_')
                    {
                        ...
                        newname = ST_Lookup(
                            symbols, strings, (char*)bp + 2, (*bp) - 1);

                        if ((newname == NullID) || !Sym_FindWithFile(symbols,
                                                       file,
                                                       0,
                                                       newname,
                                                       &symBlock,
            ...
            case MO_LEDATA:
            {
                ...
                if ((msobjBuf[reclen] == MO_FIXUPP) ||
                    ((sd->combine == SEG_LMEM) && (sd->group->segs[0] == sd) &&
                        (fileOps->rtrelsize != 0) && (startOff == 0)))
                {
                    /*
                     * Record has fixups. Process them.
                     */
                    Pass2MSProcessRels(file, rectype, sd, startOff, reclen, bp);
                }
                /*
