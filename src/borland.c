...
#define BORLAND_INCR_SYMS                      \
    128 /* Number of symbols to add to a block \
         * when it gets too small but ain't up \
         * to capacity yet */

#define BORLAND_FIRST_MEMBER (sizeof(ObjSymHeader) + sizeof(ObjSym))

#define BT_MAX_TYPE_LEN               \
    12 /* The most # bytes in a type  \
        * description from the object \
        * file */
/*
 * The stack of current local-variable scopes.
 */
#define BORLAND_MAX_SCOPE             \
    30 /* Greatest scope depth we can \
        * handle */
...
#define BORLAND_CONVERTED_LONG_TYPE                                   \
    2                           /* Value stored in shortType when     \
 ...
 *
 ***********************************************************************/
static BorlandSegData* BorlandGetSegData(int index)
{
 ...
 *
 ***********************************************************************/
static BorlandType* BorlandGetTypeData(word index)
{
 ...
 *
 ***********************************************************************/
static VMBlockHandle BorlandAllocAndInitSymBlock(void)
{
    VMBlockHandle block = VMAlloc(symbols,
        sizeof(ObjSymHeader) + BORLAND_INIT_SYMS * sizeof(ObjSym),
        OID_SYM_BLOCK);
    ObjSymHeader* osh;
 ...
 *
 ***********************************************************************/
static VMPtr BorlandEnterAddressSymbol(ID id, /* Name of new sym */
                                               ...
                                               * symbol resides */
{
    ...
    else
    {
        ...
        if (osh == NULL)
        {
                ...
                (scopeTop == 0))
            {
                 ...
                 */
                sData->addrT = next = osh->next = BorlandAllocAndInitSymBlock();
                VMUnlockDirty(symbols, block);
                /*
         ...
         */
        for (os = ObjFirstEntry(osh, ObjSym), i = osh->num; i > 0; os++, i--)
        {
    ...
    if (i != 0)
    {
        ...
        if (os->u.addrSym.address == offset)
        {
            ...
            while ((os->u.addrSym.address == offset) && !replace && i > 0)
            {
                ...
                else if (symType == OSYM_BLOCKSTART &&
    ...
    if ((i != 0) && (need != osh->num))
    {
         ...
         */
        for (j = osh->num + 1, tos = ObjFirstEntry(osh, ObjSym); j > 0;
             j--, tos++)
        {
            switch (tos->type)
            {
             ...
             j--, bls++)
        {
            ...
            if ((bls->varBlock == block) && (bls->varOffset >= insertOff))
            {
                ...
                lsos =
                    (ObjSym*)VMLockVMPtr(symbols, bls->local, (MemHandle*)NULL);
 ...
 *
 ***********************************************************************/
static void BorlandProcessPublic(
    const char* file, byte rectype, word reclen, byte* bp)
{
    ...
    while (bp < endRecord)
    {
        ...
        lastPublic = BorlandEnterAddressSymbol(
            id, offset, OSYM_UNKNOWN, OSYM_GLOBAL | mangled, sdata);
    }
}
 ...
 *
 ***********************************************************************/
static void BorlandProcessExternal(
    const char* file, byte rectype, word reclen, byte* bp)
{
    /*
    ...
    if (lastExternal == NullVMPtr)
    {
        ...
        lastExternal =
            MAKE_VMP(extBlock, sizeof(ObjSymHeader) - sizeof(ObjSym));
    }
    ...
    while (bp < endRecord)
    {
        ...
        os++;
        lastExternal =
            MAKE_VMP(extBlock, VMP_OFFSET(lastExternal) + sizeof(ObjSym));
 ...
 *
 ***********************************************************************/
static void BorlandProcessExternalType(const char* file,
 ...
 *
 ***********************************************************************/
static void BorlandProcessPublicType(const char* file,
    word reclen, /* Length of remainder of
                  * COMENT record */
    byte* bp)    /* Remainder of COMENT record */
{
 ...
 *
 ***********************************************************************/
static void BorlandRecordMemberBlock(VMBlockHandle symBlock)
{
    BorlandMembers memb;
 ...
 *
 ***********************************************************************/
static void BorlandProcessStructMembers(const char* file,
    word reclen, /* Length of remainder of
                  * COMENT record */
    byte* bp)    /* Remainder of COMENT record */
{
        ...
        switch (flags)
        {
            ...
            default:
                ...
                else
                {
                    ...
                    {
                        os->name =
                            ST_Enter(symbols, strings, (char*)bp + 1, *bp);
                    }
                    else
                    {
                        os->name = NullID;
 ...
 *
 ***********************************************************************/
static void BorlandProcessEnumMembers(const char* file,
    word reclen, /* Length of remainder of
                  * COMENT record */
    byte* bp)    /* Remainder of COMENT record */
{
 ...
 *
 ***********************************************************************/
static void BorlandFinishSEU(byte* tname, /* Name for the type (counted
                                           ...
                                           * thing being finished */
{
    ...
    if (tname != NULL)
    {
        ...
        else
        {
             ...
             * front of the tag to be consistent with CodeView and general
             * usage.
             */
            switch (symType)
            {
 ...
 *
 ***********************************************************************/
static void BorlandProcessTypedef(const char* file,
    word reclen, /* Length of remainder of
                  * COMENT record */
    byte* bp)    /* Remainder of COMENT record */
{
    ...
    switch (*bp++)
    {
        ...
        case BTID_FUNCTION: /* Function/procedure:
            ...
            MSObj_GetIndex(bp);
            bt.shortType =
                ((*bp == 0 || *bp == 1) ? OTYPE_MAKE_NEAR() : OTYPE_MAKE_FAR());
 ...
 *
 ***********************************************************************/
static ObjSym* BorlandAllocLocal(
    VMBlockHandle block, ObjSym** sosPtr, VMPtr* symPtr)
{
    ObjSymHeader* osh;
    word size;
    MemHandle mem;
 ...
 *
 ***********************************************************************/
static void BorlandLinkLocal(VMPtr sym)
{
    ...
    sos = (ObjSym*)VMLockVMPtr(
        symbols, scopeStack[scopeTop - 1].scope, (MemHandle*)NULL);
    ...
    switch (os->type)
    {
        default:
        {
             ...
             */
            os = BorlandAllocLocal(
                VMP_BLOCK(scopeStack[scopeTop - 1].scope), &sos, &bls.local);
 ...
 *
 ***********************************************************************/
static void BorlandProcessBeginScope(const char* file,
    word reclen, /* Length of remainder of
                  * COMENT record */
    byte* bp)    /* Remainder of COMENT record */
{
    ...
    switch (scopeTop)
    {
        case 0:
             ...
             * us its address.
             */
            sym = BorlandEnterAddressSymbol(
                NullID, MSObj_GetWordImm(bp), OSYM_BLOCKSTART, 0, sdata);
            scopeNum = 0;
            break;
        case 1:
            /*
             ...
             */
            sym = BorlandEnterAddressSymbol(
                ST_EnterNoLen(symbols, strings, OSYM_PROC_START_NAME),
                MSObj_GetWordImm(bp),
                OSYM_LOCLABEL,
                OSYM_NAMELESS,
                sdata);
 ...
 *
 ***********************************************************************/
static void BorlandEnterLocalAddrSymbol(
    const char* file, ID name, byte symType, word typeIndex, byte** bpPtr)
{
    /*
     * bp points to:
     ...
     */
    sym = BorlandEnterAddressSymbol(
        name, MSObj_GetWordImm(bp), symType, 0, sdata);
 ...
 *
 ***********************************************************************/
static void BorlandEnterLocalStackSymbol(
    const char* file, ID name, byte symType, word typeIndex, short offset)
{
    VMPtr sym;
    ObjSym* os;
    ObjSym* sos;
    ...
    os = BorlandAllocLocal(
        VMP_BLOCK(scopeStack[scopeTop - 1].scope), (ObjSym**)NULL, &sym);
    /*
    ...
    if (sos->u.scope.first != VMP_OFFSET(scopeStack[scopeTop - 1].scope))
    {
        ...
        for (los = (ObjSym*)((genptr)osh + sos->u.scope.first); los != sos;
             los = (ObjSym*)((genptr)osh + los->u.procLocal.next))
        {
            if (los->name == name)
            {
                 ...
                 */
                if (los->type != symType || los->u.localVar.type != typeIndex)
                {
                    los->type = symType;
 ...
 *
 *********************************************************************/
static void BorlandCreateLocalTypeSymbol(const char* file,
                     ...
                     * this is mapped */
{
    ...
    if (typeIndex < Vector_Length(borlandTypes))
    {
        ...
        if ((btp->desc[0] == BTID_ENUM) || (btp->desc[0] == BTID_STRUCT) ||
            (btp->desc[0] == BTID_UNION) || (btp->desc[0] == BTID_VLSTRUCT) ||
            (btp->desc[0] == BTID_VLUNION))
        {
            ...
            else
            {
                ...
                os = (ObjSym*)VMLockVMPtr(symbols,
                    MAKE_VMP(btp->ot.words[1], btp->ot.words[0]),
                    (MemHandle*)NULL);
 ...
 *
 ***********************************************************************/
static void BorlandProcessLocals(const char* file, word reclen, byte* bp)
{
    ID name;
    word typeIndex;
    byte* endRecord;
    ...
    again:
        switch (*bp++)
        {
            case BSC_STATIC: /* A static variable:
                              ...
                              */
                BorlandEnterLocalAddrSymbol(
                    file, name, OSYM_VAR, typeIndex, &bp);
                break;
            ...
            case BSC_AUTO: /* Local (auto) variable/parameter:
                            ...
                            */
                BorlandEnterLocalStackSymbol(
                    file, name, OSYM_LOCVAR, typeIndex, MSObj_GetWordImm(bp));
                break;
                              ...
                              */
            {
                ...
                bp += 1;
                BorlandEnterLocalStackSymbol(
                    file, name, OSYM_REGVAR, typeIndex, offset);
                break;
            }
            ...
            case BSC_FUNCTION: /* Static function:
                                ...
                                */
                BorlandEnterLocalAddrSymbol(
                    file, name, OSYM_PROC, typeIndex, &bp);
                break;
            case BSC_OPTIMIZED: /* Variable's been optimized:
                ...
                {
                    Notify(NOTIFY_ERROR,
                        "%s: cannot cope with more than one live range for "
                        "variable %i",
                        file,
                        name);
                    return;
        ...
        if ((borlandMajorVersion > 3) ||
            ((borlandMajorVersion == 3) && (borlandMinorVersion >= 1)))
        {
            /*
            ...
            {
                Notify(NOTIFY_ERROR,
                    "%s: glue can't deal with browser information. Please "
                    "recompile this file without passing the -R flag to BCC, "
                    "and try again.\n",
                    file);
            }
        }
 ...
 *
 ***********************************************************************/
static void BorlandProcessEndScope(const char* file, word reclen, byte* bp)
{
    assert(scopeTop != 0);
    ...
    if (scopeTop > 1)
    {
        ...
        endScope = BorlandEnterAddressSymbol(
            NullID, MSObj_GetWordImm(bp), OSYM_BLOCKEND, 0, sdata);
 ...
 *
 ***********************************************************************/
static void BorlandProcessSourceFile(const char* file, word reclen, byte* bp)
{
    word index;
 ...
 *
 ***********************************************************************/
static void BorlandProcessDependency(const char* file, word reclen, byte* bp) {}
 ...
 *
 ***********************************************************************/
static void BorlandProcessCompilerDesc(const char* file, word reclen, byte* bp)
{
    /*
 ...
 *
 ***********************************************************************/
static void BorlandProcessPublicByName(const char* file, word reclen, byte* bp)
{
    byte* endRecord = bp + reclen;
 ...
 *
 ***********************************************************************/
static void BorlandProcessExternalByName(
    const char* file, word reclen, byte* bp)
{
    byte* endRecord = bp + reclen;
    ObjSymHeader* osh;
    ...
    osh = (ObjSymHeader*)VMLock(
        symbols, VMP_BLOCK(lastExternal), (MemHandle*)NULL);
    while (bp < endRecord)
    {
        ID name = ST_Enter(symbols, strings, (char*)bp + 1, *bp);
        ObjSym* os;
...
static void BorlandInit(void)
{
  ...
  /* BT_TBYTE */
     ...
     */
    borlandSegs = Vector_Create(sizeof(BorlandSegData), ADJUST_ADD, 10, 10);
    borlandSources = Vector_Create(sizeof(ID), ADJUST_ADD, 10, 10);
    borlandTypes = Vector_Create(
        sizeof(BorlandType), ADJUST_MULTIPLY, BT_LAST_PREDEF + 1, 2);

    borlandLocalStatics =
        Vector_Create(sizeof(BorlandLocalStatic), ADJUST_ADD, 10, 10);

    borlandMembers =
        Vector_Create(sizeof(BorlandMembers), ADJUST_MULTIPLY, 10, 2);
    /*
     * Initialize all the pre-defined types
     */
    for (i = sizeof(predefs) / sizeof(predefs[0]); i > 0; i--)
    {
 ...
 *
 ***********************************************************************/
int Borland_Check(const char* file,
    byte rectype,
    word reclen,
    byte* bp,
 ...
 *
 ***********************************************************************/
static word BorlandCopyTypeDesc(int index, /* Index of type to copy */
    ...
    Boolean isLMem)
{
    ...
    else
    {
        ...
        else if (OTYPE_IS_ARRAY(btp->ot.words[0]))
        {
            ...
            if (isLMem && ((btp->size / baseBTP->size) == 1) &&
            ...
            else
            {
                ...
                retval = MSObj_CreateArrayType(
                    types, base, btp->size / baseBTP->size);
            }
        }
        else
        {
            ...
            assert((btp->desc[0] == BTID_VLSTRUCT) ||
                   ...
                   (btp->desc[0] == BTID_VLUNION) ||
                   (btp->desc[0] == BTID_UNION) || (btp->desc[0] == BTID_ENUM));
 ...
 *
 ***********************************************************************/
static void BorlandConvertSymbolBlock(VMBlockHandle cur,
    ObjSymHeader* osh,
    ...
    for (i = osh->num; i > 0; i--, os++)
    {
        switch (os->type)
        {
            ...
            case OSYM_TYPEDEF: /* typeDef.type */
                 ...
                 */
                os->u.typeDef.type =
                    BorlandCopyTypeDesc(os->u.typeDef.type, types, isLMem);
                break;
            ...
            case OSYM_BITFIELD:
                ...
                if (!isUnion && ((i == 1) || (os[1].type != OSYM_BITFIELD) ||
                                    ((os[1].u.sField.type & OTYPE_BF_OFFSET) <=
                                        (os->u.sField.type & OTYPE_BF_OFFSET))))
                {
                    int fieldWidth;

                    fieldWidth = ((os->u.sField.type & OTYPE_BF_OFFSET) >>
                                     OTYPE_BF_OFFSET_SHIFT) +
            ...
            case OSYM_UNKNOWN:
            {
                    ...
                    os->u.variable.type < Vector_Length(borlandTypes))
                {
                    ...
                    else
                    {
                         ...
                         */
                        os->type = OSYM_VAR;
                        os->u.variable.type = BorlandCopyTypeDesc(
                            os->u.variable.type, types, isLMem);
                    }
                }
                break;
            }
...
static void BorlandEnsureTypeConverted(int i, Boolean isLMem)
{
    ...
    switch (btp->desc[0])
    {
        ...
        case BTID_UNION:
        {
            ...
            if (os->flags & OSYM_NAMELESS)
            {
                MSObj_AddAnonStruct(os,
                    types,
                    os->u.sType.size,
                    ((os->u.sType.last - os->u.sType.first) / sizeof(ObjSym)));
                OTYPE_ID_TO_STRUCT(os->name, &btp->ot);
            }
            VMUnlockDirty(symbols, block);
            break;
        ...
        case BTID_ENUM:
        {
             ...
             */
            VMBlockHandle block = btp->ot.words[1];
            ObjSym* osp = (ObjSym*)VMLockVMPtr(
                symbols, MAKE_VMP(block, btp->ot.words[0]), (MemHandle*)NULL);
 ...
 *
 ***********************************************************************/
static void BorlandConvertSymbolTypes(const char* file, /* Object file name */
 ...
 *
 ***********************************************************************/
static void BorlandProcessAllTypes(const char* file)
{
    /*
 ...
 *
 ***********************************************************************/
static void BorlandProcessAllExternals(const char* file)
{
    if (lastExternal != NullVMPtr)
    {
        ObjSymHeader* osh;
 ...
 *
 ***********************************************************************/
static void BorlandProcessAllPublics(const char* file)
{
    int i;
    int max;
    BorlandSegData* sdata;
    ...
    for (i = 0, sdata = (BorlandSegData*)Vector_Data(borlandSegs); i < max;
         i++, sdata++)
    {
        if (sdata->addrH != 0)
        {
             ...
             */
            BorlandConvertSymbolTypes(
                file, sdata->addrH, sd->combine == SEG_LMEM);
       ...
       * foo, data is _foo) */
            ...
            else
            {
                 ...
                 */
                osh =
                    (ObjSymHeader*)VMLock(symbols, sd->addrT, (MemHandle*)NULL);
 ...
 *
 ***********************************************************************/
static void BorlandNukeTypes(void)
{
    VMBlockHandle next, cur;
    ObjSymHeader* osh;
