...
#define MSL_BLOCK_FULL                                \
    255 /* Value placed in byte after the last bucket \
         ...
         * to lack of space. */
#define MSL_FIRST_FREE                               \
    37 /* Offset of the first-free-word index in the \
        * block */
#define MSL_BLOCK_SIZE                                   \
    512 /* Size of a dictionary block. This is different \
         * from the page size of the library, of         \
         * course */

#define MSL_USE_HASH                               \
    FALSE /* Define TRUE if we ever find the right \
           * hash function. */
 ...
 *
 ***********************************************************************/
void Pass1MSL_Load(const char* file, genptr handle)
{
    FILE* f = (FILE*)handle;
    word pageSize;
    dword dictStart;
    ...
    for (sup = *prev; sup != NULL; sup = *prev)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    if ((*bp++ == nameLen) && (strncmp(name, bp, nameLen) == 0))
                    {
                        /*
                        ...
                        offset =
                            (bp[nameLen] | (bp[nameLen + 1] << 8)) * pageSize;
                        ...
                        if (numOffs >= 0)
                        {
                            Notify(NOTIFY_WARNING,
                                "%s: symbol %s should have been defined by "
                                "previous read of module at %ld\n",
                                file,
                                name,
                                offset);
                            break;
        ...
        for (block = 0, blockBuf = dict; block < dictLen && !found;
 ...
 *
 ***********************************************************************/
void Pass2MSL_Load(const char* file, genptr handle)
{
    FILE* f = (FILE*)handle;
    MSLib* lib;
    int n;
