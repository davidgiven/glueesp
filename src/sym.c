 ...
 *
 ***********************************************************************/
VMBlockHandle Sym_Create(VMHandle file)
{
    int size;
    /*
 ...
 *
 ***********************************************************************/
void Sym_Close(VMHandle file, VMBlockHandle table)
{
    ObjHashHeader* hdr;
    int i;
    int obj_hash_chains;
    ...
    for (i = 0; i < obj_hash_chains; i++)
    {
        if (hdr->chains[i] != 0)
        {
            ...
            (void)MemReAlloc(
                mem, (genptr)&hb->entries[hb->nextEnt] - (genptr)hb, 0);
 ...
 *
 ***********************************************************************/
void Sym_Destroy(VMHandle file, VMBlockHandle table)
{
    ObjHashHeader* hdr;
    int i;
    int obj_hash_chains;
 ...
 *
 ***********************************************************************/
static int SymLookup(VMHandle file,
    VMBlockHandle table,
    ID id,
 ...
 *
 ***********************************************************************/
static void SymCompareSyms(VMHandle file, /* Output file */
                           ...
                           * ObjType's for sym2 */
{
    ...
    if ((sym1->type == OSYM_VAR) &&
        ((sym2->type == OSYM_CLASS) || (sym2->type == OSYM_MASTER_CLASS) ||
    ...
    if ((sym1->type != sym2->type) &&
        !((sym2->type == OSYM_VAR) &&
            ((sym1->type == OSYM_CLASS) || (sym1->type == OSYM_MASTER_CLASS) ||
 ...
 *
 ***********************************************************************/
static int SymCheckUndef(VMHandle file, /* File containing sym */
                                         ...
                                         * ObjType records required by sym */
{
    ...
    else if ((u->sym.type == OSYM_UNKNOWN) && (sym->type != OSYM_UNKNOWN))
    {
        /*
 ...
 *
 ***********************************************************************/
void Sym_Enter(VMHandle file, /* Output file */
        ...
        if (vmid == OID_SYM_BLOCK)
        {
            ...
            if (os->flags & OSYM_GLOBAL)
            {
                ...
                for (i = 0; i < seg_NumSegs; i++)
                {
                    ...
                    if ((sd->syms != table) && SymLookup(symbols,
                                                   sd->syms,
                                                   id,
                                                   &obucket,
                                                   &oblock,
                                                   ...
                                                   &ohe))
                    {
                        ...
                        osym = (ObjSym*)((genptr)VMLock(symbols,
                                             ohe->block,
                                             (MemHandle*)NULL) +
                                         ohe->offset);
...
int Sym_FindWithSegmentAndFile(VMHandle file, /* File in which to
                                               * search */
...
int Sym_FindWithSegment(VMHandle file,
    ID id,
    ...
    SegDesc** sdPtr)
{
    return Sym_FindWithSegmentAndFile(
        file, 0, id, symBlockPtr, symOffPtr, globalOnly, sdPtr);
}

/***********************************************************************
 *				Sym_FindWithFile
 ***********************************************************************
 * SYNOPSIS:	    Find a symbol in from a given object file, if it's there if
 *non-Global. Filename search only takes effect if isGlobal is false CALLED BY:
 *EXTERNAL RETURN:	    non-zero and block/offset if defined. 0 if not. SIDE
 *EFFECTS:    None
 *
 * STRATEGY:
 *
...
int Sym_FindWithFile(VMHandle file, /* File in which to search */
                                     ...
                                     * acceptable */
{
    ...
    return Sym_FindWithSegmentAndFile(
        file, fileName, id, symBlockPtr, symOffPtr, globalOnly, &sd);
}
 ...
 *
 ***********************************************************************/
int Sym_Find(VMHandle file,     /* File in which to search */
    VMBlockHandle table,        /* Table in which to look. If NULL,
                                 * searches through the tables of all
                                 * known segments */
    ID id,                      /* Name of symbol to find */
                                 ...
                                 * acceptable */
{
    ...
    if (table == 0)
    {
        ...
        return (Sym_FindWithSegment(
            file, id, symBlockPtr, symOffPtr, globalOnly, &sd));
    }
    else
    {
 ...
 *
 ***********************************************************************/
void Sym_EnterUndef(VMHandle file, /* Output file */
                                    ...
                                    * records required by the symbol */
{
    if (!SymCheckUndef(file, table, id, os, symOff, tfile, types))
    {
        ...
        else
        {
            ...
            u = (SymUndef*)malloc(
                sizeof(SymUndef) + (ntypes * sizeof(ObjType)));
            u->table = table;
            u->sym = *os;
            u->next = symUndefHead;
            symUndefHead = u;
                ...
                while (tnum != ntypes)
                {
                    ...
                    else
                    {
                        ...
                        if (OTYPE_IS_STRUCT(t->words[0]))
                        {
                             ...
                             */
                            ID id = ST_Dup(
                                tfile, OTYPE_STRUCT_ID(t), symbols, strings);
 ...
 *
 ***********************************************************************/
void Sym_AllocCommon(SegDesc* sd)
{
    ...
    for (u = *prev; u != (SymUndef*)NULL; u = *prev)
    {
        if ((u->table == sd->syms) && (u->sym.type == OSYM_VAR))
        {
            ...
            sd->size += Obj_TypeSize(u->sym.u.variable.type, (void*)u, FALSE);
            numAddrSyms += 1;
            ...
            if (symBlock == 0)
            {
                ...
                if (sd->addrT != 0)
                {
                    ...
                    addrT = (ObjSymHeader*)VMLock(
                        symbols, sd->addrT, (MemHandle*)NULL);
                    addrT->next = symBlock;
