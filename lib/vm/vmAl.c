 ...
 ***********************************************************************/
#ifndef lint
static char* rcsid = "$Id: vmAl.c,v 1.2 96/05/20 18:58:01 dbaumann Exp $";
#endif lint
...
VMBlockHandle VMAlloc(
    VMHandle vmHandle, /* File in which to allocate the block */
                        ...
                        * don't care) */
{
    ...
    block->VMB_pos = 0;
    block->VMB_uid = id;
    block->VMB_memHandle = MemAlloc(
        numBytes, HF_SWAPABLE | HF_SHARABLE, HAF_ZERO_INIT | HAF_NO_ERR);
