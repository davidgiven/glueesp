 ...
 *
 ***********************************************************************/
static void VMQueueWrite(int offset, int size, genptr block)
{
    VMQueue* q;
    VMQueue* pred;
    ...
    for (pred = writeQueue.prev; pred != &writeQueue && pred->fileOff > offset;
         pred = pred->prev)
    {
        ;
    }
 ...
 *
 ***********************************************************************/
static void VMFlushWrites(VMFilePtr file)
{
    genptr buf;
    int inbuf;
    struct stat stb;
    ...
    for (q = writeQueue.next; q != &writeQueue; q = nq)
    {
        if (offset != q->fileOff)
        {
            ...
            FileUtil_Seek(file->fd,
                ((file->flags & VM_2_0) ? q->fileOff + sizeof(GeosFileHeader2)
                                        : q->fileOff),
                SEEK_SET);
            offset = q->fileOff;
        }
 ...
 *
 ***********************************************************************/
void VMSwapHeader(VMHeader* hdr, word size)
{
 ...
 *
 ***********************************************************************/
int VMWriteBlock(VMFilePtr file, /* File to which block belongs */
 ...
 *
 ***********************************************************************/
int VMUpdate(VMHandle vmHandle)
{
