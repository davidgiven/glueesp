 ...
 ***********************************************************************/
#ifndef lint
static char* rcsid = "$Id: vmAlRd.c,v 1.4 92/07/17 19:35:10 adam Exp $";
#endif lint
 ...
 *
 ***********************************************************************/
MemHandle VMAllocAndRead(VMFilePtr file, /* File from which to read it */
    ...
    word size)                           /* Size of block to read */
{
    ...
    /*XXX: check return */
    memHandle =
        MemAllocAndLock(size, HF_SWAPABLE | HF_SHARABLE, HAF_NO_ERR, &addr);

    if (FileUtil_Seek(file->fd,
            ((file->flags & VM_2_0) ? pos + sizeof(GeosFileHeader2) : pos),
            SEEK_SET) == -1)
    {
        char errmsg[512];

        FileUtil_SprintError(
            errmsg, "cannot seek in vmfile \"%s\"", file->name);
    ...
    while (lastError == EINTR)
    {
        ...
        {
            __eprintf(
                "VM file %s has been deleted -- no longer able "
                "to read it\n",
                file->name ?: "(unknown)");
        }
        else if (errno == EINTR)
...
#endif
        FileUtil_SprintError(
            errmsg, "cannot read from vmfile \"%s\"", file->name);
        fprintf(stderr, "%s", errmsg);
        MemFree(memHandle);
        return (0);
    }
