 ...
 * initially */
#define VM_EXTEND_NUM_BLKS                 \
    3 /* Number of VM block handles to add \
       * when need to extend the table */
#define VM_DEF_COMPACT_THRESH                \
    70 /* Default compaction threshold (just \
...
#define VM_DIRTY_BLK_SIG                \
    0xfe /* Signature if dirty (also an \
          * illegal handle) */
...
#define VMBF_HAS_BACKUP              \
    0x02 /* Block has backup version \
          * (unused here) */
#define VMBF_PRESERVE                                      \
    0x04                /* Preserve block's handle while   \
...
#define VM_HANDLE_TO_BLOCK(hid, hdr)                                    \
    (assert(((hid) >= VM_HEADER_ID && (hid) < (hdr)->VMH_lastHandle) || \
            (hid) == 0),                                                \
...
#define VM_LAST(file) \
    ((VMBlock*)((char*)(file)->blkHdr + (file)->blkHdr->VMH_lastHandle))
...
#define VM_TEMP_FILE                                      \
    0x00000002 /* File is temporary and should be removed \
                * on close */
#define VM_SHRINK_HDR                                                \
    0x00000004            /* Header was allocated big when file was  \
