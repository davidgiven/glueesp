 ...
 *
 ***********************************************************************/
static int VMInitFile(VMFilePtr file)
{
    ...
    hdrHandle = MemAllocAndLock(
        size, HF_SWAPABLE | HF_SHARABLE, HAF_ZERO_INIT, (void**)&hdr);
    ...
    if (geosRelease > 1)
    {
        ...
        {
            VMCopyToDBCSString(
                file->fileHdr.v2.VMFH_gfh.longName, cp, GFH_LONGNAME_SIZE);
        }
        else
        {
    ...
    returnCode = FileUtil_Write(
        file->fd, (unsigned char*)&file->fileHdr, file->fsize, &bytesWritten);
    if (returnCode == FALSE)
    {
        char errmsg[512];
 ...
 *
 ***********************************************************************/
VMHandle VMOpen(short flags, /* Flags for open */
    ...
    short* status)           /* RETURN: status */
{
    ...
    switch (flags & VMO_OPEN_TYPE)
    {
        case VMO_TEMP_FILE:
        {
            ...
            {
                returnCode = FileUtil_Open(
                    &(file->fd), file->name, oflags, sflags, 0666);
                if (returnCode == TRUE)
                {
                    break;
                }
        ...
        decode_access:
            ...
            returnCode =
                FileUtil_Open(&(file->fd), fileName, oflags, sflags, 0666);
            if (returnCode == FALSE)
            {
                /*
    ...
    switch (bytesRead)
    {
        ...
        case sizeof(file->fileHdr.v2):
            ...
            if (file->blkHdr->VMH_signature == VM_HEADER_SIG)
            {
                 ...
                 */
                for (block = &file->blkHdr->VMH_blockTable[1]; block < end;
                     block++)
                {
...
int VMCopyToDBCSString(char* dest, char* source, int max)
{
    int si = 0, di = 0;
