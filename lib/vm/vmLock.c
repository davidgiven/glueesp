 ...
 ***********************************************************************/
#ifndef lint
static char* rcsid = "$Id: vmLock.c,v 1.6 92/06/17 17:41:27 jimmy Exp $";
#endif lint
 ...
 *
 ***********************************************************************/
genptr VMLock(VMHandle vmHandle, /* File to which block belongs */
    ...
    MemHandle* handlePtr)        /* Place to store memory handle */
{
    ...
    if (block->VMB_memHandle == 0)
    {
        ...
        {
            block->VMB_memHandle =
                VMAllocAndRead(file, block->VMB_pos, block->VMB_size);
            /*
