 ...
 */
extern VMHandle VMOpen(
    short flags, short compress, const char* fileName, short* status);
/* FLAGS argument: */
#define VMO_OPEN_TYPE 0xff00 /* Mask for type of open: */
#define VMO_CREATE_TRUNCATE                    \
    0x0400 /* Truncate file if it exists, else \
            * create it */
#define VMO_CREATE_ONLY                      \
    0x0300 /* Can only create the file -- it \
            * cannot have existed before the \
            * VMOpen call */
#define VMO_TEMP_FILE                             \
    0x0200 /* Opening temp file (file removed on  \
            * close). fileName is actually the    \
            * directory in which a uniquely named \
            * file will be created */
#define VMO_CREATE                                          \
    0x0100              /* Create the given file if doesn't \
                         * exist. */
...
#define VM_SHARING_DENIED                      \
    0x0083 /* Open unsuccessful -- conflicting \
            * SHARE_MODEs */

/*
 * Lock a VM block down, returning its address and handle.
 */
extern genptr VMLock(
    VMHandle vmHandle, VMBlockHandle vmBlock, MemHandle* handlePtr);
...
extern void VMEmpty(VMHandle vmHandle, VMBlockHandle vmBlock);
/*
 ...
 */
extern VMBlockHandle VMAlloc(VMHandle vmHandle, int numBytes, VMID id);
/*
 * Free a VM block and its data
 */
extern void VMFree(VMHandle vmHandle, VMBlockHandle vmBlock);

/*
 * Mark a VM block as dirty
 */
extern void VMDirty(VMHandle vmHandle, VMBlockHandle vmBlock);
...
extern void VMSetMapBlock(VMHandle vmHandle, VMBlockHandle vmBlock);
...
extern void VMSetDBMap(VMHandle vmHandle, VMBlockHandle vmBlock);
 ...
 */
extern void VMSetReloc(VMHandle vmHandle, VMRelocRoutine* reloc);
...
extern void VMSetAttributes(VMHandle vmHandle, byte set, byte reset);
#define VMA_SYNC_UPDATE                     \
    0x80 /* Allow synchronous updates only. \
          * Tells the system it may not     \
          * write dirty blocks to the file  \
          * except via a VMUpdate */
#define VMA_BACKUP                        \
    0x40 /* Maintain a backup copy of all \
          * data */
#define VMA_OBJECT_RELOC                   \
    0x20 /* Use built-in object relocation \
          * routines to relocate and       \
          * unrelocate blocks as they      \
          * come in and go out */
#define VMA_PRESERVE_HANDLES                \
    0x10 /* Preserve memory handles while   \
          * file is open, even if block     \
          * is written to disk or discarded \
          */
#define VMA_NOTIFY_DIRTY                   \
    0x08 /* Notify when first block marked \
          * dirty */
/*
 * Mark a VM block as containing an LMem heap
 */
extern void VMSetLMemFlag(VMHandle vmHandle, VMBlockHandle vmBlock);
/*
 * Mark a VM block as needing to have its memory handle preserved.
 */
extern void VMSetPreserveFlag(VMHandle vmHandle, VMBlockHandle vmBlock);
 ...
 */
extern void VMAttach(VMHandle vmHandle, VMBlockHandle vmBlock, MemHandle mem);
...
#define DBGF_IS_UNGROUP                                               \
    0x8000                   /* Set if the group is the "ungroup" for \
                              * the file */
 ...
 */
extern int VMCopyToDBCSString(char* dest, char* source, int max);
