 ...
 *
 ***********************************************************************/
void VMLinkNewBlocks(VMHeader* hdr, /* Header for file */
 ...
 *
 ***********************************************************************/
VMBlock* VMAllocUnassigned(VMFilePtr file)
{
    ...
    if (hdr->VMH_numUnassigned == 0)
    {
        ...
        {
            printf(
                "Have exceeded glue's handle table limit!  Try reducing the "
                "number of resources.");
        }
        VMLinkNewBlocks(
            hdr, VM_HANDLE_TO_BLOCK(firstHandle, hdr), VM_EXTEND_NUM_BLKS);
    }
