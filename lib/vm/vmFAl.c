 ...
 ***********************************************************************/
#ifndef lint
static char* rcsid = "$Id: vmFAl.c,v 1.2 92/06/02 21:18:20 adam Exp $";
#endif lint
 ...
 *
 ***********************************************************************/
dword VMFileAlloc(VMFilePtr file, /* File in which to allocate room */
    word size)                    /* Size of area required */
{
    ...
    for (cur = VM_HANDLE_TO_BLOCK(*curP, file->blkHdr); cur != VM_NULL(file);
    ...
    if (cur == VM_NULL(file))
    {
         ...
         * and have done.
         */
        pos = ((file->flags & VM_2_0) ? file->fsize - sizeof(GeosFileHeader2)
                                      : file->fsize);
