 ...
 *	Name	  	    Description
 *	----	  	    -----------
 *	gprintf	    	    Print to stdout
 *	fgprintf	    	    Print to a stream
 *	sgprintf	    	    Print to a string
 *	vgprintf	    	    Print varargs to stdout
 *	vfgprintf    	    Workhorse -- print varargs to a stream
 *	vsgprintf    	    Print varargs to a string.
 *
 * REVISION HISTORY:
 ...
 *
 * DESCRIPTION:
 *	Specialized implementation of gprintf functions to deal with
 *	identifiers.
 *
 * 	All these functions are based around a modified vfgprintf written
...
static char* CvtUtoA(    /*i, base, buf, lengthPtr) */
 ...
 *	formatted information.  It is called by gprintf, fgprintf,
 *	sgprintf, vgprintf, and vsgprintf.
 *
 * Results:
...
int i_vfgprintf(FILE* stream, /* Where to output formatted results. */
                                 ...
                                 to a buffer pointer */
{
    ...
    for (c = *format; c != 0; format++, c = *format)
    {
        ...
        switch (c)
        {
            ...
            case 'U':
            case 'u':
                field = CvtUtoA(va_arg(args, unsigned), 10, buf, &actualLength);
                break;
...
int vfgprintf(FILE* stream, /* Where to output formatted results. */
    const char* format,     /* Contains literal text and format control
                             * sequences indicating how args are to be
                             * printed.  See the man page for details. */
    va_list args)           /* Variable number of values to be formatted
                             * and printed. */
{
    return i_vfgprintf(stream, format, args, TRUE);
}
 ...
 *
 ***********************************************************************/
int vgprintf(const char* fmt, va_list args)
{
    return (vfgprintf(stdout, fmt, args));
}
 ...
 *
 ***********************************************************************/
int vsgprintf(char* str, const char* fmt, va_list args)
{
    int res;
    char* strPtr = str;

    res = i_vfgprintf((FILE*)&strPtr, fmt, args, FALSE);
 ...
 *
 ***********************************************************************/
int fgprintf(FILE* stream, const char* fmt, ...)
{
    va_list args;
    int res;
 ...
 *
 ***********************************************************************/
int sgprintf(char* str, const char* fmt, ...)
{
    va_list args; /* List of passed args */
    int res;      /* Result of vfgprintf */
    char* strPtr = str;
