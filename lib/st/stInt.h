 ...
 */
#define ST_NEXT_CP(stcp, len) \
    ((STChainPtr)((stcp)->string + (((len) + 2) & ~1)))
 ...
 */
#define ST_NUM_BUCKETS                           \
    257 /* Number of buckets in the header block \
         * for the table */
#define ST_HASH_TO_BUCKET(val) ((val) % ST_NUM_BUCKETS)

#define ST_INIT_CHAIN_BYTES 1024 /* Initial size of a chain block */
#define ST_INCR_CHAIN_BYTES                     \
    512 /* Number of bytes by which to increase \
         * a chain block */
