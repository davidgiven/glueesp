 ...
 */
extern ID ST_Enter(VMHandle vmHandle, VMBlockHandle table, char* name, int len);
/*
 * Similar to above, but calls strlen(name) before calling ST_Enter.
 * This is a code-saving device, no more.
 */
extern ID ST_EnterNoLen(VMHandle vmHandle, VMBlockHandle table, char* name);
 ...
 */
extern ID ST_Lookup(
    VMHandle vmHandle, VMBlockHandle table, const char* name, int len);
/*
 * Similar to above, but calls strlen(name) before calling ST_Lookup.
 * This is a code-saving device, no more.
 */
extern ID ST_LookupNoLen(
    VMHandle vmHandle, VMBlockHandle table, const char* name);
...
#define ST_Lock(vmHandle, id)                                         \
    ((char*)VMLock(                                                   \
         (vmHandle), (VMBlockHandle)((id) >> 16), (MemHandle*)NULL) + \
        ((id)&0xffff))

/*
 * Release a string after it's been used
 */
#define ST_Unlock(vmHandle, id) VMUnlock((vmHandle), (id))
 ...
 */
extern word ST_Index(VMHandle vmHandle, ID id);
 ...
 */
extern int ST_Close(VMHandle vmHandle, VMBlockHandle table);

/*
 * Destroy a string table, freeing up all its VM blocks.
 */
extern void ST_Destroy(VMHandle vmHandle, VMBlockHandle table);
