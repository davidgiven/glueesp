...
#define GA_REQUIRES_COPROC                                                 \
    0x0010                                  /* Requires coprocessor, if no \
                                             * emulator present */
...
#define PROCESS_DEF_STACK_SIZE                 \
    2000 /* Default stack size for the initial \
          * thread of a geode that is a process */
#define INTERFACE_THREAD_DEF_STACK_SIZE                       \
    2000                    /* Default stack size for the     \
...
typedef struct geodeHeaderStruc
{ /* GEODE header */
...
typedef struct geodeHeaderStruc2
{ /* GEODE header */
...
#define RESF_DISCARDED                        \
    0x0002 /* Needs to be brought in from the \
            * executable */
#define RESF_DEBUG                                                       \
    0x0004                      /* Attached to debugger (should never be \
                                 * set) */
...
#define RESF_CONFORMING                                             \
    0x0100                    /* May be called from lower privilege \
                               * level */
...
#define RESF_UI                                    \
    0x1000 /* Combined with RESF_OBJECT, indicates \
...
#define RESF_PRELOAD                              \
    0x0030 /* Preloaded standard (discardable and \
            * swapable, but not discarded) */
#define RESF_STANDARD                                             \
    0x0032                  /* Standard (discardable and swapable \
                             * but discarded) */
