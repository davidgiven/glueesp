...
#define swapsp(p)                                     \
    {                                                 \
        unsigned char *_cp = (unsigned char*)(p), _c; \
        _c = *_cp++;                                  \
        _cp[-1] = *_cp;                               \
        *_cp = _c;                                    \
    }
 ...
 *
 ***********************************************************************/
static inline void ObjSwapObjRel(word** wPtr)
{
    byte b;
    int one = 1;
 ...
 *
 ***********************************************************************/
void ObjSwap_Header(ObjHeader* hdr)
{
 ...
 *
 ***********************************************************************/
static void ObjSwapMapBlock(VMHandle vmHandle, /* File from whence it comes */
 ...
 *
 ***********************************************************************/
static void ObjSwapSrcMap(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                   /* The block's VM handle */
    VMID vmID,                               /* Its vm ID */
    MemHandle handle,                        /* Its memory handle */
    genptr block)                            /* The base of the locked block */
 ...
 *
 ***********************************************************************/
static void ObjSwapAddrMap(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                    /* The block's VM handle */
    VMID vmID,                                /* Its vm ID */
    MemHandle handle,                         /* Its memory handle */
    genptr block)                             /* The base of the locked block */
 ...
 *
 ***********************************************************************/
static void ObjSwapLineBlock(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                      /* The block's VM handle */
    VMID vmID,                                  /* Its vm ID */
    MemHandle handle,                           /* Its memory handle */
    genptr block) /* The base of the locked block */
 ...
 *
 ***********************************************************************/
static void ObjSwapTypeBlock(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                      /* The block's VM handle */
    VMID vmID,                                  /* Its vm ID */
    MemHandle handle,                           /* Its memory handle */
    genptr block) /* The base of the locked block */
 ...
 *
 ***********************************************************************/
static void ObjSwapCodeBlock(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                      /* The block's VM handle */
    VMID vmID,                                  /* Its vm ID */
    MemHandle handle,                           /* Its memory handle */
    genptr block) /* The base of the locked block */
 ...
 *
 ***********************************************************************/
static void ObjSwapHashHeadBlock(
 ...
 *
 ***********************************************************************/
static void ObjSwapHashHeadBlock_NewFormat(
    VMHandle vmHandle,     /* File from whence it comes */
    VMBlockHandle vmBlock, /* The block's VM handle */
    VMID vmID,             /* Its vm ID */
    MemHandle handle,      /* Its memory handle */
 ...
 *
 ***********************************************************************/
static void ObjSwapHashBlock(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                      /* The block's VM handle */
    VMID vmID,                                  /* Its vm ID */
    MemHandle handle,                           /* Its memory handle */
    genptr block) /* The base of the locked block */
 ...
 *
 ***********************************************************************/
static void ObjSwapSymBlock(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                     /* The block's VM handle */
    VMID vmID,                                 /* Its vm ID */
    MemHandle handle,                          /* Its memory handle */
    genptr block) /* The base of the locked block */
 ...
 *
 ***********************************************************************/
static void ObjSwapRelBlock(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                     /* The block's VM handle */
    VMID vmID,                                 /* Its vm ID */
    MemHandle handle,                          /* Its memory handle */
    genptr block) /* The base of the locked block */
 ...
 *
 ***********************************************************************/
void ObjSwap_Reloc(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,            /* The block's VM handle */
    VMID vmID,                        /* Its vm ID */
    MemHandle handle,                 /* Its memory handle */
    genptr block)                     /* The base of the locked block */
{
    ...
    {
        (*relocRoutines[vmID - OID_STRING_HEAD])(
            vmHandle, vmBlock, vmID, handle, block);
    }
}
 ...
 *
 ***********************************************************************/
void ObjSwap_Reloc_NewFormat(VMHandle vmHandle, /* File from whence it comes */
    VMBlockHandle vmBlock,                      /* The block's VM handle */
    VMID vmID,                                  /* Its vm ID */
    MemHandle handle,                           /* Its memory handle */
    genptr block) /* The base of the locked block */
{
    ...
    if (vmID >= OID_STRING_HEAD && vmID <= OID_SRC_BLOCK)
    {
        (*relocRoutines_NewFormat[vmID - OID_STRING_HEAD])(
            vmHandle, vmBlock, vmID, handle, block);
    }
}
