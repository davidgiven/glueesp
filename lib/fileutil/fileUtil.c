 ...
 ***********************************************************************/
int FileUtil_Open(
    FileType* file, const char* path, int oflags, int sflags, int mode)
{
#if defined(unix) || defined(_LINUX)
    char flags[5];
    ...
    *file = (HANDLE)CreateFile(
        path, winOFlags, winSFlags, NULL, winCreate, 0, NULL);
 ...
 ***********************************************************************/
int FileUtil_Write(
    FileType file, const unsigned char* buf, long len, long* nWrit)
{
#if defined(unix) || defined(_LINUX)
...
#elif defined(_WIN32)
    returnCode =
        GetFileTime((HANDLE)file, &ftCreation, &ftLastAccess, &ftLastWrite);
    if (returnCode == TRUE)
    {
...
void FileUtil_SprintError(char* result, char* fmt, ...)
{
    ...
    *result = '\0';
    ...
    sprintf(result, ": %serror #%d\n", (char*)lpMessageBuffer, GetLastError());
 ...
 *
 ***********************************************************************/
int FileUtil_TestFat(const char* absPath)
{
    char root[10];
    BOOL success;
    DWORD mcl;
    ...
    success = GetVolumeInformation(root, NULL, 0, NULL, &mcl, NULL, NULL, 0);
    if (success == TRUE)
    {
        if (mcl > 12)
        {
