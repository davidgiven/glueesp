...
static Hash_Entry* ChainSearch(
    Hash_Table* tablePtr, register Address key, register HashLink* hashList);
...
void Hash_InitTable(register Hash_Table* tablePtr, /* Structure to use to hold
                                                    * table. */
                  ...
                  * expanded */
{
    ...
    tablePtr->bucketPtr = (HashLink*)calloc(tablePtr->size, sizeof(HashLink));
...
void Hash_DeleteTable(Hash_Table* tablePtr) /* Hash table whose entries are all
                                             * to be freed.  */
{
...
Hash_Entry* Hash_FindEntry(Hash_Table* tablePtr, /* Hash table to search. */
    Address key)                                 /* A hash key. */
{
    return (ChainSearch(
        tablePtr, key, &(tablePtr->bucketPtr[Hash(tablePtr, key)])));
}
    ...
    switch (tablePtr->keyType)
    {
        case HASH_STRING_KEYS:
        {
            ...
            if (len > 4)
            {
                 ...
                 */
                hashEntryPtr =
                    (Hash_Entry*)malloc(sizeof(Hash_Entry) + len - 4);
        ...
        default:
        {
            ...
            n = tablePtr->keyType;
            hashEntryPtr =
                (Hash_Entry*)malloc(sizeof(Hash_Entry) + (n - 1) * sizeof(int));
            hashKeyPtr = hashEntryPtr->key.words;
            do
            {
                *hashKeyPtr++ = *keyPtr++;
...
Hash_Entry* Hash_EnumFirst(Hash_Table* tablePtr, /* Table to be searched. */
...
Hash_Entry* Hash_EnumNext(
    register Hash_Search* hashSearchPtr) /* Area used to keep state
                                          * about search. */
{
    register Hash_Entry* hashEntryPtr;
...
void Hash_PrintStats(Hash_Table* tablePtr, /* Table for which to print info. */
...
static Hash_Entry* ChainSearch(Hash_Table* tablePtr, /* Hash table to search. */
...
static void RebuildTable(Hash_Table* tablePtr) /* Table to be enlarged. */
{
    int oldSize;
    int bucket;
    ...
    Hash_InitTable(
        tablePtr, tablePtr->size * 4, tablePtr->keyType, tablePtr->limit);
    ...
    for (oldHashList = oldBucketPtr; oldSize > 0; oldSize--, oldHashList++)
    {
        while (oldHashList->next != (Hash_Entry*)oldHashList)
        {
            ...
            insque((struct qelem*)&hashEntryPtr->links,
                (struct qelem*)&tablePtr->bucketPtr[bucket]);
            tablePtr->numEntries++;
        }
    }
