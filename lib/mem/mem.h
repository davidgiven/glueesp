...
#define HF_SHARABLE                                                    \
    0x0040                    /* May be shared by other (non-existent) \
                               * processes */
#define HF_DISCARDABLE 0x0020 /* Block may be discarded (but won't be) */
#define HF_SWAPABLE                                              \
    0x0010                  /* Block may be swapped (and may be, \
                             * but not by us) */
...
#define HF_SWAPPED                               \
    0x0001 /* Block has been swapped to extended \
...
#define HAF_LOCK                                                 \
    0x0040                   /* Lock block before returning. NOT \
...
#define HAF_OBJECT_RESOURCE                       \
    0x0004 /* Block is object resource to be run  \
 ...
 */
extern MemHandle MemAlloc(word numBytes, short typeFlags, short allocFlags);

/*
 * Allocate a block and lock it, returning both the handle and the
 * address of the block.
 */
extern MemHandle MemAllocAndLock(
    short numBytes, short typeFlags, short allocFlags, void** addrPtr);
/*
 * Change the size of a block. Only HAF_ZERO_INIT is valid for allocFlags.
 * Returns non-zero if successful.
 */
extern int MemReAlloc(MemHandle handle, word newSize, short allocFlags);
 ...
 */
extern int MemReAllocAndLock(
    MemHandle handle, short newSize, short allocFlags, void** addrPtr);
/*
 ...
 */
extern void MemInfo(MemHandle handle, genptr* addrPtr, word* sizePtr);
/*
