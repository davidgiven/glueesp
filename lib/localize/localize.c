...
#define FOR_ALL_RESOURCES(__res, code)                             \
    ...
    {                                                              \
        ...
        Hash_Search hashSearch;                                    \
                                                                   \
        for (__ent = Hash_EnumFirst(&locHash, &hashSearch); __ent; \
...
#define FOR_ALL_LOCINFOS_IN_RESOURCE(__res, __locinfo, __code) \
    ...
    {                                                          \
        ...
        for (__locinfo = RES_INFO(__res); __locinfo;           \
 ...
 *
 ***********************************************************************/
void Localize_Init(char* fileName,   /* Regular output file,
                                      * from which name of
 ...
 *
 ***********************************************************************/
int Localize_EnterResource(Opaque key, /* 32-bit value that identifies the
                                        * resource */
 ...
 *
 ***********************************************************************/
void Localize_AddLocalization(LocalizeInfo* loc)
{
    if (loc == NULL)
    {
        return;
    ...
    if (RES_INSERT_INFO(currentResource) == NULL)
    {
         ...
         * the list.
         */
        RES_INSERT_INFO(currentResource) = RES_INFO(currentResource) = loc;
    }
    else
    {
        /*
 ...
 *
 ***********************************************************************/
void Localize_RemoveLocalization(LocalizeInfo* loc, Opaque resKey)
{
    Hash_Entry* resEnt;
    ResourceSym* res;
    ...
    for (prevPtr = &res->r_locInfo, prev = NULL; *prevPtr != NULL;
 ...
 *
 ***********************************************************************/
static void DumpLocInfo(LocalizeInfo* loc)
{
    int loc_min = 0, loc_max = 0, flags = 0;
    ...
    if (strchr(LOC_INST(loc) ? LOC_INST(loc) : "", '"') != NULL)
    {
        ...
        fprintf(
            locOut, "%s %d %d \"", LOC_NAME(loc), LOC_NUM(loc), LOC_HINT(loc));
 ...
 *
 ***********************************************************************/
void Localize_DumpLocalizations(void)
{
    int needFile = 0;
    ...
    if (needFile)
    {
        ...
        {
            fprintf(
                stderr, "couldn't open localization info file %s.", locName);
            return;
        }
        ...
        FOR_ALL_RESOURCES(res, {
            ...
            FOR_ALL_LOCINFOS_IN_RESOURCE(res, loc, { DumpLocInfo(loc); });
        });
